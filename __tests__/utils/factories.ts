...
export interface MockSupplier {
  ...
  name: string;
  email: string;
  category: 'photography' | 'catering' | 'venue' | 'florals' | 'music' | 'planning';
  location: string;
  referralCode: string;
...
export interface MockReferral {
  ...
  code: string;
  status: 'active' | 'clicked' | 'signup_started' | 'trial_started' | 'converted' | 'expired';
...
const WEDDING_CATEGORIES: MockSupplier['category'][] = [
  'photography', 'catering', 'venue', 'florals', 'music', 'planning'
];

const UK_LOCATIONS = [
  'London, UK', 'Manchester, UK', 'Birmingham, UK', 'Edinburgh, UK', 
  'Bristol, UK', 'Liverpool, UK', 'Brighton, UK', 'Bath, UK',
  'York, UK', 'Cambridge, UK', 'Oxford, UK', 'Canterbury, UK'
];
...
const REFERRAL_SOURCES: MockReferral['source'][] = [
  'dashboard', 'email', 'whatsapp', 'qr', 'direct'
];
...
export const createMockSupplier = (overrides: Partial<MockSupplier> = {}): MockSupplier => {
  ...
  const businessNames = {
    ...
    venue: () => `${faker.location.city()} ${faker.helpers.arrayElement(['Manor', 'Hall', 'House', 'Castle', 'Barn'])}`,
    florals: () => `${faker.company.name()} Florals`,
    music: () => `${faker.person.lastName()} ${faker.helpers.arrayElement(['DJ Services', 'Music', 'Entertainment'])}`,
  ...
  const baseSupplier: MockSupplier = {
    ...
    referralCode: faker.string.alphanumeric(8).toUpperCase().replace(/[0OI1]/g, '2'), // No confusing characters
  ...
  baseSupplier.conversionCount = Math.floor(baseSupplier.referralCount * faker.number.float({ min: 0.2, max: 0.6 }));
 ...
 */
export const createMockReferral = (overrides: Partial<MockReferral> = {}): MockReferral => {
  ...
  const baseReferral: MockReferral = {
    ...
    referredId: faker.datatype.boolean({ probability: 0.6 }) ? faker.string.uuid() : null, // 60% chance of conversion
    code: faker.string.alphanumeric(8).toUpperCase().replace(/[0OI1]/g, '2'),
    status: faker.helpers.arrayElement(['active', 'clicked', 'signup_started', 'trial_started', 'converted']),
    conversionStage: null,
    customMessage: faker.datatype.boolean({ probability: 0.4 }) ? faker.lorem.sentence() : null,
    ...
    metadata: {
      ...
      userAgent,
      location: faker.helpers.arrayElement(UK_LOCATIONS),
      device: isMobile ? 'mobile' : faker.helpers.arrayElement(['desktop', 'tablet'])
    },
    createdAt,
    convertedAt: null,
    expiresAt
  ...
  if (baseReferral.status === 'converted') {
    baseReferral.convertedAt = faker.date.between({ from: createdAt, to: new Date() });
 ...
 */
export const createMockLeaderboardEntry = (overrides: Partial<MockLeaderboardEntry> = {}): MockLeaderboardEntry => {
  const totalReferrals = faker.number.int({ min: 5, max: 100 });
  const paidConversions = Math.floor(totalReferrals * faker.number.float({ min: 0.2, max: 0.8 }));
  const conversionRate = parseFloat(((paidConversions / totalReferrals) * 100).toFixed(2));
 ...
 */
export const createReferralChain = (depth: number = 3): {
 ...
 */
export const createLoadTestData = (supplierCount: number = 100): {
  suppliers: MockSupplier[];
  referrals: MockReferral[];
  leaderboard: MockLeaderboardEntry[];
} => {
  const suppliers = Array.from({ length: supplierCount }, () => createMockSupplier());
  ...
  suppliers.forEach(supplier => {
    ...
    for (let i = 0; i < referralCount; i++) {
      referrals.push(createMockReferral({
        referrerId: supplier.id,
        status: faker.helpers.arrayElement(['active', 'converted', 'expired'])
      }));
    }
  });
...
export const createApiTestData = () => {
  return {
    validCreateReferralRequest: {
      customMessage: 'Join me on WedSync - the best platform for wedding suppliers!',
      source: 'dashboard'
    },
