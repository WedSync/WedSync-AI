-- Dashboard Templates System Migration (SAFE VERSION)
...
-- 1. Dashboard Template Categories (Handle existing type)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'dashboard_template_category') THEN
...
-- 2. Dashboard Section Types (Handle existing type)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'dashboard_section_type') THEN
...
CREATE TABLE IF NOT EXISTS dashboard_template_sections (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS dashboard_section_library (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Template lookups
CREATE INDEX IF NOT EXISTS idx_dashboard_templates_supplier ON dashboard_templates(supplier_id);
CREATE INDEX IF NOT EXISTS idx_dashboard_templates_category ON dashboard_templates(category);
CREATE INDEX IF NOT EXISTS idx_dashboard_templates_active ON dashboard_templates(is_active) WHERE is_active = true;

-- Section queries
CREATE INDEX IF NOT EXISTS idx_template_sections_template ON dashboard_template_sections(template_id);
CREATE INDEX IF NOT EXISTS idx_template_sections_type ON dashboard_template_sections(section_type);
CREATE INDEX IF NOT EXISTS idx_template_sections_position ON dashboard_template_sections(template_id, sort_order);

-- Client assignments
CREATE INDEX IF NOT EXISTS idx_client_assignments_client ON client_template_assignments(client_id);
CREATE INDEX IF NOT EXISTS idx_client_assignments_template ON client_template_assignments(template_id);
CREATE INDEX IF NOT EXISTS idx_client_assignments_supplier ON client_template_assignments(supplier_id);
CREATE INDEX IF NOT EXISTS idx_client_assignments_active ON client_template_assignments(is_active) WHERE is_active = true;

-- Assignment rules
CREATE INDEX IF NOT EXISTS idx_assignment_rules_template ON template_assignment_rules(template_id);
CREATE INDEX IF NOT EXISTS idx_assignment_rules_priority ON template_assignment_rules(supplier_id, priority DESC);
CREATE INDEX IF NOT EXISTS idx_assignment_rules_active ON template_assignment_rules(is_active) WHERE is_active = true;

-- Performance metrics
CREATE INDEX IF NOT EXISTS idx_performance_template_date ON template_performance_metrics(template_id, date_bucket);
CREATE INDEX IF NOT EXISTS idx_performance_render_time ON template_performance_metrics(render_time_ms);

-- Section library
CREATE INDEX IF NOT EXISTS idx_section_library_type ON dashboard_section_library(section_type);
CREATE INDEX IF NOT EXISTS idx_section_library_category ON dashboard_section_library(category);
CREATE INDEX IF NOT EXISTS idx_section_library_active ON dashboard_section_library(is_active) WHERE is_active = true;
...
-- 12. Update triggers for updated_at timestamps
