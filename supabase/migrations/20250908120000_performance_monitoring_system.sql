-- Migration: 20250908120000_performance_monitoring_system.sql
-- WS-339: Performance Monitoring System
-- Comprehensive schema for APM, metrics, alerts, and service health monitoring
-- Optimized for high-frequency inserts and wedding-specific monitoring needs
...
-- Optimized for high-frequency inserts with time-series data
-- =============================================
...
-- =============================================
-- PERFORMANCE ALERT INCIDENTS TABLE
-- Track alert firing and resolution history
-- =============================================
CREATE TABLE performance_alert_incidents (
    id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
    organization_id uuid NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE INDEX idx_performance_metrics_org_timestamp ON performance_metrics(organization_id, timestamp DESC);
CREATE INDEX idx_performance_metrics_service_metric ON performance_metrics(service_name, metric_name, timestamp DESC);
CREATE INDEX idx_performance_metrics_wedding_day ON performance_metrics(organization_id, timestamp) 
...
CREATE INDEX idx_performance_metrics_wedding_context ON performance_metrics USING GIN(wedding_context);
...
CREATE INDEX idx_performance_alerts_severity ON performance_alerts(severity, is_active);
CREATE INDEX idx_performance_alerts_wedding_override ON performance_alerts(organization_id) 
    WHERE wedding_day_override = true;

-- Service health status indexes
CREATE INDEX idx_service_health_org_status ON service_health_status(organization_id, status);
CREATE INDEX idx_service_health_wedding_critical ON service_health_status(organization_id, wedding_critical);
CREATE INDEX idx_service_health_next_check ON service_health_status(next_check_at) 
    WHERE status != 'maintenance';

-- Alert incidents indexes
CREATE INDEX idx_alert_incidents_alert_triggered ON performance_alert_incidents(alert_id, triggered_at DESC);
CREATE INDEX idx_alert_incidents_org_status ON performance_alert_incidents(organization_id, status);
CREATE INDEX idx_alert_incidents_wedding_day ON performance_alert_incidents(organization_id, wedding_date) 
...
ALTER TABLE performance_alert_incidents ENABLE ROW LEVEL SECURITY;

-- Performance metrics policies
CREATE POLICY "Users can view own organization performance metrics" ON performance_metrics
    FOR SELECT USING (
        organization_id IN (
...
CREATE POLICY "Users can insert own organization performance metrics" ON performance_metrics
...
CREATE POLICY "Users can manage own organization alerts" ON performance_alerts
    FOR ALL USING (
        organization_id IN (
            SELECT organization_id FROM user_profiles 
            WHERE user_id = auth.uid()
...
-- Service health status policies
CREATE POLICY "Users can view own organization service health" ON service_health_status
    FOR SELECT USING (
        organization_id IN (
            SELECT organization_id FROM user_profiles 
            WHERE user_id = auth.uid()
...
CREATE POLICY "Service role can update service health" ON service_health_status
    FOR ALL TO service_role USING (true);

-- Alert incidents policies
CREATE POLICY "Users can view own organization alert incidents" ON performance_alert_incidents
    FOR SELECT USING (
        organization_id IN (
            SELECT organization_id FROM user_profiles 
            WHERE user_id = auth.uid()
...
CREATE POLICY "Users can update own organization alert incidents" ON performance_alert_incidents
    FOR UPDATE USING (
        organization_id IN (
            SELECT organization_id FROM user_profiles 
            WHERE user_id = auth.uid()
...
CREATE OR REPLACE FUNCTION is_wedding_day(org_id uuid, check_date date DEFAULT CURRENT_DATE)
RETURNS boolean
LANGUAGE sql
STABLE
AS $$
...
BEGIN
    ...
    VALUES (
        ...
        now(), now() + check_interval, now()
    )
    ON CONFLICT (organization_id, service_name)
    DO UPDATE SET
        status = p_status,
        response_time_ms = COALESCE(p_response_time_ms, service_health_status.response_time_ms),
        error_rate = COALESCE(p_error_rate, service_health_status.error_rate),
        availability_percentage = COALESCE(v_availability, service_health_status.availability_percentage),
        status_history = v_status_history,
        last_check_at = now(),
        next_check_at = now() + service_health_status.check_interval,
        updated_at = now();
END;
$$;
...
DECLARE
    ...
    metric_cursor CURSOR FOR
        SELECT DISTINCT organization_id, service_name, metric_name
...
BEGIN
    ...
    LOOP
        -- This would contain the logic to evaluate metrics against alert thresholds
        -- Implementation would depend on specific alert evaluation requirements
        NULL;
    END LOOP;
END;
$$;
...
-- Update timestamp on apm_configurations changes
...
CREATE TRIGGER update_apm_configurations_updated_at
    BEFORE UPDATE ON amp_configurations
    FOR EACH ROW
...
COMMENT ON TABLE performance_metrics IS 'High-frequency time-series data for application performance monitoring';
COMMENT ON TABLE apm_configurations IS 'Configuration for various APM providers with encrypted credentials';
COMMENT ON TABLE performance_alerts IS 'Alert rules with wedding-day specific overrides and thresholds';
COMMENT ON TABLE service_health_status IS 'Health monitoring for third-party services critical to wedding operations';
COMMENT ON TABLE performance_alert_incidents IS 'Historical record of alert firings and resolutions';

COMMENT ON COLUMN performance_metrics.wedding_context IS 'Wedding-specific context like wedding_date, vendor_type, is_wedding_day for filtering';
COMMENT ON COLUMN performance_alerts.wedding_day_override IS 'Enable stricter monitoring thresholds on wedding days';
COMMENT ON COLUMN service_health_status.wedding_critical IS 'Flag services that are critical for wedding day operations';

-- Add helpful views for common queries
CREATE VIEW wedding_day_performance_metrics AS
SELECT 
    pm.*,
    c.wedding_date,
    c.partner_1_name || ' & ' || c.partner_2_name as couple_names
FROM performance_metrics pm
JOIN clients c ON c.organization_id = pm.organization_id
WHERE (pm.wedding_context->>'is_wedding_day')::boolean = true
    AND DATE(pm.timestamp) = c.wedding_date;

COMMENT ON VIEW wedding_day_performance_metrics IS 'Performance metrics specifically for wedding days with couple context';