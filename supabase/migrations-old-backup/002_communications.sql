-- Communications System Tables for WedSync/WedMe
...
CREATE TABLE IF NOT EXISTS public.notification_preferences (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE(COALESCE(user_id, contact_id), organization_id, channel, event_type)
);
...
CREATE POLICY "Users can send messages in their organization's conversations"
...
CREATE POLICY "Users can create email notifications for their organization"
...
-- Activity Feeds RLS
CREATE POLICY "Users can view their organization's activity feeds"
    ON public.activity_feeds FOR SELECT
    USING (organization_id IN (
        SELECT organization_id FROM public.user_profiles WHERE user_id = auth.uid()
    ));

CREATE POLICY "Users can create activity entries for their organization"
    ON public.activity_feeds FOR INSERT
    WITH CHECK (organization_id IN (
        SELECT organization_id FROM public.user_profiles WHERE user_id = auth.uid()
    ));
...
GRANT ALL ON public.activity_feeds TO authenticated;
