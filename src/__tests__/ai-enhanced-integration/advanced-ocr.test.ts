...
describe('WS-164: AI-Enhanced Manual Tracking Integration', () => {
  ...
  describe('Receipt Processing', () => {
    test('should process receipt image with 90%+ accuracy', async () => {
      ...
      vi.spyOn(ocrTracker as any, 'performAdvancedOCR').mockResolvedValue({
      ...
      vi.spyOn(ocrTracker['duplicateDetector'], 'checkForDuplicates').mockResolvedValue({
        is_duplicate: false,
        confidence: 0.95,
        matching_expenses: [],
        recommendation: 'keep_separate'
      // Mock fraud detection
      vi.spyOn(ocrTracker['fraudDetector'], 'analyzeExpense').mockResolvedValue({
        fraud_risk: 'low',
        risk_score: 0.15,
        risk_factors: [],
    ...
    test('should extract receipt data with NLP enhancement', async () => {
      ...
      vi.spyOn(ocrTracker as any, 'standardizeVendorName').mockResolvedValue('Joe\'s Floral & Garden');
      vi.spyOn(ocrTracker as any, 'enhanceItemDescriptions').mockResolvedValue([
        {
      ...
      const enhanced = await (ocrTracker as unknown).enhanceWithNLP(mockReceiptData);
    ...
    test('should detect duplicate expenses accurately', async () => {
      ...
      vi.spyOn(ocrTracker['duplicateDetector'] as any, 'findSimilarExpenses').mockResolvedValue([
          id: 'expense-123',
          vendor_name: 'Duplicate Test Vendor',
          amount: 500.00,
          date: '2025-01-20',
  ...
  describe('Bank Reconciliation', () => {
    test('should reconcile bank transactions with receipts', async () => {
      ...
      vi.spyOn(ocrTracker as any, 'findMatchingExpenses').mockResolvedValue([
  ...
  describe('Performance and Accuracy Tests', () => {
    test('should meet OCR accuracy requirements', async () => {
      ...
      for (const testCase of testReceipts) {
        ...
        vi.spyOn(ocrTracker as any, 'performAdvancedOCR').mockResolvedValueOnce({
          vendor_name: testCase.expected.vendor,
          total_amount: testCase.expected.amount,
          transaction_date: testCase.expected.date,
          confidence_score: 0.92
        });
        const result = await (ocrTracker as unknown).performAdvancedOCR(testCase.image);
  ...
  describe('Error Handling', () => {
    test('should handle corrupted images gracefully', async () => {
      ...
      // Mock OCR failure
      vi.spyOn(ocrTracker as any, 'performAdvancedOCR').mockRejectedValue(
    ...
    test('should provide fallback for AI service failures', async () => {
      ...
      vi.spyOn(ocrTracker as any, 'enhanceWithNLP').mockRejectedValue(
