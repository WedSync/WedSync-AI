...
describe('WS-190 Evidence Preservation Service', () => {
  ...
  beforeEach(() => {
    mockSupabase = {
      ...
      storage: {
        from: vi.fn().mockReturnThis(),
        upload: vi.fn().mockResolvedValue({ data: { path: 'evidence/test.json' }, error: null }),
    ...
    mockCrypto = {
      createHash: vi.fn().mockReturnValue({
        update: vi.fn().mockReturnThis(),
        digest: vi.fn().mockReturnValue('mocked_hash_123')
      }),
      createCipher: vi.fn().mockReturnValue({
        update: vi.fn().mockReturnValue('encrypted_'),
        final: vi.fn().mockReturnValue('_data')
      createDecipher: vi.fn().mockReturnValue({
        update: vi.fn().mockReturnValue('decrypted_'),
      randomBytes: vi.fn().mockReturnValue(Buffer.from('random_key_data'))
    ;(createClient as ReturnType<typeof vi.fn>edFunction<typeof createClient>).mockReturnValue(mockSupabase)
    ;(crypto as unknown).createHash = mockCrypto.createHash
    ;(crypto as unknown).createCipher = mockCrypto.createCipher
    ;(crypto as unknown).createDecipher = mockCrypto.createDecipher
    ;(crypto as unknown).randomBytes = mockCrypto.randomBytes
  ...
  describe('Evidence Integrity Verification', () => {
    test('should verify evidence integrity using cryptographic hashes', async () => {
      ...
      mockCrypto.createHash.mockReturnValue({
        digest: vi.fn().mockReturnValue('original_hash_123')
    ...
    test('should detect evidence tampering', async () => {
      ...
      // Mock hash verification - different from original (indicates tampering)
        digest: vi.fn().mockReturnValue('different_hash_456')
