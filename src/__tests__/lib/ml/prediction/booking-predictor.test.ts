...
describe('BookingPredictor', () => {
  ...
  describe('Core Prediction Logic', () => {
    test('should predict high booking probability for 24hr questionnaire completion', async () => {
      ...
      vi.spyOn(predictor as any, 'fetchClientData').mockResolvedValue(mockClientData)
    ...
    test('should predict low booking probability for 5+ day questionnaire delay', async () => {
      ...
      vi.spyOn(predictor as any, 'fetchClientData').mockResolvedValue(delayedClient)
    ...
    test('should handle client with no questionnaire completion', async () => {
      ...
      vi.spyOn(predictor as any, 'fetchClientData').mockResolvedValue(incompleteClient)
  ...
  describe('Feature Extraction', () => {
    test('should correctly extract timing features', () => {
      const features = (predictor as unknown).extractFeatures(mockClientData)
    ...
    test('should handle edge cases in feature extraction', () => {
      const edgeCaseClient = {
        ...
        page_views_total: 0,
        response_time_avg: 0
      const features = (predictor as unknown).extractFeatures(edgeCaseClient)
  ...
  describe('Prediction Model Logic', () => {
    test('should implement the 24hr vs 5+ day requirement accurately', async () => {
      ...
      const slowClient = {
        ...
        engagement_score: 30
      vi.spyOn(predictor as any, 'fetchClientData')
    ...
    test('should apply venue booking status correctly', async () => {
      const venueBookedClient = {
        venue_booked: true
      vi.spyOn(predictor as any, 'fetchClientData').mockResolvedValue(venueBookedClient)
  ...
  describe('Batch Processing', () => {
    test('should handle batch predictions correctly', async () => {
      ...
      vi.spyOn(predictor, 'predictBookingProbability')
  ...
  describe('Confidence Calculation', () => {
    test('should calculate confidence based on data completeness', () => {
      ...
      const highConfidence = (predictor as unknown).calculateConfidence(highQualityFeatures, 0.75)
      ...
      const lowConfidence = (predictor as unknown).calculateConfidence(lowQualityFeatures, 0.25)
    ...
    test('should increase confidence for extreme predictions', () => {
      ...
      const highProbConfidence = (predictor as unknown).calculateConfidence(features, 0.9)
      const lowProbConfidence = (predictor as unknown).calculateConfidence(features, 0.1)
      const midProbConfidence = (predictor as unknown).calculateConfidence(features, 0.5)
  ...
  describe('Risk Indicators', () => {
    test('should identify risk indicators correctly', () => {
      ...
      const riskClient = {
        engagement_score: 20
      const riskIndicators = (predictor as unknown).identifyRiskIndicators(riskFeatures, riskClient)
  ...
  describe('Error Handling', () => {
    test('should handle client data fetch errors', async () => {
      vi.spyOn(predictor as any, 'fetchClientData').mockRejectedValue(new Error('Database error'))
    ...
    test('should handle invalid client data gracefully', async () => {
      ...
      vi.spyOn(predictor as any, 'fetchClientData').mockResolvedValue(invalidClient)
  ...
  describe('Caching', () => {
    test('should cache predictions for performance', async () => {
      ...
      vi.spyOn(predictor as any, 'getCachedPrediction')
        .mockReturnValueOnce(null) // First call - no cache
        .mockReturnValueOnce(mockPrediction) // Second call - cached
      vi.spyOn(predictor as any, 'setCachedPrediction').mockImplementation()
  ...
  describe('Validation Against Historical Data', () => {
    test('should validate predictions against outcomes', async () => {
      ...
      // Mock Supabase query
      vi.spyOn(predictor['supabase'], 'from').mockReturnValue({
