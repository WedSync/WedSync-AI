...
describe('CommunicationsSecurity', () => {
  ...
  describe('validateBulkMessageRequest', () => {
    ...
    beforeEach(() => {
      ...
      const mockValidateGuestOwnership = vi.spyOn(CommunicationsSecurity, 'validateGuestOwnership')
        .mockResolvedValue({ isValid: true, invalidGuestIds: [] });
      // Mock successful rate limiting
      const mockRateLimitMessaging = vi.spyOn(CommunicationsSecurity, 'rateLimitMessaging')
        .mockResolvedValue({
          allowed: true,
          remaining: 4,
    ...
    it('should reject request with invalid guest ownership', async () => {
      vi.spyOn(CommunicationsSecurity, 'validateGuestOwnership')
    ...
    it('should reject request when rate limit exceeded', async () => {
      vi.spyOn(CommunicationsSecurity, 'rateLimitMessaging')
          allowed: false,
          remaining: 0,
