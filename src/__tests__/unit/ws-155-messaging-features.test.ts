...
describe('WS-155: Voice Message Integration', () => {
  ...
  it('should get preferred mime type for recording', () => {
    ...
    const voiceIntegration = new (voiceMessageIntegration.constructor as unknown)();
...
describe('WS-155: Advanced Offline Sync', () => {
    // Mock IndexedDB
    const mockDB = {
      syncQueue: { add: vi.fn(), count: jest.fn(() => 0), clear: vi.fn() },
      conflicts: { add: vi.fn(), get: vi.fn(), update: vi.fn() },
      metadata: { get: vi.fn(), put: vi.fn(), clear: vi.fn() },
      messages: { bulkPut: vi.fn() },
      guests: { bulkPut: vi.fn() },
      templates: { bulkPut: vi.fn() }
    (advancedOfflineSync as unknown).db = mockDB;
  it('should queue operation for sync', async () => {
    ...
    const db = (advancedOfflineSync as unknown).db;
  ...
  it('should handle conflict resolution strategies', async () => {
    ...
    const hasConflict = (advancedOfflineSync as unknown).hasConflict(localData, serverData);
