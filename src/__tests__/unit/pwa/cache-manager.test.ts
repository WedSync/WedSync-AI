...
Object.defineProperty(global, 'CompressionStream', {
  value: vi.fn().mockImplementation(() => mockCompressionStream),
  writable: true
});
Object.defineProperty(global, 'DecompressionStream', {
  value: vi.fn().mockImplementation(() => mockDecompressionStream),
...
Object.defineProperty(global, 'Blob', {
  value: vi.fn().mockImplementation((data: any[]) => ({
...
describe('PWACacheManager', () => {
  ...
  beforeEach(() => {
    ...
    test('should preload critical wedding data', async () => {
      ...
      vi.spyOn(cacheManager as any, 'fetchWeddingData').mockImplementation(
  ...
  beforeEach(() => {
    ...
    test('should handle compression gracefully when not supported', async () => {
      ...
      const originalCompressionSupported = (cacheManager as unknown).compressionSupported;
      (cacheManager as unknown).compressionSupported = false;
      const data = { data: 'test' };
      const key = 'test:data';
      const setResult = await cacheManager.set(key, data, { compress: true });
      expect(retrieved).toEqual(data);
      // Restore original value
      (cacheManager as unknown).compressionSupported = originalCompressionSupported;
  ...
  beforeEach(() => {
    ...
    test('should handle storage quota exceeded errors gracefully', async () => {
      ...
      vi.spyOn(cacheManager as any, 'hasSpaceFor').mockReturnValue(false);
      vi.spyOn(cacheManager as any, 'makeSpaceFor').mockRejectedValue(
  ...
  beforeEach(() => {
    ...
    test('should clean up expired entries during cleanup', async () => {
      ...
      await (cacheManager as unknown).cleanup();
