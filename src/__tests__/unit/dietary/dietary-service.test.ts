...
vi.mock('@supabase/supabase-js', () => ({
  createClient: jest.fn(() => ({
    from: jest.fn(() => ({
      insert: vi.fn().mockResolvedValue({ data: mockRequirement, error: null }),
      update: vi.fn().mockResolvedValue({ data: mockRequirement, error: null }),
      select: vi.fn().mockResolvedValue({ data: [mockRequirement], error: null }),
      eq: vi.fn().mockReturnThis(),
      single: vi.fn().mockResolvedValue({ data: mockRequirement, error: null }),
...
describe('DietaryService - Critical Allergy Handling', () => {
  ...
  describe('generateDietaryMatrix', () => {
    it('should generate matrix within 2 seconds for 500 guests', async () => {
      ...
      vi.spyOn(dietaryService as any, 'supabase', 'get').mockReturnValue({
        from: vi.fn().mockReturnValue({
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockResolvedValue({ data: mockRequirements, error: null }),
          }),
        }),
      });
  ...
  describe('generateCatererExport', () => {
    it('should include all critical medical alerts in export', async () => {
      ...
      vi.spyOn(dietaryService, 'generateDietaryMatrix').mockResolvedValue(mockMatrix);
    ...
    it('should generate proper kitchen cards with severity indicators', async () => {
      ...
      const kitchenCards = (dietaryService as unknown).generateKitchenCards(mockMatrix);
