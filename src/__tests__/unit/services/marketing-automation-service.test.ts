...
const mockABTestingService = {
  getVariantForClient: vi.fn(),
  recordEvent: vi.fn()
const mockAIEmailGenerator = {
  generateEmailTemplate: vi.fn()
describe('MarketingAutomationService', () => {
  ...
  beforeEach(() => {
    ...
    (require('@/lib/services/email-service').EmailService as ReturnType<typeof vi.fn>)
      .mockImplementation(() => mockEmailService);
    (require('@/lib/services/ab-testing-service').ABTestingService as ReturnType<typeof vi.fn>)
      .mockImplementation(() => mockABTestingService);
    (require('@/lib/services/ai-email-generator').aiEmailGenerator as unknown) = mockAIEmailGenerator;
  ...
  describe('launchCampaign', () => {
    ...
    it('should launch campaign for all target clients', async () => {
      ...
      const serviceSpy = vi.spyOn(service, 'startCampaignExecution')
        .mockResolvedValueOnce({ id: 'exec-1' } as unknown)
        .mockResolvedValueOnce({ id: 'exec-2' } as unknown);
  ...
  describe('processScheduledExecutions', () => {
    it('should process all scheduled executions', async () => {
      ...
      const processSpy = vi.spyOn(service, 'processExecutionStep')
        .mockResolvedValue();
  ...
  describe('personalizeTemplate', () => {
    it('should replace template variables with client data', () => {
      ...
      const result = (service as unknown).personalizeTemplate(template, client, campaign);
  ...
  describe('convertHtmlToText', () => {
    it('should strip HTML tags and convert entities', () => {
      ...
      const result = (service as unknown).convertHtmlToText(html);
