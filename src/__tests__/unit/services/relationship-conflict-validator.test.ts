...
vi.mock('@/lib/realtime/guest-sync', () => ({
  __esModule: true,
  default: {
    subscribeToIntegrationUpdates: vi.fn(),
    broadcastIntegrationUpdate: vi.fn()
  }
describe('RelationshipConflictValidator', () => {
  ...
  describe('validateEntireSeatingPlan', () => {
    test('validates multiple tables efficiently', async () => {
      ...
      validator.getSeatingPlan = vi.fn().mockResolvedValue(mockSeatingPlan)
      ...
      validator.validateSeatingConflict = vi.fn()
