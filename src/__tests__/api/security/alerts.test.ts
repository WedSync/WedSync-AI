...
vi.mock('@/lib/security/incident-response-system')
...
describe('WS-190 Security Alerts API', () => {
  ...
  beforeEach(() => {
    ...
    ;(createClient as ReturnType<typeof vi.fn>edFunction<typeof createClient>).mockReturnValue(mockSupabase)
    ;(crypto as unknown).createHmac = mockCrypto.createHmac
    ;(crypto as unknown).timingSafeEqual = mockCrypto.timingSafeEqual
    mockRequest = {
      ip: '192.168.1.100',
      json: vi.fn(),
  ...
  describe('POST /api/security/alerts - Webhook Alert Processing', () => {
    test('should process valid webhook alert from Cloudflare WAF', async () => {
      ...
      mockRequest.json = vi.fn().mockResolvedValue(cloudflareAlert)
      ...
      const mockIncidentSystem = {
        processSecurityAlert: vi.fn().mockResolvedValue({
          alert: {
    ...
    test('should reject webhook with invalid signature', async () => {
      ...
      mockRequest.json = vi.fn().mockResolvedValue(suspiciousAlert)
    ...
    test('should process AWS GuardDuty threat intelligence alert', async () => {
      ...
      mockRequest.json = vi.fn().mockResolvedValue(guardDutyAlert)
    ...
    test('should handle authenticated API alert submission', async () => {
      ...
      mockRequest.json = vi.fn().mockResolvedValue(manualAlert)
    ...
    test('should handle organization extraction from webhook alert data', async () => {
      ...
      mockRequest.json = vi.fn().mockResolvedValue(alertWithOrgData)
  ...
  describe('Alert Validation and Error Handling', () => {
    test('should validate alert schema with comprehensive checks', async () => {
      ...
      mockRequest.json = vi.fn().mockResolvedValue(invalidAlert)
    ...
    test('should handle database errors gracefully', async () => {
      ...
      mockRequest.json = vi.fn().mockResolvedValue(validAlert)
      // Mock database error
        processSecurityAlert: vi.fn().mockRejectedValue(
  ...
  describe('Wedding-Specific Alert Processing', () => {
    test('should escalate venue-related security alerts', async () => {
      ...
      mockRequest.json = vi.fn().mockResolvedValue(venueAlert)
            id: 'alert-venue-123',
