...
describe('Payment Calendar Integration Tests', () => {
  ...
  let mockBudgetService: unknown;
  let mockNotificationService: unknown;
  let mockVendorSyncService: unknown;
  let mockCashFlowService: unknown;
  const mockWeddingId = 'wedding-123';
  const mockUserId = 'user-456';
  beforeEach(() => {
    // Initialize mocked services
    mockBudgetService = new BudgetIntegrationService() as ReturnType<typeof vi.fn>ed<BudgetIntegrationService>;
    mockNotificationService = new NotificationService() as ReturnType<typeof vi.fn>ed<NotificationService>;
    mockVendorSyncService = new VendorPaymentSyncService() as ReturnType<typeof vi.fn>ed<VendorPaymentSyncService>;
    mockCashFlowService = new CashFlowCalculatorService() as ReturnType<typeof vi.fn>ed<CashFlowCalculatorService>;
