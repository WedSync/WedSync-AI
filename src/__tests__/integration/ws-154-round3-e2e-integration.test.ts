...
vi.mock('@/lib/api/seating', () => ({
  optimizeSeating: vi.fn(),
  saveArrangement: vi.fn(),
  loadArrangement: vi.fn(),
  validateArrangement: vi.fn()
}))
vi.mock('@/lib/services/realtime-sync', () => ({
  subscribeToArrangementUpdates: vi.fn(),
  publishArrangementUpdate: vi.fn(),
  disconnectFromRealtime: vi.fn()
...
describe('WS-154 Round 3: End-to-End Integration Testing', () => {
  ...
  describe('Complete Seating Workflow Integration', () => {
    it('should complete full seating arrangement lifecycle with all team integrations', async () => {
      ...
      const mockOnUpdateArrangement = vi.fn()
    ...
    it('should implement error handling with graceful degradation', async () => {
      ...
      for (const test of networkFailureTests) {
        ...
        global.fetch = vi.fn().mockRejectedValue(test.mockError)
