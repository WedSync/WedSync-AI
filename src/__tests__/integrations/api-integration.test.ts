...
class MockZapierAPI {
  private triggeredZaps: Array<{ zapId: string; data: unknown; timestamp: string }> = [];
  ...
  getTriggeredZaps(): Array<{ zapId: string; data: unknown; timestamp: string }> {
...
describe('API Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    global.fetch = vi.fn() as ReturnType<typeof vi.fn>edFunction<typeof fetch>;
  });
  afterEach(() => {
  ...
  describe('API Connectors', () => {
    it('should create Stripe customer correctly', async () => {
      const mockFetch = global.fetch as ReturnType<typeof vi.fn>edFunction<typeof fetch>;
      const mockCustomer = {
        id: 'cus_test123',
      ...
      const connector = new StripeConnector('TEST_STRIPE_KEY_PLACEHOLDER');
      const vendorData = {
        id: 'vendor_123',
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          headers: expect.objectContaining({
            'Authorization': 'Bearer TEST_STRIPE_KEY_PLACEHOLDER',
          }),
        })
      );
