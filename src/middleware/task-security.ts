...
export class TaskSecurityMiddleware {
  private rate_limit_cache: Map<string, { count: number; reset_time: number }> =
    new Map();
  ...
  async auditTaskOperation(
    ...
    try {
      ...
      if (
        audit_record.severity === 'high' ||
        audit_record.severity === 'critical'
      ) {
  ...
  private async validatePermissions(
    ...
    try {
      ...
      const has_permission = action_permissions.some(
        (permission) =>
  ...
  private async validateResourceAccess(
    ...
    try {
      ...
      if (resource.created_by === user.id || resource.assigned_to === user.id)
        return true;
  ...
  private getRequiredPermissions(action: string): string[] {
    ...
    return (
      permission_map[action as keyof typeof permission_map] || ['tasks.read']
    );
  }
  ...
  private async alertSecurityTeam(
    audit_record: TaskOperationAudit,
  ): Promise<void> {
    try {
  ...
  async validateTemplateAccess(
    ...
    try {
      ...
      const authz_result = await this.validatePermissions(
        auth_result.user!,
        action,
        organization_id,
      );
      ...
      if (template_id) {
        const template_access = await this.validateTemplateOwnership(
          auth_result.user!,
          template_id,
          organization_id,
        );
      ...
      if (
        auth_result.user!.organization_id !== organization_id &&
        !auth_result.user!.permissions.includes('admin')
      ) {
        return {
          valid: false,
   ...
   */
  async auditTemplateOperation(
    audit_record: Omit<TaskOperationAudit, 'created_at'> & {
      organization_id: string;
    },
  ): Promise<void> {
    try {
