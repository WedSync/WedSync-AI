...
export class WebhookProcessor {
  ...
  private initializeWebhookConfigs(): void {
    const configs: WebhookConfig[] = [
      ...
      },
      {
        ...
        secret:
          process.env.SUPPLIER_WEBHOOK_SECRET || 'default-supplier-secret',
    ...
    console.info(
      `[WebhookProcessor] Initialized ${configs.length} webhook configurations`,
    );
  }
  ...
  async processWebhook(
    ...
    try {
      ...
      if (!config) {
        console.warn(
          `[WebhookProcessor] Provider not configured: ${providerId}`,
          { requestId },
        );
      ...
      if (!signature) {
        console.warn(
          `[WebhookProcessor] Missing signature header: ${config.signatureHeader}`,
          {
            requestId,
            providerId,
          },
      ...
      if (!isValidSignature) {
        console.warn(
          `[WebhookProcessor] Invalid webhook signature from ${providerId}`,
          {
            requestId,
      ...
      try {
        ...
        console.error(
          `[WebhookProcessor] Invalid JSON payload from ${providerId}:`,
          {
            requestId,
      ...
      if (!config.eventTypes.includes(eventType)) {
        console.info(
          `[WebhookProcessor] Ignoring webhook event type: ${eventType}`,
          {
            requestId,
            providerId,
      ...
      console.error(
        `[WebhookProcessor] Webhook processing error for ${providerId}:`,
        {
          requestId,
          error: error instanceof Error ? error.message : 'Unknown error',
          duration: Date.now() - startTime,
  ...
  private async verifySignature(
    ...
    try {
      switch (config.signatureFormat) {
        ...
        case 'hmac-sha256':
          const expectedHmac = createHmac('sha256', config.secret)
            .update(payload, 'utf8')
            .digest('hex');
        ...
        case 'sha256':
          ...
          return timingSafeEqual(
            Buffer.from(expectedSha256),
            Buffer.from(receivedSha256),
          );
        ...
        case 'sha1':
          ...
          return timingSafeEqual(
            ...
            Buffer.from(receivedSha1),
          );

        default:
          console.error(
            `[WebhookProcessor] Unsupported signature format: ${config.signatureFormat}`,
          );
          return false;
      }
    } catch (error) {
  ...
  private verifyStripeSignature(
    ...
    if (Math.abs(now - timestampNum) > 300) {
      console.warn(
        `[WebhookProcessor] Stripe webhook timestamp outside tolerance`,
      );
      return false;
    }
  ...
  private extractEventType(providerId: string, payload: Record<string, unknown>): string {
    switch (providerId) {
      case 'stripe':
      ...
      case 'sms_provider':
        return payload.MessageStatus
          ? `message.${payload.MessageStatus.toLowerCase()}`
          : 'unknown';
      default:
  ...
  private extractWeddingId(payload: Record<string, unknown>): string | undefined {
    // Extract wedding ID from various payload structures
    return (
      payload.metadata?.wedding_id ||
  ...
  private extractSupplierId(payload: Record<string, unknown>): string | undefined {
  ...
  private determinePriority(
    ...
    const lowPriorityEvents = ['email.opened', 'email.clicked', 'message.read'];
  ...
  private async processQueue(
    ...
    try {
      ...
      console.error(
        `[WebhookProcessor] Queue processing error for ${priority}:`,
        {
          error: error instanceof Error ? error.message : 'Unknown error',
        },
      );
  ...
  private async processWebhookEvent(event: WebhookEvent): Promise<void> {
    ...
    if (!processor) {
      console.warn(
        `[WebhookProcessor] No processor found for ${processorKey}`,
        {
          eventId: event.id,
        },
      );
    ...
    while (attempt < config.retryPolicy.maxAttempts) {
      try {
        ...
        console.info(
          `[WebhookProcessor] Webhook event processed successfully`,
          {
            eventId: event.id,
            providerId: event.providerId,
            eventType: event.eventType,
        ...
        if (attempt < config.retryPolicy.maxAttempts) {
          ...
          await this.sendToDeadLetterQueue(
            event,
            config.retryPolicy.deadLetterQueue,
          );
        }
      }
    }
  ...
  private registerEventProcessors(): void {
    ...
    this.eventProcessors.set(
      'supplier_platform.booking.confirmed',
      async (payload, context) => {
    ...
    this.eventProcessors.set(
      'supplier_platform.booking.cancelled',
      async (payload, context) => {
    ...
    this.eventProcessors.set(
      'supplier_platform.availability.updated',
      async (payload, context) => {
    ...
    this.eventProcessors.set(
      'email_provider.email.delivered',
      async (payload, context) => {
    ...
    this.eventProcessors.set(
      'sms_provider.message.delivered',
      async (payload, context) => {
    ...
    console.info(
      `[WebhookProcessor] Registered ${this.eventProcessors.size} event processors`,
    );
  }
  ...
  private async processPaymentSuccess(
    ...
    try {
      ...
      await this.updateWeddingPaymentStatus(
        weddingId,
        supplierId,
        'completed',
        paymentAmount,
      ...
      if (paymentType === 'final') {
        await this.updateSupplierBookingStatus(
          supplierId,
          weddingId,
          'confirmed',
        );
  ...
  private async processBookingConfirmation(
    ...
    try {
      ...
      await this.sendBookingConfirmationToCouple(
        wedding_id,
        supplier_id,
        booking_date,
      );
      ...
      console.error(
        `[WebhookProcessor] Failed to process booking confirmation:`,
        {
          eventId: context.eventId,
          bookingId: booking_id,
          error: error instanceof Error ? error.message : 'Unknown error',
  ...
  private async processBookingCancellation(
    payload: Record<string, unknown>,
    context: EventProcessorContext,
  ): Promise<void> {
    const { booking_id, wedding_id, supplier_id, cancellation_reason } =
      payload;
  ...
  private async processAvailabilityUpdate(
    payload: Record<string, unknown>,
    context: EventProcessorContext,
  ): Promise<void> {
  ...
  private async processEmailDelivered(
    payload: Record<string, unknown>,
    context: EventProcessorContext,
  ): Promise<void> {
  ...
  private async processEmailBounced(
    payload: Record<string, unknown>,
    context: EventProcessorContext,
  ): Promise<void> {
  ...
  private async processSMSDelivered(
    payload: Record<string, unknown>,
    context: EventProcessorContext,
  ): Promise<void> {
  ...
  async getProcessingStats(): Promise<any> {
    ...
    for (const [providerId, config] of this.webhookConfigs) {
      stats.deadLetterQueues[providerId] = await this.redis.llen(
        config.retryPolicy.deadLetterQueue,
      );
    }
    ...
    stats.processedToday = parseInt(
      (await this.redis.get(processedKey)) || '0',
    );
