...
export class IntegrationGateway {
  ...
  private async initializeServices(): Promise<void> {
    const services: IntegrationConfig[] = [
      ...
      },
      {
        ...
        baseUrl:
          process.env.SUPPLIER_API_URL || 'https://api.wedsync.com/suppliers',
        authType: 'oauth2',
        credentials: {
  ...
  private setupCircuitBreaker(config: IntegrationConfig): void {
    ...
    breaker.on('open', () => {
      console.warn(
        `[IntegrationGateway] Circuit breaker opened for service: ${config.serviceId}`,
      );
    ...
    breaker.on('halfOpen', () => {
      console.info(
        `[IntegrationGateway] Circuit breaker half-open for service: ${config.serviceId}`,
      );
    ...
    breaker.on('close', () => {
      console.info(
        `[IntegrationGateway] Circuit breaker closed for service: ${config.serviceId}`,
      );
    ...
    breaker.on('fallback', () => {
      console.warn(
        `[IntegrationGateway] Fallback triggered for service: ${config.serviceId}`,
      );
    });
  ...
  async routeRequest(
    ...
    try {
      ...
      if (!rateLimitResult.allowed) {
        throw new Error(
          `Rate limit exceeded for service ${serviceId}. Reset at ${rateLimitResult.resetTime}`,
        );
      }
      ...
      const authenticatedOptions = await this.addAuthentication(
        config,
        options,
      );
  ...
  private async makeServiceCall(
    ...
    try {
      ...
      // SENIOR CODE REVIEWER FIX: Use fixed timeout instead of undefined context
      const timeoutId = setTimeout(() => controller.abort(), 30000); // 30 second timeout

      // SENIOR CODE REVIEWER FIX: Build proper fetch options from ServiceRequest
      const url = endpoint;
      ...
      // SENIOR CODE REVIEWER FIX: Use serviceId from request instead of undefined context
      ...
      if (metrics) {
        ...
        metrics.responseTime = Date.now() - Date.now(); // Will be calculated by caller
      }
  ...
  private async getOAuth2Token(config: IntegrationConfig): Promise<string> {
    ...
    try {
      ...
      console.error(
        `[IntegrationGateway] OAuth2 token fetch failed for ${config.serviceId}:`,
        error,
      );
    ...
    try {
      ...
      if (current === 1) {
        await this.redis.expire(
          windowKey,
          Math.ceil(config.rateLimit.window / 1000),
        );
      }
  ...
  private normalizeStripeResponse(
    data: Record<string, unknown>,
  ): Record<string, unknown> {
    return {
      id: data.id,
  ...
  private normalizeEmailResponse(
    data: Record<string, unknown>,
  ): Record<string, unknown> {
    return {
      id: data.id,
  ...
  private normalizeSupplierResponse(
    data: Record<string, unknown>,
  ): Record<string, unknown> {
    return {
  ...
  private async recordMetrics(
    ...
    try {
      ...
      if (totalRequests) {
        metrics.errorRate =
          (parseInt(errorCount) / parseInt(totalRequests)) * 100;
      }
    } catch (error) {
      console.error(
        `[IntegrationGateway] Failed to record metrics for ${serviceId}:`,
        error,
      );
    }
  }
  ...
  async getServiceHealth(
    serviceId: string,
  ): Promise<ServiceHealthMetrics | null> {
  ...
  async getServiceMetrics(
    serviceId: string,
    days: number = 1,
  ): Promise<ServiceResponse<ServiceHealthMetrics[]>> {
    const metrics = [];
    const now = new Date();
