  ...
  return async (request: NextRequest, ...args: T): Promise<NextResponse> => {
    ...
    try {
      ...
      if (auditConfig?.preRequestAudit) {
        auditEvent = await createPreRequestAuditEvent(
          request,
          auditContext,
          auditConfig,
        );
...
async function extractAuditContext(
  request: NextRequest,
  requestId: string,
): Promise<ExtendedAuditContext> {
...
function extractWeddingId(request: NextRequest): string | undefined {
  ...
  const weddingIndex = pathSegments.findIndex(
    (segment) => segment === 'weddings',
  );
...
function determineEventTypeFromRequest(
  request: NextRequest,
...
function determineSeverityFromResponse(
  response: NextResponse,
  config?: ApiAuditConfig,
): AuditSeverity {
  if (config?.severity) {
