...
export class ReviewNodeExecutor extends BaseNodeExecutor {
  ...
  private isWeddingComplete(
    ...
    const daysSinceWedding = Math.floor(
      (today.getTime() - weddingDate.getTime()) / (1000 * 60 * 60 * 24),
    );
  ...
  private async createReviewRequest(
    ...
    const personalizedUrl = await this.createPersonalizedUrl(
      reviewUrl,
      context,
    );
  ...
  private getReviewUrl(
    ...
    switch (config.platform) {
      ...
      default:
        ...
        const baseUrl =
          process.env.NEXT_PUBLIC_APP_URL || 'https://wedsync.com';
        return `${baseUrl}/reviews/submit`;
    }
  }

  private async createPersonalizedUrl(
    baseUrl: string,
    context: NodeExecutorContext,
  ): Promise<string> {
    ...
    const { error } = await this.supabase.from('review_url_tracking').insert({
      token,
  ...
  private async sendReviewRequest(
    ...
    const emailContent = {
      ...
      template_data: {
        ...
        incentive_details: config.incentiveDetails,
        min_rating_message: config.minRating
          ? `We'd love a ${config.minRating}+ star review!`
          : '',
      },
    };
  ...
  private async scheduleFollowUp(
    ...
    const { error } = await this.supabase.from('review_follow_ups').insert({
