...
export function ChannelManager({
    ...
    const initializeChannels = async () => {
      try {
        ...
        if (userType === 'supplier') {
          const supplierChannels = channels
            .filter(
              (ch) =>
          ...
          await Promise.all(
            supplierChannels.map((channel) => subscribeToChannel(channel.name)),
          );
        ...
        if (userType === 'couple') {
          const coupleChannels = channels.filter(
            (ch) =>
          ...
          await Promise.all(
            coupleChannels.map((channel) => subscribeToChannel(channel.name)),
          );
  ...
  const handleChannelSwitch = useCallback(
    async (channelName: string) => {
      ...
      try {
        ...
        if (switchTime > 200) {
          console.warn(
            `Channel switch took ${switchTime.toFixed(2)}ms - exceeds 200ms target`,
          );
        }
      } catch (error) {
  ...
  const handleNewMessage = useCallback(
    (channelName: string, message: ChannelMessage) => {
      if (channelName !== currentChannel) {
        ...
        if (isUrgent || isEmergency) {
          toast({
            title: isEmergency ? '🚨 Emergency Message' : '⚡ Urgent Message',
  ...
  const organizedChannels = useMemo(() => {
    ...
    activeChannels.forEach((channel) => {
      ...
      if (channel.type === 'emergency' || channel.name.includes('emergency')) {
        ...
        channel.scope === 'supplier' &&
        channel.entity === 'dashboard'
      ) {
