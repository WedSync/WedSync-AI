...
const PERMISSIONS = [
  {
    ...
    description: 'Access client information and wedding details',
  },
  {
    id: 'write_galleries',
  ...
  },
  {
    id: 'send_messages',
    name: 'Send Messages',
    description: 'Communicate with clients through the platform',
  },
  {
    id: 'access_timeline',
    name: 'Wedding Timeline',
    description: 'View and update wedding day timeline',
  },
  {
    id: 'manage_bookings',
  ...
  },
  {
    ...
    description: 'View payment and invoice information',
  },
];

export default function VendorAPIAccessManager(): JSX.Element {
  const [vendors, setVendors] = useState<VendorAccess[]>([]);
  const [selectedVendor, setSelectedVendor] = useState<VendorAccess | null>(
    null,
  );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage API access and permissions for wedding vendors
          </p>
        </div>
        <Button>
          ...
          <div className="space-y-2">
            {filteredVendors.map((vendor) => {
              ...
              const quotaPercentage =
                (vendor.usedQuota / vendor.monthlyQuota) * 100;
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-3">
                        <div
                          className={`p-2 rounded-lg ${VENDOR_TYPES[vendor.type].color} text-white`}
                        >
                              ...
                              setVendors((prev) =>
                                prev.map((v) =>
                                  v.id === selectedVendor.id
                                    ? { ...v, weddingDayAccess: checked }
                                    : v,
                                ),
                              );
                              setSelectedVendor({
                                ...selectedVendor,
                                weddingDayAccess: checked,
                              });
                            }}
                          />
                          <span className="text-sm text-muted-foreground">
                            {selectedVendor.weddingDayAccess
                              ? 'Enabled'
                              : 'Disabled'}
                          </span>
                        </div>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <Alert>
                        ...
                        <AlertDescription>
                          API keys provide full access to vendor permissions.
                          Keep them secure and regenerate if compromised.
                        </AlertDescription>
                      </Alert>
                    </div>
                  </CardContent>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {PERMISSIONS.map((permission) => {
                        const hasPermission =
                          selectedVendor.permissions.includes(permission.id);
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>
                            Used: {selectedVendor.usedQuota.toLocaleString()}
                          </span>
                          <span>
                            Limit:{' '}
                            {selectedVendor.monthlyQuota.toLocaleString()}
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full transition-all ${
                              selectedVendor.usedQuota >
                              selectedVendor.monthlyQuota
                                ? 'bg-red-500'
                                : selectedVendor.usedQuota /
                                      selectedVendor.monthlyQuota >
                                    0.8
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          ...
                          <span className="text-sm font-medium">
                            {selectedVendor.rateLimit}/hour
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm">
                            Wedding Day Multiplier
                          </span>
            ...
            <Card className="h-96 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-500">
                  Select a vendor from the list to view and manage their API
                  access.
                </p>
              </div>
            </Card>
