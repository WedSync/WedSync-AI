...
test.describe('Custom Domains System - WS-222', () => {
  ...
  test.describe('Domain Manager UI', () => {
    test('renders domain management interface correctly', async ({ page }) => {
      ...
      await expect(
        page.getByText('Brand your client portal with your own domain'),
      ).toBeVisible();
    ...
    test('displays default domain when no custom domain configured', async ({
      page,
    }) => {
      // Should show default WedSync domain
      await expect(
        page.locator('[data-testid="current-domain"]'),
      ).toContainText('.wedsync.app');
  ...
  test.describe('Domain Configuration Workflow', () => {
    test('complete domain setup workflow', async ({ page }) => {
      ...
      await expect(
        page.getByText('Please enter a valid domain format'),
      ).toBeVisible();
      ...
      await expect(
        page.getByText('Domain is valid and available!'),
      ).toBeVisible();
    ...
    test('domain validation with various inputs', async ({ page }) => {
      ...
      for (const domain of invalidDomains) {
        ...
        await expect(
          page.getByText(/Invalid|Please enter a valid/),
        ).toBeVisible();
      }
    });
  ...
  test.describe('DNS Instructions', () => {
    test('displays DNS configuration instructions', async ({ page }) => {
      ...
      await expect(
        page.getByRole('tab', { name: 'All Records' }),
      ).toBeVisible();
      await expect(
        page.getByRole('tab', { name: 'TXT Records' }),
      ).toBeVisible();
      await expect(
        page.getByRole('tab', { name: 'CNAME Records' }),
      ).toBeVisible();
    ...
    test('DNS help section provides guidance', async ({ page }) => {
      ...
      await expect(
        page.getByRole('link', { name: /GoDaddy Guide/i }),
      ).toBeVisible();
      await expect(
        page.getByRole('link', { name: /Cloudflare Guide/i }),
      ).toBeVisible();
    });
  });
  ...
  test.describe('SSL Status Monitoring', () => {
    ...
    test('displays SSL expiry warnings', async ({ page }) => {
      ...
      await expect(
        page.getByText(/Certificate Renewal Required/),
      ).toBeVisible();
      await expect(
        page.getByRole('button', { name: 'Force Renewal Now' }),
      ).toBeVisible();
    });
  });
  ...
  test.describe('Domain Management Actions', () => {
    ...
    test('remove domain with confirmation', async ({ page }) => {
      ...
      await expect(page.getByText(/Domain removed successfully/)).toBeVisible({
        timeout: 5000,
      });
    });
      ...
      await page.route('/api/supplier/domain**', (route) => {
        ...
        if (callCount === 1) {
          ...
          route.fulfill({
            ...
            body: JSON.stringify({
              success: true,
              data: { default_domain: 'test.wedsync.app' },
            }),
          });
        }
      });
  ...
  test.describe('Accessibility', () => {
    ...
    test('supports keyboard navigation', async ({ page }) => {
      ...
      await expect(
        page.getByRole('tab', { name: 'Configure' }),
    ...
    test('provides appropriate focus management', async ({ page }) => {
      ...
      await page.keyboard.press('Tab');
      await expect(
        page.getByRole('button', { name: 'Validate Domain' }),
      ).toBeFocused();
    });
  });
...
test.describe('API Integration Tests', () => {
  ...
  test('domain configuration endpoint requires authentication', async ({
    request,
  }) => {
