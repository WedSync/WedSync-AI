          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant="outline"
                  className={getServiceLevelColor(supplier.serviceLevel)}
                >
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <span className="text-sm text-gray-600">
                ({supplier.totalReviews} review
                {supplier.totalReviews !== 1 ? 's' : ''})
              </span>
              {supplier.priceRange && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="space-y-2">
            ...
            {supplier.additionalTravelCost &&
              supplier.additionalTravelCost > 0 && (
                <div className="flex items-center gap-2 text-sm">
                  <DollarSign className="h-4 w-4 text-gray-400" />
                  <span>+£{supplier.additionalTravelCost} travel cost</span>
                </div>
              )}
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 gap-2 text-sm text-gray-600">
                  <div className="flex justify-between">
                    ...
                    <span className="capitalize">
                      {supplier.serviceLevel.replace('_', ' ')}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Coverage Radius:</span>
                    <span>
                      {supplier.serviceRadius} miles (
                      {(supplier.serviceRadius * 1.60934).toFixed(1)} km)
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {supplier.additionalTravelCost && (
                    <div className="flex justify-between">
                      ...
                      <span>£{supplier.additionalTravelCost.toFixed(2)}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      <Card className={`w-full ${className}`}>
        <CardContent className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            We couldn't find any suppliers in your area. Try expanding your
            search radius or changing your filters.
          </p>
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {!hasMore && results.length > 0 && (
        <div className="text-center pt-4">
          <p className="text-sm text-gray-600">
            You've seen all {totalCount} supplier{totalCount !== 1 ? 's' : ''}{' '}
            in this area
          </p>
        </div>
      )}
