...
import {
  ...
  LocationSuggestion,
} from '@/lib/services/geographic-search-service';
...
export default function LocationSearchFilter({
  ...
  const [filters, setFilters] = useState({
    ...
    sortBy: (initialParams.sortBy || 'distance') as
      | 'distance'
      | 'rating'
      | 'price'
      | 'reviews',
  ...
  useEffect(() => {
    ...
    if (searchAddress.length >= 2) {
      searchTimeoutRef.current = setTimeout(async () => {
        try {
          const suggestions =
            await geographicSearchService.getLocationSuggestions(
              searchAddress,
              8,
            );
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <div className="flex gap-2">
              ...
              >
                <Navigation
                  className={`h-4 w-4 ${geoLocationLoading ? 'animate-spin' : ''}`}
                />
              </Button>
            </div>
