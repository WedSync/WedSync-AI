        ...
        <div className="space-y-4">
          <div className="flex flex-wrap gap-2">
            {availableStyles.map((style) => (
              <Button
                key={style.id}
                variant={
                  criteria.styles.includes(style.name) ? 'primary' : 'outline'
                }
                size="sm"
              ...
              >
                {style.name}
                {criteria.styles.includes(style.name) && (
          ...
          <div>
            <Label className="text-sm">
              Minimum Style Confidence:{' '}
              {Math.round(criteria.styleConfidence * 100)}%
            </Label>
            <Slider
              value={[criteria.styleConfidence]}
              onValueChange={(value) =>
                updateCriteria({ styleConfidence: value[0] })
              }
              min={0}
              max={1}
              step={0.1}
            ...
            <div className="flex flex-wrap gap-2">
              {colorPaletteOptions.map((palette) => (
                <Button
                  key={palette.name}
                  variant={
                    criteria.colorPalette.includes(palette.name)
                      ? 'primary'
                      : 'outline'
                  }
                  size="sm"
            ...
            <div className="flex flex-wrap gap-2">
              {moodOptions.map((mood) => (
                <Button
                  key={mood}
                  variant={
                    criteria.moodTags.includes(mood) ? 'primary' : 'outline'
                  }
                  size="sm"
                  onClick={() =>
                    handleMultiSelectToggle(mood, criteria.moodTags, 'moodTags')
                  }
                >
                  {mood}
                </Button>
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label className="text-sm">
                Search Radius: {criteria.radius} km
              </Label>
              <Slider
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label className="text-sm">
                Price Range: ${criteria.priceRange[0]} - $
                {criteria.priceRange[1]}
              </Label>
              <Slider
                value={criteria.priceRange}
                onValueChange={(value) =>
                  updateCriteria({ priceRange: [value[0], value[1]] })
                }
                min={0}
                max={50000}
                step={500}
            ...
            <div className="flex flex-wrap gap-2">
              {availabilityOptions.map((status) => (
                <Button
                  key={status}
                  variant={
                    criteria.availability.includes(status)
                      ? 'primary'
                      : 'outline'
                  }
                  size="sm"
        ...
        <div className="flex flex-wrap gap-2">
          {eventTypeOptions.map((eventType) => (
            <Button
              key={eventType}
              variant={
                criteria.eventTypes.includes(eventType) ? 'primary' : 'outline'
              }
              size="sm"
              onClick={() =>
                handleMultiSelectToggle(
      ...
      {isAdvancedMode && (
        <FilterSection title="Technical Quality" icon={Settings} id="technical">
          <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {lightingOptions.map((quality) => (
                  <Button
                    key={quality}
                    variant={
                      criteria.technical.lightingQuality.includes(quality)
                        ? 'primary'
                        : 'outline'
                    }
                    size="sm"
                    onClick={() => {
                      const newLighting =
                        criteria.technical.lightingQuality.includes(quality)
                          ? criteria.technical.lightingQuality.filter(
                              (q) => q !== quality,
                            )
              ...
              <div className="flex flex-wrap gap-2">
                {focusOptions.map((focus) => (
                  <Button
                    key={focus}
                    variant={
                      criteria.technical.focusQuality.includes(focus)
                        ? 'primary'
                        : 'outline'
                    }
                    size="sm"
                    onClick={() => {
                      const newFocus = criteria.technical.focusQuality.includes(
                        focus,
                      )
                        ? criteria.technical.focusQuality.filter(
                            (f) => f !== focus,
                          )
            ...
            <div>
              <Label className="text-sm">
                Minimum Composition Score: {criteria.technical.compositionScore}
                /10
              </Label>
              <Slider
      ...
      <div className="flex items-center justify-between pt-4">
        <div className="text-sm text-gray-600">
          {criteria.styles.length > 0 && (
            <span>
              {criteria.styles.length} style
              {criteria.styles.length !== 1 ? 's' : ''} selected
            </span>
          )}
        </div>
