...
export default function FAQReviewQueue({
  ...
  const filteredAndSortedFAQs = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        default:
          return (
            new Date(b.extractionDate).getTime() -
            new Date(a.extractionDate).getTime()
          );
      }
    });
  ...
  const stats = useMemo(() => {
    ...
    const approved = extractedFAQs.filter(
      (f) => f.status === 'approved',
    ).length;
    const rejected = extractedFAQs.filter(
      (f) => f.status === 'rejected',
    ).length;
    const highConfidence = extractedFAQs.filter(
      (f) => f.confidence >= 80,
    ).length;
  ...
  const handleBulkDelete = useCallback(() => {
    if (
      window.confirm(
        `Delete ${selectedFAQs.size} selected FAQs? This action cannot be undone.`,
      )
    ) {
      onBulkAction({
        type: 'bulk_delete',
  ...
  const handleEditSave = useCallback(
    (faqId: string, updates: Partial<ExtractedFAQ>) => {
      onEdit(faqId, {
        ...updates,
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Review and approve extracted FAQ content before adding to your
              library
            </p>
          </div>
          <button
            ...
            >
              <FAQReviewCard
                ...
                onCategoryChange={(categoryId) =>
                  onCategoryChange(faq.id, categoryId)
                }
                onTogglePreview={() =>
                  setPreviewId(previewId === faq.id ? null : faq.id)
                }
              />
            </motion.div>
          ))
...
function FAQReviewCard({
  ...
  return (
    <div
      className={`bg-white border rounded-lg shadow-sm transition-all ${isSelected ? 'border-primary-500 ring-2 ring-primary-200' : 'border-gray-200'}`}
    >
      <div className="p-6">
        {/* Header */}
        ...
        {isEditing ? (
          ...
          <div className="space-y-4">
            <div>
              ...
              <div
                className={`text-gray-600 ${isPreview ? '' : 'line-clamp-3'}`}
              >
                {faq.answer}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-200">
              <div className="flex items-center space-x-4 text-xs text-gray-500">
                <span>
                  Extracted: {new Date(faq.extractionDate).toLocaleDateString()}
                </span>
                {faq.lastModified && (
                  <span>
                    Modified: {new Date(faq.lastModified).toLocaleDateString()}
                  </span>
                )}
                <a
                  href={faq.sourceUrl}
