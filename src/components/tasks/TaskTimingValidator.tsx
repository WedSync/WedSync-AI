...
export const TaskTimingValidator = React.memo<TaskTimingValidatorProps>(
  ({
    ...
    const timelineVisualization = useMemo((): TimelineVisualization => {
      ...
      const proposedEnd = new Date(
        proposedStart.getTime() +
          proposedTask.estimated_duration * 60 * 60 * 1000,
      );
      ...
      const sortedTasks = [...timelineTasks].sort(
      ...
      sortedTasks.forEach((task) => {
        const taskStart = new Date(
          Math.max(task.startTime.getTime(), dayStart.getTime()),
        );
        const taskEnd = new Date(
          Math.min(
            task.endTime.getTime() + task.bufferTime * 60 * 60 * 1000,
            dayEnd.getTime(),
          ),
        );
        ...
        if (currentTime < taskStart) {
          const slotDuration =
            (taskStart.getTime() - currentTime.getTime()) / (1000 * 60 * 60);
        ...
        currentTime = new Date(
          Math.max(currentTime.getTime(), taskEnd.getTime()),
        );
      });
      ...
      const scheduledHours = timelineTasks.reduce(
        (total, task) => total + task.duration + task.bufferTime,
        0,
      );
      const utilizationPercentage = Math.round(
        (scheduledHours / totalHours) * 100,
      );
    ...
    const calculateRecommendationScore = useCallback(
      (start: Date, end: Date, task: any): number => {
    ...
    const detectConflicts = useCallback(async (): Promise<TimingConflict[]> => {
      ...
      existingTasks.forEach((existingTask) => {
        if (!existingTask.start_date || existingTask.status === 'cancelled')
          return;

        const existingStart = existingTask.start_date;
        const existingEnd = new Date(
          existingStart.getTime() +
            existingTask.estimated_duration * 60 * 60 * 1000,
        );
        const existingBufferEnd = new Date(
          existingEnd.getTime() + existingTask.buffer_time * 60 * 60 * 1000,
        );
        ...
        // Buffer time violation (within buffer but not overlapping)
          ...
          proposedStart < existingBufferEnd &&
        ...
        if (
          ...
          conflicts.push({
            ...
            conflictingTasks: [existingTask.id],
            message:
              'This task conflicts with critical path items that cannot be moved',
            suggestions: [],
          });
        }
      });
    ...
    const generateSuggestions = useCallback(
      (conflicts: TimingConflict[]): ConflictResolution[] => {
        ...
        if (proposedTask.estimated_duration > 4) {
          suggestions.push({
            ...
            description:
              'Consider reducing task duration or splitting into smaller tasks',
        ...
        if (proposedTask.estimated_duration > 6) {
          suggestions.push({
            ...
            newTasks: Array.from(
              { length: Math.ceil(proposedTask.estimated_duration / 3) },
              (_, i) => ({
                title: `${proposedTask.title} - Part ${i + 1}`,
                estimated_duration: Math.ceil(
                  proposedTask.estimated_duration! /
                    Math.ceil(proposedTask.estimated_duration! / 3),
                ),
              }),
            ),
        ...
        // Buffer adjustment
        const hasBufferConflicts = conflicts.some(
          (c) => c.type === 'buffer_violation',
        );
        if (hasBufferConflicts) {
          suggestions.push({
            ...
            newBufferTime: Math.max(
              0.25,
              (proposedTask.buffer_time || 0) + 0.25,
            ),
            effort_required: 'low',
            impact: 'minimal',
          });
        ...
        return suggestions
          .sort((a, b) => {
            const scoreA =
              a.effort_required === 'low'
                ? 3
                : a.effort_required === 'medium'
                  ? 2
                  : 1;
            const scoreB =
              b.effort_required === 'low'
                ? 3
                : b.effort_required === 'medium'
                  ? 2
                  : 1;
    ...
    const handleTimeSlotDrop = useCallback(
      (e: React.DragEvent, timeSlot: TimeSlot) => {
        e.preventDefault();
    ...
    const hourMarkers = useMemo(() => {
      const markers = [];
      for (
        let hour = timelineHours.start;
        hour <= timelineHours.end;
        hour += 2
      ) {
        const date = new Date();
        ...
        markers.push({
          ...
          position:
            ((hour - timelineHours.start) /
              (timelineHours.end - timelineHours.start)) *
            100,
        });
      }
      return markers;
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              ...
              <div
                ...
                data-testid={
                  conflict.type === 'overlap'
                    ? 'conflict-warning'
                    : conflict.type === 'buffer_violation'
                      ? 'buffer-warning'
                      : conflict.type === 'critical_path_conflict'
                        ? 'critical-path-conflict'
              ...
              >
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    ...
                    {conflict.type === 'buffer_violation' &&
                      conflict.requiredBuffer &&
                      conflict.actualBuffer && (
                        <div className="text-xs bg-white/50 rounded px-2 py-1">
                          Required: {conflict.requiredBuffer}h | Available:{' '}
                          {conflict.actualBuffer.toFixed(1)}h
                        </div>
                      )}
                  </div>
                </div>
        ...
        {timelineVisualization.tasks.length > 0 && (
          <div className="mb-6">
            ...
            >
              ...
              <div className="space-y-2">
                {timelineVisualization.tasks.map((task) => {
                  const startPercent =
                    ((task.startTime.getHours() - timelineHours.start) /
                      (timelineHours.end - timelineHours.start)) *
                    100;
            ...
            <div className="mt-4 flex items-center justify-between text-sm text-gray-600">
              ...
              <span>
                {timelineVisualization.utilizationPercentage}% timeline
                utilization
              </span>
            </div>
          </div>
              ...
              {suggestions.slice(0, 3).map((suggestion, index) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      {suggestion.type === 'time_adjustment' && (
                        <Move3D className="w-4 h-4 text-primary-600 mr-2" />
                      )}
                      {suggestion.type === 'duration_reduction' && (
                        <Timer className="w-4 h-4 text-primary-600 mr-2" />
                      )}
                      {suggestion.type === 'split_task' && (
                        <Split className="w-4 h-4 text-primary-600 mr-2" />
                      )}
                      {suggestion.type === 'buffer_adjustment' && (
        ...
        {conflicts.length > 0 && (
          ...
          >
            ...
            {suggestions.length > 0 &&
              ` ${suggestions.length} alternative solutions available.`}
          </div>
        )}
