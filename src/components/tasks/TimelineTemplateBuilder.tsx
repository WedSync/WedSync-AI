...
export default function TimelineTemplateBuilder({
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      const templatesWithParsedMilestones =
        data?.map((template) => ({
          ...template,
  ...
  const handleCreateTemplate = async () => {
    ...
    setLoading(true);
    try {
      const initialMilestones = generateInitialMilestones(
        newTemplate.months_before_wedding,
      );
  ...
  const handleAddTask = (milestoneId: string) => {
    setEditingMilestones((prev) =>
      prev.map((m) =>
        m.id === milestoneId ? { ...m, tasks: [...m.tasks, 'New task'] } : m,
      ),
    );
  };
  ...
  const handleApplyToWedding = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('workflow_tasks').insert(tasks);
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span>
                        {template.months_before_wedding} months timeline
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setNewTemplate({
                    ...newTemplate,
                    months_before_wedding: parseInt(value),
                  })
                }
              >
                <SelectTrigger>
