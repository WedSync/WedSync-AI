...
export function AppropriatenessChecker({
  ...
  const analyzeTrack = useCallback(async () => {
    ...
    try {
      ...
      const appropriatenessScore: AppropriatenessScore = {
        score:
          analysis.score || calculateHeuristicScore(track, weddingCategory),
        category: weddingCategory,
        reasoning:
          analysis.reasoning ||
          generateHeuristicReasoning(track, weddingCategory),
        flags: {
  ...
  const applyOverride = useCallback(() => {
    ...
    const overriddenScore: AppropriatenessScore = {
      ...score!,
      score: overrideScore,
      reasoning: [...score!.reasoning, `DJ Override: ${overrideReason.trim()}`],
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900">
                    Wedding Appropriateness Analysis
                  </h2>
            ...
            <Card className="mb-6">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  <Badge variant="outline">{track.primary_genre}</Badge>
            ...
            <Card className="mb-6">
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {CATEGORY_DESCRIPTIONS[weddingCategory].requirements.map(
                      (req, index) => (
                        <Badge
                          key={index}
                          variant="secondary"
              ...
              <Card>
                <CardContent className="py-12">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600">
                      Using AI to assess wedding appropriateness
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="space-y-6">
                ...
                <Card>
                  <CardContent className="py-6">
                    <div className="text-center">
                      <div
                        className={`inline-flex items-center gap-3 px-6 py-4 rounded-2xl ${scoreDisplay.bg} mb-4`}
                      >
                        <scoreDisplay.icon
                          className={`w-8 h-8 ${scoreDisplay.color}`}
                        />
                        <div>
                          <div
                            className={`text-3xl font-bold ${scoreDisplay.color}`}
                          >
                            {score.score}/10
                          </div>
                          <div
                            className={`text-sm font-medium ${scoreDisplay.color}`}
                          >
                            {scoreDisplay.status}
                          </div>
                        </div>
                  ...
                  {showOverride && (
                    <CardContent className="space-y-4">
                      ...
                      <Button
                        ...
                        disabled={
                          overrideScore === null || !overrideReason.trim()
                        }
                        className="w-full"
                      >
                        Apply Override
...
function generateHeuristicReasoning(
  ...
  if (track.explicit_content) {
    reasons.push(
      'Track contains explicit content which may not be suitable for all wedding guests',
    );
  }

  if (track.energy_level > threshold.maxEnergy) {
    reasons.push(
      `Energy level (${track.energy_level}/10) is higher than typical for ${category} events`,
    );
  }
  ...
  if (track.wedding_usage_count > 100) {
    reasons.push(
      `Popular wedding choice (used in ${track.wedding_usage_count} weddings)`,
    );
  }
