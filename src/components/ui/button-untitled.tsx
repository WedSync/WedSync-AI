  ...
  'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98]',
  {
    variants: {
      variant: {
        ...
        romantic:
          'bg-gradient-to-r from-pink-400 to-rose-400 text-white hover:from-pink-500 hover:to-rose-500 shadow-md hover:shadow-lg',
        luxury:
          'bg-gradient-to-r from-amber-400 to-yellow-400 text-gray-900 hover:from-amber-500 hover:to-yellow-500 shadow-lg hover:shadow-xl border border-amber-300',
        ...
        shine:
          'bg-gray-900 text-white hover:bg-gray-800 relative overflow-hidden before:absolute before:inset-0 before:bg-gradient-to-r before:from-transparent before:via-white/20 before:to-transparent before:translate-x-[-100%] hover:before:translate-x-[100%] before:transition-transform before:duration-700',
        glass:
          'bg-white/10 backdrop-blur-sm border border-white/20 text-gray-900 hover:bg-white/20',

        // Mobile optimized (preserved)
        touch:
          'bg-blue-600 text-white hover:bg-blue-700 min-h-[44px] shadow-sm',

        // Link styles (preserved)
        linkDanger:
          'text-red-600 hover:text-red-800 hover:underline p-0 h-auto',
      },
      size: {
        xs: 'h-8 px-3 text-xs',
...
export const WeddingButton = React.forwardRef<
  HTMLButtonElement,
...
export const VendorButton = React.forwardRef<
  HTMLButtonElement,
  ButtonProps & { vendorType: 'photographer' | 'venue' | 'florist' | 'caterer' }
>(({ vendorType, ...props }, ref) => (
  <Button {...props} variant={vendorType} ref={ref} />
));
