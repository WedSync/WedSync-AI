...
export function PopulationStatusBanner({
  ...
  const stats = useMemo(() => {
    ...
    const highConfidence = dataArray.filter(
      (d) => d.confidenceScore >= 0.9,
    ).length;
    const mediumConfidence = dataArray.filter(
      (d) => d.confidenceScore >= 0.7 && d.confidenceScore < 0.9,
    ).length;
    const lowConfidence = dataArray.filter(
      (d) => d.confidenceScore < 0.7,
    ).length;

    const sensitive = dataArray.filter((d) => d.isSensitive).length;
    const averageConfidence =
      dataArray.reduce((sum, d) => sum + d.confidenceScore, 0) /
      dataArray.length;
  ...
  const handleAcceptAllHighConfidence = useCallback(() => {
    const highConfidenceFields = Array.from(populationData.entries()).filter(
      ([_, data]) => data.confidenceScore >= 0.9 && data.status === 'pending',
    );
          ...
          {!compactMode && (
            <div className="space-y-2">
              <p className="text-sm text-blue-700">
                Found <strong>{stats.populated}</strong> auto-fill suggestions
                for your wedding form
              ...
              <CardContent className="pt-0 space-y-4">
                        ...
                        <div key={level} className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between items-center mb-1">
                              <div
                                className={cn(
                                  'h-2 bg-gray-200 rounded-full flex-1 mr-2',
                                )}
                              >
                                <div
                                  className={cn(
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-xs">
                    ...
                    <div>
                      ...
                      <div
                        className={cn(
                          'font-medium',
                          sessionSummary.securityScore >= 80
                            ? 'text-green-600'
                            : sessionSummary.securityScore >= 60
                              ? 'text-yellow-600'
                              : 'text-red-600',
                        )}
                      >
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-blue-100">
                    ...
                    {stats.sensitive > 0 && (
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>
                            These fields contain sensitive information and are
                            encrypted
                          </p>
                        </TooltipContent>
                      </Tooltip>
                ...
                <div className="bg-purple-50 rounded-lg p-3">
                  ...
                  <p className="text-sm text-purple-700">
                    ...
                    <strong>
                      {Math.round((stats.populated * 2) / 60)} minutes
                    </strong>{' '}
                    of form filling time, helping you focus on planning your
                    perfect wedding day.
                  </p>
                </div>
              </CardContent>
