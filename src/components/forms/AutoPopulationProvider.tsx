...
export function useAutoPopulationContext() {
  ...
  if (!context) {
    throw new Error(
      'useAutoPopulationContext must be used within an AutoPopulationProvider',
    );
  }
  return context;
}
...
export function AutoPopulationProvider({
  ...
  const [populationData, setPopulationData] = useState<
    Map<string, FieldPopulationData>
  >(new Map());
  ...
  const decryptSessionData = useCallback(
    (encryptedData: string): PopulationSession | null => {
      try {
  ...
  const addPopulationEvent = useCallback(
    (event: Omit<PopulationEvent, 'id' | 'timestamp'>) => {
      if (!session) return;
  ...
  const requestPopulation = useCallback(
    async (request: PopulationRequest): Promise<FieldPopulationData | null> => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          setPopulationData(
            (prev) => new Map(prev.set(request.fieldId, populationData)),
          );
  ...
  const acceptPopulation = useCallback(
    (fieldId: string) => {
  ...
  const rejectPopulation = useCallback(
  ...
  const getSessionSummary = useCallback((): SessionSummary => {
    ...
    const sensitiveCount = Array.from(populationData.values()).filter(
      (data) => data.isSensitive,
    ).length;
