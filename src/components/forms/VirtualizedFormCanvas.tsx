...
const VirtualizedFieldItem = memo<VirtualizedFieldItemProps>(
  function VirtualizedFieldItem({
    field,
    sectionId,
    sectionTitle,
    isSelected,
    ...
    return (
      ...
      >
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500 truncate">
                {getFieldTypeLabel()} • {sectionTitle}
              </p>
            </div>
          </div>
        ...
        <div className="text-sm text-gray-600 mb-3">
          {field.placeholder && <p className="italic">"{field.placeholder}"</p>}
          {field.helperText && (
...
export const VirtualizedFormCanvas: React.FC<VirtualizedFormCanvasProps> = ({
  ...
  const handleFieldPreview = useCallback(
    (field: FormField) => {
  ...
  const renderVirtualizedItem = useCallback(
    (item: (typeof virtualizedItems)[0], index: number) => {
    ...
    <div className={`flex flex-col h-full ${className}`} data-testid={testId}>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="p-2 text-xs text-gray-500 bg-gray-50 border-t">
          Rendering {virtualizedItems.length} items • Sections:{' '}
          {sections.length} • Expanded: {expandedSections.size}
        </div>
      )}
    </div>
  );
