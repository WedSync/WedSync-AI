...
const ResponseNotifications: React.FC<ResponseNotificationsProps> = ({
  formId,
}) => {
...
View Response: {{response_link}}`,
  ...
  const defaultChannels: NotificationChannel[] = [
    ...
    },
    {
      ...
      config: {
        slack_channel: '#wedding-inquiries',
  ...
  const defaultRules: NotificationRule[] = [
    {
      ...
      template: {
        subject: '🌟 VIP Wedding Inquiry - Immediate Attention Required',
        message:
          'A high-value wedding inquiry has been received and requires immediate attention.',
    ...
    },
    {
      ...
      recipients: [{ type: 'role', value: 'coordinator' }],
      timing: {
        immediate: false,
      ...
      template: {
        ...
        message:
          'A new wedding inquiry has been submitted and is awaiting review.',
    ...
    },
    {
      ...
      channels: ['email-primary'],
      recipients: [{ type: 'role', value: 'account_manager' }],
      timing: {
        immediate: false,
        schedule: {
      ...
      {activeTab === 'rules' && (
        <div className="space-y-4">
          {rules.map((rule) => (
            <Card
              key={rule.id}
              className={`border-l-4 ${rule.isActive ? 'border-l-blue-500' : 'border-l-gray-300'}`}
            >
              <CardHeader>
                <div className="flex items-center justify-between">
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      {rule.metrics.lastSent && (
                        <div className="text-xs text-muted-foreground">
                          Last:{' '}
                          {new Date(rule.metrics.lastSent).toLocaleDateString()}
                        </div>
                      )}
                    </div>
                  </div>
      ...
      {activeTab === 'channels' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {channels.map((channel) => (
            <Card
              key={channel.id}
              className={`border-l-4 ${channel.isActive ? 'border-l-green-500' : 'border-l-gray-300'}`}
            >
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-muted-foreground">
                    {channel.config.email && (
                      <div>Email: {channel.config.email}</div>
                    )}
                    {channel.config.phone && (
                      <div>Phone: {channel.config.phone}</div>
                    )}
                    {channel.config.slack_channel && (
                      <div>Slack: {channel.config.slack_channel}</div>
                    )}
                    {channel.config.webhook_url && (
                      <div>Webhook: {channel.config.webhook_url}</div>
                    )}
                  </div>
                </div>
      ...
      {activeTab === 'templates' && (
        <div className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              These templates are optimized for wedding businesses and include
              wedding-specific variables and language.
            </AlertDescription>
          </Alert>
