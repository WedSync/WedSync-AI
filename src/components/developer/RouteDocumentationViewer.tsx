 ...
 */
export default function RouteDocumentationViewer({
  route,
}: {
  route: APIRoute;
}) {
  const [selectedTab, setSelectedTab] = useState<
    'overview' | 'parameters' | 'responses' | 'examples' | 'security'
  >('overview');
...
async function ${route.name.toLowerCase().replace(/\s+/g, '')}() {
  try {
    const response = await client.${route.method.toLowerCase()}('${endpoint}', {
      ${route.parameters
        .filter((p) => p.required)
        .map((p) => `${p.name}: '${p.weddingExample}' // ${p.description}`)
        .join(',\n      ')}
    });
...
curl -X ${route.method} "${baseUrl}${endpoint}" \\
  ...
  -H "User-Agent: WedSync-Wedding-App/1.0" \\${
    route.method !== 'GET'
      ? `
  -d '{
    ${route.parameters
      .filter((p) => p.required)
      .map((p) => `"${p.name}": "${p.weddingExample}"`)
      .join(',\n    ')}
  }'`
      : ''
  }`,
    ...
    ${
      route.method !== 'GET'
        ...
        ${route.parameters
          .filter((p) => p.required)
          .map(
            (p) => `"${p.name}" => "${p.weddingExample}", // ${p.description}`,
          )
          .join('\n        ')}
    ];
      ...
      <div className="border-b border-gray-200 pb-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <span
              className={`px-3 py-1 text-sm font-medium rounded-full ${getMethodColor(route.method)}`}
            >
              {route.method}
            </span>
                ...
                {route.parameters.map((param, index) => (
                  ...
                  >
                    ...
                    <div className="bg-rose-50 border border-rose-200 rounded p-3">
                      ...
                      <code className="text-rose-800 text-sm">
                        {param.weddingExample}
                      </code>
                    </div>
                        ...
                        {Object.entries(response.headers).map(
                          ([key, value]) => (
                            <div key={key} className="flex">
                              <span className="font-mono text-gray-600 w-48">
                                {key}:
                              </span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              ...
              <p className="text-yellow-800 text-sm">
                All wedding data is encrypted at rest and in transit. Personal
                information like guest lists, vendor contacts, and wedding
                details are protected under GDPR and wedding industry privacy
                standards. Never log or store sensitive wedding information in
                client-side code.
              </p>
            </div>
          </div>
