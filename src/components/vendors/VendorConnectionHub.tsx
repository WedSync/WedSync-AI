...
export function VendorConnectionHub({
  currentVendorId,
  organizationId,
}: Props) {
    ...
    try {
      ...
      if (apiResponse.ok) {
        setConnectionRequests(
          connectionRequests.filter((req) => req.id !== requestId),
        );
  ...
  const filteredVendors = vendors.filter((vendor) => {
    ...
    const matchesLocation =
      locationFilter === 'all' || vendor.county === locationFilter;

    return matchesSearch && matchesCategory && matchesLocation;
  });

  const connectedVendors = vendors.filter(
    (v) => v.connection_status === 'connected',
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Discover and connect with other wedding professionals
          </p>
        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {(vendor.mutual_connections_count > 0 ||
                        ...
                        <div className="flex gap-4 text-xs text-gray-500 mb-4">
                          {vendor.mutual_connections_count > 0 && (
                            <span>
                              {vendor.mutual_connections_count} mutual
                              connections
                            </span>
                          )}
                          {vendor.shared_weddings_count > 0 && (
                            <span>
                              {vendor.shared_weddings_count} shared weddings
                            </span>
                          )}
                        </div>
                      )}
                      ...
                      <Button
                        ...
                        className="w-full"
                      >
                        {vendor.connection_status === 'connected' && (
                          <CheckIcon className="size-4 mr-1" />
                        )}
                        {vendor.connection_status === 'pending_sent' && (
                          <ClockIcon className="size-4 mr-1" />
                        )}
                        {vendor.connection_status === 'none' && (
        ...
        <div className="space-y-4">
          <Card className="p-4">
            <h3 className="font-semibold mb-4">
              Your Network ({connectedVendors.length} vendors)
            </h3>
            {connectedVendors.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm mt-1">
                  Start networking to grow your professional circle
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mt-2">
                          ...
                          {request.mutual_connections > 0 && (
                            <span className="ml-2">
                              • {request.mutual_connections} mutual connections
                            </span>
                          )}
                        </p>
                      </div>
