  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (
        ...
        (currentPage === pages.length - 1 && deltaX < 0)
      ) {
      ...
      const clampedTranslate = Math.max(
        -maxTranslate,
        Math.min(maxTranslate, adjustedDeltaX),
      );
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      ...
      const shouldSwipe =
        ...
        (velocity > 0.5 && Math.abs(deltaX) > 20);
...
export const WeddingSwipeNavigation: React.FC<
  Omit<SwipeNavigationProps, 'pages'> & {
    photosContent?: React.ReactNode;
    scheduleContent?: React.ReactNode;
    clientsContent?: React.ReactNode;
  }
> = ({ photosContent, scheduleContent, clientsContent, ...props }) => {
  const weddingPages: NavigationPage[] = [
    {
      ...
      content: photosContent || (
        <div className="text-center py-8">Photo management interface</div>
      ),
    },
    {
      id: 'schedule',
      ...
      content: scheduleContent || (
        <div className="text-center py-8">Event schedule interface</div>
      ),
    },
    {
      id: 'clients',
      ...
      content: clientsContent || (
        <div className="text-center py-8">Client communication interface</div>
      ),
    },
  ];
