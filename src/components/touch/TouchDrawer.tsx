...
export const TouchDrawer: React.FC<TouchDrawerProps> = ({
  ...
  const animateToPosition = useCallback(
    (targetPosition: number, initialVelocity = 0) => {
      ...
      const animate = () => {
        ...
        if (
          Math.abs(targetPosition - currentPos) < 0.5 &&
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      switch (direction) {
        case 'bottom':
          delta =
            ((touchStartRef.current.y - currentTouch.y) / containerSize) * 100;
          break;
        case 'top':
          delta =
            ((currentTouch.y - touchStartRef.current.y) / containerSize) * 100;
          break;
        case 'right':
          delta =
            ((touchStartRef.current.x - currentTouch.x) / containerSize) * 100;
          break;
        case 'left':
          delta =
            ((currentTouch.x - touchStartRef.current.x) / containerSize) * 100;
          break;
      }
...
export const PhotoDrawer: React.FC<
  Omit<TouchDrawerProps, 'direction' | 'size'>
> = (props) => (
  <TouchDrawer
    direction="bottom"
    size="lg"
...
export const ClientContactDrawer: React.FC<
  Omit<TouchDrawerProps, 'direction' | 'size'>
> = (props) => (
  <TouchDrawer
    direction="right"
...
export const EmergencyDrawer: React.FC<
  Omit<TouchDrawerProps, 'direction' | 'size'>
> = (props) => (
  <TouchDrawer
    direction="bottom"
