...
export default function StatusProgressionTimeline({
  ...
  const getNextStatusOptions = (status: string) => {
    return (
      NEXT_STATUS_OPTIONS[status as keyof typeof NEXT_STATUS_OPTIONS] || []
    );
  };

  const calculateAverageTimeInStatus = (status: string): number | null => {
    const statusEntries = statusHistory.filter(
      (h) => h.previousStatus === status,
    );
    if (statusEntries.length === 0) return null;

    const totalHours = statusEntries.reduce(
      (sum, entry) => sum + (entry.timeInPreviousStatusHours || 0),
      0,
    );
  ...
  const getCurrentStatusDuration = (): string => {
    ...
    const hoursSince = Math.floor(
      (new Date().getTime() -
        new Date(latestChange.statusChangedAt).getTime()) /
        (1000 * 60 * 60),
    );
              ...
              statusHistory.map((change, index) => {
                ...
                return (
                  <div key={change.id} className="relative flex gap-3">
                    ...
                    {/* Status icon */}
                    <div
                      className={`flex-shrink-0 w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm ${
                        isLatest
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-gray-500">
                          {format(
                            new Date(change.statusChangedAt),
                            'MMM d, HH:mm',
                          )}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                        {change.changedByUser && (
                          <div className="flex items-center gap-1">
                            ...
                            {change.changedByUser.firstName}{' '}
                            {change.changedByUser.lastName}
                          </div>
                        )}

                        {change.timeInPreviousStatusHours &&
