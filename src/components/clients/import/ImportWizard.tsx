...
const STEPS: ImportStep[] = [
  ...
  },
  {
    ...
    description: 'Map columns to client fields',
    completed: false,
  },
  {
    id: 'import',
    title: 'Import',
    description: 'Import clients to your database',
    completed: false,
  },
  {
    id: 'complete',
...
export default function ImportWizard({
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          try {
            ...
            resolve(
              clients.filter((client) => client.first_name || client.email),
            );
          } catch (error) {
            reject(error);
          }
  ...
  const autoDetectMappings = (columns: string[]): Record<string, string> => {
    ...
    Object.entries(COLUMN_MAPPINGS).forEach(
      ([targetField, possibleSources]) => {
  ...
  const validateData = (): ImportError[] => {
    ...
    parsedData.forEach((client, index) => {
      ...
      if (
        client.phone &&
        !/^[\+]?[1-9][\d]{0,15}$/.test(client.phone.replace(/[\s\-\(\)]/g, ''))
      ) {
        errors.push({
          row: rowNum,
          field: 'phone',
      ...
      if (
        client.wedding_date &&
        isNaN(new Date(client.wedding_date).getTime())
      ) {
        errors.push({
          row: rowNum,
          field: 'wedding_date',
      ...
      {steps.map((step, index) => (
        <div key={step.id} className="flex items-center">
          ...
          {index < steps.length - 1 && (
            <div
              className={`w-12 h-0.5 mx-2 ${step.completed ? 'bg-primary-600' : 'bg-gray-300'}`}
            />
          )}
        </div>
      ))}
    ...
    <Card className="p-8">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600">
          Import clients from a CSV or Excel file. Maximum file size: 50MB, up
          to 50,000 clients.
        </p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-green-600">
                {parsedData.length} clients found •{' '}
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Review the first 10 rows of your data. Total: {parsedData.length}{' '}
            clients
          </p>
        </div>
    ...
    <Card className="p-8">
          ...
          >
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600">
                Sample:{' '}
                {parsedData[0] && (parsedData[0] as any)[sourceColumn]
                  ? String((parsedData[0] as any)[sourceColumn]).substring(
                      0,
                      50,
                    )
                  : 'No data'}
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div className="text-sm text-blue-700 space-y-1">
          <p>
            {Object.keys(columnMappings).length} columns mapped automatically
          </p>
          <p>
            {detectedColumns.length - Object.keys(columnMappings).length}{' '}
            columns require manual mapping
          </p>
  ...
  const renderImportStep = () => (
    <Card className="p-8">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">
          Processing {parsedData.length} clients... Please don't close this
          window.
        </p>
      </div>
      ...
      <div className="text-center mb-8">
        {importResult?.successful_imports ? (
          <div className="space-y-4">
            ...
            <p className="text-gray-600">
              {importResult.successful_imports} out of{' '}
              {importResult.total_clients} clients imported
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <p className="text-gray-600">
              There were issues importing your clients. Please review the errors
              below.
            </p>
          </div>
        )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">
                  {importResult.performance_metrics.file_size_mb} MB
                </p>
              </div>
              <div>
                <p className="text-gray-600">Batch Size</p>
                <p className="font-medium">
                  {importResult.performance_metrics.batch_size}
                </p>
              </div>
              <div>
                <p className="text-gray-600">Batches Processed</p>
                <p className="font-medium">
                  {importResult.performance_metrics.batches_processed}
                </p>
              </div>
              <div>
