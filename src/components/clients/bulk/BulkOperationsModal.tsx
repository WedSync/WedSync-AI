...
interface LocalBulkOperationsModalProps extends BulkOperationsModalProps {
...
const EXPORT_FIELDS = [
  ...
  },
  {
    ...
    description: 'Date, venue, status',
  },
  {
    key: 'package_info',
    label: 'Package Information',
    description: 'Package name, price',
  },
  {
...
export function BulkOperationsModal({
  ...
  onExecute,
  isProcessing,
}: LocalBulkOperationsModalProps) {
  const [currentStep, setCurrentStep] = useState<
    'select' | 'configure' | 'confirm'
  >('select');
  const [currentOperation, setCurrentOperation] =
    useState<BulkOperation | null>(selectedOperation);
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Choose an operation to apply to {selectedClients.length} selected
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Set parameters for {selectedClients.length} selected client
            ...
            <div className="grid grid-cols-2 gap-3">
              {STATUS_OPTIONS.map((status) => (
                <button
                  key={status.value}
                  onClick={() =>
                    setParameters({ ...parameters, new_status: status.value })
                  }
                ...
                {EXPORT_FIELDS.map((field) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={
                        parameters.include_fields?.includes(field.key) || false
                      }
                      onChange={(e) => {
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-700 mb-4">
                    This action will permanently delete {selectedClients.length}{' '}
                    client{selectedClients.length !== 1 ? 's' : ''} and all
            ...
            <div>
              ...
              <div className="space-y-2 text-sm text-blue-800">
                <p>
                  ...
                  {selectedClients.length !== 1 ? 's' : ''} will be affected
                </p>
    ...
    <div className="fixed inset-0 z-50 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center">
      <div className="bg-white rounded-2xl max-w-2xl w-full mx-4 shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-2 h-2 rounded-full ${currentStep === 'select' ? 'bg-blue-600' : 'bg-gray-300'}`}
              />
              <div
                className={`w-2 h-2 rounded-full ${currentStep === 'configure' ? 'bg-blue-600' : 'bg-gray-300'}`}
              />
              <div
                className={`w-2 h-2 rounded-full ${currentStep === 'confirm' ? 'bg-blue-600' : 'bg-gray-300'}`}
              />
            </div>
          </div>
          <Button
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-200 bg-gray-50">
          <div className="text-sm text-gray-500">
            Step{' '}
            {currentStep === 'select' ? 1 : currentStep === 'configure' ? 2 : 3}{' '}
            of 3
          </div>
