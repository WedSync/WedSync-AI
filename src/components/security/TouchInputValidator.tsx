...
export default function TouchInputValidator({
  ...
  const [validationResult, setValidationResult] =
    useState<TouchValidationResult | null>(null);
  ...
  const calculateMotionMetrics = (
    ...
    const acceleration =
      points.length > 2 ? (velocity - 0) / (totalTime / 1000) : 0;
    ...
    <Card className={className}>
      ...
      <CardContent className="space-y-4">
        ...
        {validationResult && (
          <Alert
            className={
              validationResult.isHuman ? 'border-green-500' : 'border-red-500'
            }
          >
              ...
              <AlertDescription>
                <div className="space-y-1">
                  <div>
                    ...
                    {validationResult.isHuman ? 'Human' : 'Suspicious'}
                    (Confidence:{' '}
                    {(validationResult.confidence * 100).toFixed(1)}%)
                  </div>
                  <div>
                    <strong>Touch Points:</strong>{' '}
                    {validationResult.pattern.points.length}
                  </div>
                  <div>
                    <strong>Duration:</strong>{' '}
                    {(validationResult.pattern.duration / 1000).toFixed(2)}s
                  </div>
                  {validationResult.flags.length > 0 && (
                    <div>
                      <strong>Flags:</strong>{' '}
                      {validationResult.flags.join(', ')}
                    </div>
                  )}
                </div>
              </AlertDescription>
