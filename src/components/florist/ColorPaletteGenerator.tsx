...
export function ColorPaletteGenerator({
  ...
  const { generatedPalette, isGenerating, error, generatePalette } =
    useColorPalette();
    ...
    >
      ...
      {generatedPalette && (
        ...
        >
          <Card className="p-6">
            <div className="space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">
                    {generatedPalette.primary_palette?.palette_name ||
                      'Generated Palette'}
                  </h3>
                  <p className="text-sm text-gray-600 mt-1">
                    {weddingStyle.charAt(0).toUpperCase() +
                      weddingStyle.slice(1)}{' '}
                    • {season.charAt(0).toUpperCase() + season.slice(1)} •{' '}
                    {harmonyType.replace('_', ' ')}
                  </p>
                </div>
              ...
              {generatedPalette.primary_palette?.primary_colors && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {generatedPalette.primary_palette.primary_colors.map(
              ...
              {generatedPalette.primary_palette?.accent_colors && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {generatedPalette.primary_palette.accent_colors.map(
                      (color: any, index: number) => (
                        <div
                          key={index}
                          className="group cursor-pointer"
                ...
                includeNeutrals && (
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                      {generatedPalette.primary_palette.neutral_colors.map(
                        (color: any, index: number) => (
                          <div
                            key={index}
                            className="group cursor-pointer"
          ...
          {/* Flower Matches */}
          {generatedPalette.flower_matches &&
            generatedPalette.flower_matches.length > 0 && (
              <Card className="p-6">
                <h4 className="text-sm font-medium text-gray-700 mb-4">
                  Matching Flowers
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {generatedPalette.flower_matches.map(
                    (colorMatch: any, index: number) => (
                      <div
                        key={index}
                      ...
                      >
                        <div className="flex items-center gap-3 mb-3">
                          <div
                            ...
                            style={{
                              backgroundColor: colorMatch.target_color?.hex,
                            }}
                        ...
                        <div className="space-y-2">
                          {colorMatch.matching_flowers
                            ?.slice(0, 3)
                            .map((flowerMatch: any, fIndex: number) => (
                              <div
                                key={fIndex}
                                className="flex items-center justify-between text-sm"
                              >
                                <span className="text-gray-900">
                                  {flowerMatch.flower?.common_name}
                                </span>
                                <Badge variant="outline" className="text-xs">
                                  {(flowerMatch.color_similarity * 100).toFixed(
                                    0,
                                  )}
                                  % match
                                </Badge>
                          ...
                          {colorMatch.matching_flowers?.length > 3 && (
                            <div className="text-xs text-gray-500 text-center pt-1">
                              +{colorMatch.matching_flowers.length - 3} more
                              flowers
                            </div>
                          )}
                        </div>
            ...
            <Card className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="text-3xl font-bold text-blue-600">
                    {(
                      generatedPalette.seasonal_analysis.overall_fit * 100
                    ).toFixed(0)}
                    %
                  </div>
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-green-600">
                    {(
                      generatedPalette.seasonal_analysis.seasonal_fit_score *
                      100
                    ).toFixed(0)}
                    %
                  </div>
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-purple-600">
                    {(
                      generatedPalette.seasonal_analysis.color_match_score * 100
                    ).toFixed(0)}
                    %
                  </div>
                  <div className="text-sm text-gray-600 mt-1">
              ...
              {generatedPalette.seasonal_analysis.recommendations && (
                <div className="mt-6 space-y-2">
                  ...
                  {generatedPalette.seasonal_analysis.recommendations.map(
