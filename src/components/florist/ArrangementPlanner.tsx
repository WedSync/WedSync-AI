          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <select
                value={item.role}
                onChange={(e) =>
                  onRoleChange(item.id, e.target.value as FlowerItem['role'])
                }
                onClick={(e) => e.stopPropagation()}
...
export function ArrangementPlanner({
  ...
  const currentArrangementType = ARRANGEMENT_TYPES.find(
    (type) => type.id === arrangementType,
  );

  const arrangementSummary = useMemo(() => {
    const summary = flowerItems.reduce(
      (acc, item) => {
    ...
    const totalFlowers = Object.values(summary).reduce(
      (sum, count) => sum + count,
      0,
    );
  ...
  const addFlowerToArrangement = useCallback(
    (flower: any) => {
  ...
  const updateFlowerRole = useCallback(
    (id: string, role: FlowerItem['role']) => {
      setFlowerItems((prev) =>
  ...
  return (
    <div
      ...
      role="region"
      aria-label="Arrangement Planner"
    >
      {/* Header */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Badge variant="outline">{flowerItems.length} items</Badge>
          </div>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div className="font-medium">
                  {arrangementSummary.totalFlowers}
                </div>
                <div>Estimated Cost:</div>
                <div className="font-medium">
                  ${arrangementSummary.estimatedCost.toFixed(2)}
                </div>
              </div>
              ...
              <div className="space-y-2">
                {FLOWER_ROLES.map((role) => {
                  ...
                  const recommended =
                    currentArrangementType?.defaultStructure[
                      role.value as keyof typeof currentArrangementType.defaultStructure
                    ] || 0;
                  ...
                  return (
                    ...
                    >
                      <span
                        className={classNames('px-2 py-1 rounded', role.color)}
                      >
                        {role.label}
                      </span>
                      <span
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="text-center">
                    <div
                      className={classNames(
                        'text-lg font-bold',
                        aiSuggestions.balance_score > 0.8
                          ? 'text-green-600'
                          : aiSuggestions.balance_score > 0.6
                            ? 'text-yellow-600'
                            : 'text-red-600',
                      )}
                    >
                  ...
                  <div className="text-center">
                    <div
                      className={classNames(
                        'text-lg font-bold',
                        aiSuggestions.color_harmony > 0.8
                          ? 'text-green-600'
                          : aiSuggestions.color_harmony > 0.6
                            ? 'text-yellow-600'
                            : 'text-red-600',
                      )}
                    >
                  ...
                  <div className="text-center">
                    <div
                      className={classNames(
                        'text-lg font-bold',
                        aiSuggestions.seasonal_appropriateness > 0.8
                          ? 'text-green-600'
                          : aiSuggestions.seasonal_appropriateness > 0.6
                            ? 'text-yellow-600'
                            : 'text-red-600',
                      )}
                    >
                      {(aiSuggestions.seasonal_appropriateness * 100).toFixed(
                        0,
                      )}
                      %
                    </div>
                  ...
                  <div className="text-center">
                    <div
                      className={classNames(
                        'text-lg font-bold',
                        aiSuggestions.cost_efficiency > 0.8
                          ? 'text-green-600'
                          : aiSuggestions.cost_efficiency > 0.6
                            ? 'text-yellow-600'
                            : 'text-red-600',
                      )}
                    >
