...
export class ElasticsearchIntegration {
  ...
  async initializeIndices(): Promise<void> {
    try {
      ...
      console.log(
        '[ElasticSearch] Wedding vendor indices initialized successfully',
      );
    } catch (error) {
      console.error('[ElasticSearch] Failed to initialize indices:', error);
      throw new Error(
        `Index initialization failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
   ...
   */
  async searchVendors(
    searchQuery: WeddingSearchQuery,
  ): Promise<SearchResult<WeddingVendor>> {
  ...
  private buildVendorSearchQuery(query: WeddingSearchQuery): any {
    ...
    if (query.priceRange) {
      ...
      if (query.priceRange.min !== undefined) {
        priceFilters.push({
          range: { 'priceRange.min': { gte: query.priceRange.min } },
        });
      }

      if (query.priceRange.max !== undefined) {
        priceFilters.push({
          range: { 'priceRange.max': { lte: query.priceRange.max } },
        });
      }
  ...
  private async fallbackSearch(
    query: WeddingSearchQuery,
  ): Promise<SearchResult<WeddingVendor>> {
    console.warn(
      '[ElasticSearch] Using fallback search due to service degradation',
    );
    ...
    try {
      ...
      console.error(
        '[ElasticSearch] Fallback search also failed:',
        fallbackError,
      );
...
export function createElasticsearchIntegration(
  config: any,
