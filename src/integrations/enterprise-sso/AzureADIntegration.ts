...
export class AzureADIntegration {
  ...
  async authenticateOAuth2(
    ...
    try {
      ...
      console.log(
        `Azure AD OAuth2 authentication successful for: ${userProfile.mail || userProfile.userPrincipalName}`,
      );
  ...
  async authenticateDeviceCode(
    ...
    try {
      ...
      console.log(
        `Azure AD device code authentication successful for: ${userProfile.mail || userProfile.userPrincipalName}`,
      );
  ...
  async validateIdToken(
    ...
    try {
      ...
      console.log(
        `Azure AD ID token validation successful for: ${verified.email || verified.upn}`,
      );
      ...
      return {
        ...
        provider: provider.id,
        error:
          error instanceof Error ? error.message : 'ID token validation failed',
      ...
      if (users.value) {
        for (const user of users.value) {
          try {
            ...
            results.errors.push(
              `Failed to process user ${user.userPrincipalName}: ${error}`,
            );
          }
        }
      }
        ...
        if (groups.value) {
          for (const group of groups.value) {
            try {
              ...
              results.errors.push(
                `Failed to process group ${group.displayName}: ${error}`,
              );
            }
          }
        }
  ...
  private async getUserProfile(
    ...
    try {
      ...
      const user = await this.graphClient.users(userId).get({
  ...
  private async getUserGroups(userId: string): Promise<Group[]> {
    ...
    try {
      const groups = await this.graphClient.users(userId).memberOf.get({
  ...
  private mapGroupsToRoles(
    ...
    if (provider.groupMapping) {
      for (const [azureGroup, wedSyncRole] of Object.entries(
        provider.groupMapping,
      )) {
        if (
          groupNames.some((group) =>
            group.toLowerCase().includes(azureGroup.toLowerCase()),
          )
        ) {
