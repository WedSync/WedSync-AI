...
describe('ActiveDirectoryIntegration', () => {
  ...
  describe('User Authentication', () => {
    it('should authenticate user successfully', async () => {
      ...
      mockClient.search.mockImplementation((base, options, callback) => {
        const mockEntry = {
          ...
          attributes: [
            ...
            {
              type: 'memberOf',
              values: ['CN=Domain Users,CN=Groups,DC=example,DC=com'],
            },
          ],
        };
      ...
      const result = await adIntegration.authenticate(
        'testuser',
        'password123',
      );
      ...
      expect(result.user?.groups).toContain(
    ...
    it('should handle user not found', async () => {
      ...
      mockClient.bind.mockImplementationOnce((dn, password, callback) =>
        callback(null),
      );
  ...
  describe('User Search and Retrieval', () => {
    it('should search users by criteria', async () => {
      ...
      const users = await adIntegration.searchUsers({
  ...
  describe('Group Operations', () => {
    ...
    it('should validate group membership', async () => {
      ...
      mockClient.search.mockImplementation((base, options, callback) => {
        const mockEntry = {
          dn: groupDN,
          attributes: [{ type: 'member', values: [userDN] }],
        };
  ...
  describe('Wedding Vendor Integration', () => {
    it('should authenticate wedding vendor with business verification', async () => {
      ...
      mockClient.search.mockImplementation((base, options, callback) => {
        const mockEntry = {
          ...
          attributes: [
            ...
            {
              type: 'memberOf',
              values: ['CN=Wedding Vendors,CN=Groups,DC=example,DC=com'],
            },
          ],
        };
      ...
      expect(result.user?.groups).toContain(
        'CN=Wedding Vendors,CN=Groups,DC=example,DC=com',
      );
    });
  });
