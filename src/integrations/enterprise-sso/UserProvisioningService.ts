...
export class UserProvisioningService {
  ...
  async configureProvisioning(
    ...
    try {
      ...
      await this.supabase.from('user_provisioning_configs').upsert({
      ...
      console.error(
        `Failed to configure provisioning for provider ${providerId}:`,
        error,
      );
      throw error;
    }
  ...
  async configureRoleAssignmentRules(
    ...
    if (sortedRules.length > 0) {
      await this.supabase.from('role_assignment_rules').insert(
  ...
  async provisionUser(
    ...
    try {
      ...
      if (!config) {
        throw new Error(
          `No provisioning configuration found for provider: ${providerId}`,
        );
      }
      ...
      if (config.approvalRequired) {
        ...
        return {
          ...
          operation: 'no_change',
          warnings: [
            `User provisioning request created: ${requestId}. Approval required.`,
          ],
          timestamp: startTime,
        };
      }
  ...
  async deprovisionUser(
    ...
    try {
      console.log(
        `Starting user deprovisioning for: ${userIdentifier} (${providerId})`,
      );
      ...
      if (policy?.trigger === 'delayed' && policy.gracePeridDays) {
        ...
        await this.scheduleDeprovisioning(
          user.id,
          policy.gracePeridDays,
          reason,
        );
        ...
        return {
          ...
          operation: 'no_change',
          warnings: [
            `User deprovisioning scheduled for ${policy.gracePeridDays} days`,
          ],
          timestamp: startTime,
        };
      }
  ...
  async processProvisioningApproval(
    ...
    try {
      ...
      console.error(
        `Failed to process provisioning approval for ${requestId}:`,
        error,
      );
      throw error;
    }
  ...
  async bulkProvisionUsers(
    ...
    console.log(
      `Starting bulk provisioning of ${users.length} users for provider: ${providerId}`,
    );
    ...
    console.log(
      `Bulk provisioning completed. Successful: ${successful}, Failed: ${failed}`,
    );
  ...
  private async loadConfigurations(): Promise<void> {
    try {
      ...
      console.log(
        `Loaded ${this.provisioningConfigs.size} provisioning configurations`,
      );
    } catch (error) {
   ...
   */
  private async validateProvisioningConfig(
    config: ProvisioningConfig,
  ): Promise<void> {
    ...
    if (!org) {
      throw new Error(
        `Default organization not found: ${config.defaultOrganizationId}`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        'Approver roles must be specified when approval is required',
      );
    }
  }
  ...
  private async findExistingUser(
    ...
    const { data } = await this.supabase
      ...
      .select('id, email')
      .or(
        `email.eq.${email}${identifier ? `,enterprise_user_id.eq.${identifier}` : ''}`,
      )
      .single();
  ...
  private async updateExistingUser(
    ...
    const newRoles = await this.determineUserRoles(
      config.providerId,
      attributes,
    );
    ...
    console.log(
      `User updated successfully: ${userId} (${changes.length} changes)`,
    );
  ...
  private constructFullName(attributes: EnterpriseUserAttributes): string {
    ...
    return (
      (attributes.customAttributes?.displayName as string) ||
  ...
  private evaluateRoleCondition(
    ...
    return value
      ? this.matchesOperator(value, condition.value, condition.operator)
      : false;
  }
  ...
  private async createProvisioningRequest(
    ...
    await this.supabase.from('provisioning_requests').insert({
      id: requestId,
  ...
  private async getProvisioningRequest(
    requestId: string,
  ...
  private async scheduleDeprovisioning(
    ...
    await this.supabase.from('scheduled_deprovisionings').insert({
      user_id: userId,
  ...
  private async sendDeprovisioningNotification(
    ...
    console.log(
      `Sending deprovisioning notification: ${operation} - ${reason}`,
    );
  }
}
