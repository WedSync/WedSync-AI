...
export class WeddingTeamDirectorySync {
  ...
  async createWeddingTeamDirectory(
    ...
    try {
      ...
      const directory: WeddingTeamDirectory = {
        ...
        defaultPermissions: directoryData.defaultPermissions || [
          'view_schedule',
          'view_contacts',
        ],
      ...
      // Store directory
      await this.supabase.from('wedding_team_directories').insert({
      ...
      console.log(
        `Wedding team directory created: ${directoryId} for wedding ${weddingId}`,
      );
  ...
  async addTeamMember(
    ...
    try {
      ...
      console.log(
        `Team member added: ${member.firstName} ${member.lastName} (${memberId})`,
      );
  ...
  async synchronizeWithPlatform(
    ...
    try {
      console.log(
        `Starting ${syncType} sync: directory ${directoryId} with platform ${platformId}`,
      );
      ...
      if (
        platform.syncDirection === 'import_only' ||
        platform.syncDirection === 'bidirectional'
      ) {
        platformMembers = await this.fetchMembersFromPlatform(
          platform,
          directory,
        );
      }
      ...
      for (const platformMember of platformMembers) {
        try {
          const mappedMember = this.mapPlatformMemberToWedSync(
            platformMember,
            platform,
          );
          ...
          if (existingMember) {
            ...
            const conflicts = this.detectMemberConflicts(
              existingMember,
              mappedMember,
            );
            ...
            // Add new member
            const newMemberId = await this.addTeamMember(
              directoryId,
              mappedMember,
            );
            await this.linkMemberToExternalId(
              newMemberId,
              platformId,
              platformMember.id,
            );
      ...
      if (
        platform.syncDirection === 'export_only' ||
        platform.syncDirection === 'bidirectional'
      ) {
        await this.exportMembersToExternalPlatform(
          directoryId,
          platform,
          result,
        );
      ...
      console.log(
        `Sync completed: ${result.totalProcessed} processed, ${result.membersAdded} added, ${result.membersUpdated} updated`,
      );
  ...
  async configurePlatformIntegration(
    ...
    try {
      ...
      if (existingPlatformIndex >= 0) {
        directory.connectedPlatforms[existingPlatformIndex] =
          platformConnection;
      } else {
      ...
      await this.supabase.from('directory_platform_configs').upsert({
        directory_id: directoryId,
      ...
      console.log(
        `Platform integration configured: ${platform.platformName} for directory ${directoryId}`,
      );
    } catch (error) {
  ...
  private getDefaultRolePermissions(): Record<WeddingTeamRole, string[]> {
    ...
    const rolePermissions: Record<WeddingTeamRole, string[]> = {} as Record<
      WeddingTeamRole,
      string[]
    >;
    ...
    Object.values([
      ...
      'maid_of_honor',
      'best_man',
    ] as WeddingTeamRole[]).forEach((role) => {
  ...
  private async loadPlatformIntegrations(): Promise<void> {
    try {
      ...
      console.log(
        `Loaded ${this.platformIntegrations.size} platform integrations`,
      );
    } catch (error) {
    ...
    try {
      ...
      if (directories) {
        for (const dir of directories) {
          this.teamDirectories.set(
            dir.directory_id,
            this.transformDirectoryData(dir),
          );
        }
      }

      console.log(
        `Loaded ${this.teamDirectories.size} wedding team directories`,
      );
    } catch (error) {
  ...
  private transformMemberData(data: any): WeddingTeamMember {
    return {
      ...
      inviteSentAt: data.invite_sent_at
        ? new Date(data.invite_sent_at)
        : undefined,
      responseDate: data.response_date
        ? new Date(data.response_date)
        : undefined,
  ...
  private async getTeamDirectory(
    directoryId: string,
  ): Promise<WeddingTeamDirectory | null> {
  ...
  private mapPlatformMemberToWedSync(
    ...
    WeddingTeamMember,
    'memberId' | 'weddingId' | 'addedDate' | 'externalIds' | 'platformProfiles'
  > {
  ...
  private async resolveConflict(
    ...
    console.log(
      `Resolving conflict for member ${memberId}: ${conflict.description}`,
    );
  }
    ...
    const errorScore =
      result.totalProcessed > 0
        ? Math.max(
            0,
            100 -
              (result.errors.length / result.totalProcessed) *
                100 *
                errorWeight,
          )
        : 100;
    ...
    const conflictScore =
      result.totalProcessed > 0
        ? Math.max(
            0,
            100 -
              (result.conflicts.length / result.totalProcessed) *
                100 *
                conflictWeight,
          )
        : 100;
  ...
  private async storeSyncResult(
    ...
    await this.supabase.from('team_directory_sync_results').insert({
  ...
  private calculateDirectoryStatistics(members: WeddingTeamMember[]): any {
    const stats = {
      ...
      platformSyncStatus: {} as Record<
        string,
        { lastSync: Date; errors: number }
      >,
    };

    for (const member of members) {
      // Count by role
      stats.membersByRole[member.role] =
        (stats.membersByRole[member.role] || 0) + 1;

      // Count by category
      stats.membersByCategory[member.category] =
        (stats.membersByCategory[member.category] || 0) + 1;
      ...
      // Invite status
      stats.inviteStatus[member.inviteStatus] =
        (stats.inviteStatus[member.inviteStatus] || 0) + 1;
    }
  ...
  private async sendMemberInvitation(
    ...
    console.log(
      `Sending invitation to ${member.firstName} ${member.lastName} for wedding ${directory.weddingId}`,
    );
  }
  ...
  private async syncMemberToConnectedPlatforms(
    ...
    console.log(
      `Syncing member ${member.firstName} ${member.lastName} to connected platforms`,
    );
  }
}
