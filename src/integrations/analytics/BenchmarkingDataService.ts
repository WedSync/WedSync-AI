...
export class BenchmarkingDataService {
  ...
  async updateBenchmarkData(sourceId?: string): Promise<BenchmarkData[]> {
    try {
      console.log(
        `📊 Updating benchmark data${sourceId ? ` from ${sourceId}` : ''}...`,
      );
      ...
      for (const source of sourcesToUpdate) {
        try {
          ...
          if (benchmarks.length > 0) {
            ...
            if (error) {
              console.error(
                `❌ Failed to store benchmarks from ${source.name}:`,
                error,
              );
            } else {
              console.log(
                `✅ Updated ${benchmarks.length} benchmarks from ${source.name}`,
              );
          ...
          console.error(
            `❌ Failed to fetch benchmarks from ${source.name}:`,
            error,
          );
        }
      }
      ...
      console.error('❌ Failed to update benchmark data:', error);
      throw error;
    }
  }
  ...
  async getBenchmarkData(
    ...
    try {
      ...
      const hoursOld =
        (Date.now() - new Date(benchmark.collected_at).getTime()) /
        (1000 * 60 * 60);
      if (hoursOld > this.config.qualityThresholds.dataFreshnessHours) {
        console.warn(
          `⚠️ Benchmark data is ${Math.round(hoursOld)} hours old for ${metricType}`,
        );
      }

      // Check sample size
      if (
        benchmark.sample_size < this.config.qualityThresholds.minimumSampleSize
      ) {
        console.warn(
          `⚠️ Benchmark has low sample size (${benchmark.sample_size}) for ${metricType}`,
        );
      }
  ...
  async compareVendorPerformance(
    ...
    try {
      console.log(
        `📈 Comparing vendor ${vendorId} against industry benchmarks...`,
      );
      ...
      if (!vendorMetrics?.length) {
        console.warn(`⚠️ No recent metrics found for vendor: ${vendorId}`);
        return comparisons;
      }
      ...
      for (const [metricType, vendorMetric] of metricsToProcess) {
        try {
          ...
          const vendorCategory = await this.determineVendorCategory(
            vendorId,
            organizationId,
          );

          // Get regional benchmark
          const region = await this.determineVendorRegion(
            vendorId,
            organizationId,
          );
          ...
          if (!benchmark) {
            console.warn(`⚠️ No benchmark available for ${metricType}`);
            continue;
          }
          ...
          console.error(`❌ Failed to compare ${metricType}:`, error);
        }
      }

      // Store comparison results for analytics
      await this.storeBenchmarkComparisons(
        vendorId,
        organizationId,
        comparisons,
      );
      ...
      console.error('❌ Failed to compare vendor performance:', error);
      throw error;
    }
  }
  ...
  async generateIndustryReport(
    ...
    try {
      console.log(`📊 Generating industry performance report for ${region}...`);
      ...
      await this.supabase.from('industry_reports').insert([
        {
          region,
   ...
   */
  private async fetchBenchmarksFromSource(
    ...
    try {
      ...
      for (const metricType of source.metricTypes) {
        for (const region of source.regions) {
          for (const category of this.config.industryCategories) {
            ...
            const baseBenchmark = this.generateRealisticBenchmark(
              metricType,
              category,
              region,
            );
            ...
            for (const season of seasons) {
              const seasonalBenchmark = this.generateRealisticBenchmark(
                metricType,
                category,
                region,
                season,
      ...
      console.log(
        `🎯 Generated ${benchmarks.length} benchmark records from ${source.name}`,
      );
      return benchmarks;
    } catch (error) {
  ...
  private generateRealisticBenchmark(
    ...
    const adjustedValue =
      baseValue * categoryMultiplier * seasonalMultiplier * regionMultiplier;
  ...
  private calculateBenchmarkComparison(
    ...
    const deviationFromBenchmark =
      ((vendorMetric.value - benchmark.benchmark_value) /
  ...
  private categorizePerformance(
    percentileRank: number,
  ): BenchmarkComparison['performanceCategory'] {
  ...
  private generateImprovementSuggestions(
    ...
    if (percentileRank < 50) {
      switch (metricType) {
        case 'RESPONSE_TIME':
          ...
          suggestions.push(
            'Set up calendar booking to reduce back-and-forth communication',
          );
          break;
        case 'SATISFACTION_SCORE':
          suggestions.push(
            'Conduct client feedback surveys to identify improvement areas',
          );
          suggestions.push(
            'Implement a client communication checklist for consistency',
          );
          break;
        case 'BOOKING_CONVERSION':
          suggestions.push('Review and optimize your pricing strategy');
          suggestions.push(
            'Improve portfolio presentation and client testimonials',
          );
          break;
        case 'QUALITY_RATING':
          suggestions.push(
            'Focus on exceeding client expectations in key service areas',
          );
          suggestions.push(
    ...
    if (deviation < -20) {
      suggestions.push(
        `Your ${metricType.toLowerCase().replace('_', ' ')} is significantly below industry average`,
      );
      suggestions.push(
        'Consider investing in professional development or training',
      );
    }
  ...
  private async determineVendorCategory(
    vendorId: string,
    organizationId: string,
  ): Promise<string> {
  ...
  private async determineVendorRegion(
    vendorId: string,
    organizationId: string,
  ): Promise<string> {
  ...
  private calculateIndustrySummary(benchmarks: BenchmarkData[]): any {
    ...
    const metricAverages: Record<MetricType, number> = {} as Record<
      MetricType,
      number
    >;
    ...
    for (const benchmark of benchmarks) {
      ...
      categoryPerformance
        .get(benchmark.vendor_category)!
        .push(benchmark.benchmark_value);
    }

    // Calculate averages
    const metricCounts: Record<MetricType, number> = {} as Record<
      MetricType,
      number
    >;
    for (const benchmark of benchmarks) {
      metricCounts[benchmark.metric_type] =
        (metricCounts[benchmark.metric_type] || 0) + 1;
    }

    for (const [metricType, total] of Object.entries(metricAverages)) {
      metricAverages[metricType as MetricType] =
        total / metricCounts[metricType as MetricType];
    }
  ...
  private analyzeTrends(benchmarks: BenchmarkData[]): any {
    ...
    for (const [metricType, metricBenchmarks] of metricGroupEntries) {
      const sortedByTime = metricBenchmarks.sort(
        (a, b) =>
      ...
      const firstAvg =
        firstHalf.reduce((sum, b) => sum + b.benchmark_value, 0) /
        firstHalf.length;
      const secondAvg =
        secondHalf.reduce((sum, b) => sum + b.benchmark_value, 0) /
        secondHalf.length;
  ...
  private generateIndustryRecommendations(
    ...
    for (const [metricType, trend] of Object.entries(trends)) {
      if (trend.trend === 'DECREASING') {
        recommendations.push(
          `Industry-wide ${metricType.toLowerCase().replace('_', ' ')} is declining - focus on improvement initiatives`,
        );
      }

      if (trend.changePercent < -10) {
        recommendations.push(
          `Significant decline in ${metricType.toLowerCase().replace('_', ' ')} requires immediate industry attention`,
        );
      }
    }
    ...
    recommendations.push(
      'Implement regular performance monitoring and benchmarking',
    );
    recommendations.push(
      'Focus on client communication and satisfaction improvement',
    );
    recommendations.push(
      'Consider seasonal business strategies to optimize performance year-round',
    );
  ...
  private async storeBenchmarkComparisons(
    ...
    try {
      ...
      if (error) {
        console.error('❌ Failed to store benchmark comparisons:', error);
      }
    } catch (error) {
      console.error('❌ Error storing benchmark comparisons:', error);
    }
  }
  ...
  private startBenchmarkUpdates(): void {
    ...
    this.updateInterval = setInterval(
      async () => {
        try {
          console.log('🔄 Starting automatic benchmark update...');
          await this.updateBenchmarkData();
        } catch (error) {
          console.error('❌ Automatic benchmark update error:', error);
        }
      },
      24 * 60 * 60 * 1000,
    ); // 24 hours
