...
class TranslationCache {
  private memoryCache: Map<
    string,
    { data: TranslationResult; expiry: number }
  > = new Map();
  ...
  public async set(result: TranslationResult): Promise<void> {
    ...
    if (this.supabase) {
      try {
        await this.supabase.from('translation_cache').upsert({
          cache_key: key,
...
export class GoogleTranslateIntegration {
  ...
  public async translateText(
    ...
    try {
      ...
      const result: TranslationResult = {
        ...
        sourceLanguage:
          googleResponse.detectedSourceLanguage || request.sourceLanguage,
    ...
    if (identifiedTerms.length > 0) {
      ...
      const preservedTerms = identifiedTerms.filter((term) => {
        ...
        return translatedText
          .toLowerCase()
          .includes(term.term.split(' ')[0].toLowerCase());
      });
  ...
  private handleTranslationError(
    error: any,
    request: Partial<TranslationRequest>,
  ): TranslationError {
...
export function createGoogleTranslateIntegration(
  config: Partial<GoogleTranslateConfig> & {
    apiKey: string;
    projectId: string;
  },
  supabaseUrl?: string,
  ...
  return new GoogleTranslateIntegration(
    defaultConfig,
    supabaseUrl,
    supabaseKey,
  );
