...
export class QualityAssuranceTranslation {
  private weddingTerminology: Map<string, WeddingTerminologyEntry[]> =
    new Map();
  ...
  async passesQualityGate(
    ...
    // Check overall score
    if (
      assessmentResult.overall_score < this.qualityGateConfig.min_overall_score
    ) {
      passes = false;
      reasons.push(
        `Overall score ${assessmentResult.overall_score} below minimum ${this.qualityGateConfig.min_overall_score}`,
      );
    }
    ...
    if (
      minDimensions.accuracy &&
      assessmentResult.dimensions.accuracy < minDimensions.accuracy
    ) {
      passes = false;
      reasons.push(
        `Accuracy score ${assessmentResult.dimensions.accuracy} below minimum ${minDimensions.accuracy}`,
      );
    }

    // Check critical and high issues
    const criticalIssues = assessmentResult.issues.filter(
      (issue) => issue.severity === 'critical',
    ).length;
    const highIssues = assessmentResult.issues.filter(
    ...
    if (criticalIssues > this.qualityGateConfig.max_critical_issues) {
      passes = false;
      reasons.push(
        `Too many critical issues: ${criticalIssues} (max: ${this.qualityGateConfig.max_critical_issues})`,
      );
    }
    ...
    if (highIssues > this.qualityGateConfig.max_high_issues) {
      passes = false;
      reasons.push(
        `Too many high-severity issues: ${highIssues} (max: ${this.qualityGateConfig.max_high_issues})`,
      );
    }

    // Check content-specific rules
    const contentRules =
      this.qualityGateConfig.content_type_specific_rules[contentType];
    if (contentRules) {
  ...
  private async assessQualityDimensions(
    ...
    return {
      ...
      terminology: await this.assessTerminology(
        originalText,
        translatedText,
        contentType,
        targetLanguage,
      ),
      cultural_appropriateness: await this.assessCulturalAppropriateness(
        translatedText,
        targetLanguage,
        contentType,
      ),
  ...
  private async assessAccuracy(
    ...
    const entityPreservationRate = this.calculateEntityPreservation(
      originalEntities,
      translatedEntities,
    );
  ...
  private async assessTerminology(
    ...
    if (termsUsed > 0) {
      ...
      terminologyScore = Math.round(
        (terminologyScore + terminologyAccuracy) / 2,
      );
    }

    // Bonus for using content-specific terminology
    const contentSpecificTerms = this.getContentSpecificTerms(
      contentType,
      targetLanguage,
    );
  ...
  private async assessConsistency(
    ...
    const terminologyConsistency =
      this.checkTerminologyConsistency(translatedText);
    consistencyScore = Math.round(
      (consistencyScore + terminologyConsistency) / 2,
    );
  ...
  private async identifyQualityIssues(
    ...
    if (dimensions.accuracy < 60) {
      issues.push({
        ...
        suggested_fix:
          'Review translation for semantic accuracy and missing content',
      });
    }
    ...
    const protocolViolations = this.checkWeddingProtocolViolations(
      translatedText,
      contentType,
    );
      ...
      switch (issue.type) {
        case QualityIssueType.TERMINOLOGY_MISMATCH:
          recommendations.push({
            ...
            action:
              'Use WedSync terminology database for consistent translations',
    ...
    if (contentRules) {
      ...
      return Math.round(
        (dimensions.accuracy * contentRules.accuracy_weight +
          ...
          (contentRules.accuracy_weight +
            contentRules.fluency_weight +
            contentRules.terminology_weight +
            contentRules.cultural_weight +
            0.25),
      );
    }
  ...
  private isTermAppropriateForContent(
    term: WeddingTerminologyEntry,
    contentType: WeddingContentType,
  ): boolean {
  ...
  private getContentSpecificTerms(
    contentType: WeddingContentType,
    language: string,
  ): string[] {
  ...
  private checkWeddingProtocolViolations(
    text: string,
    contentType: WeddingContentType,
  ): QualityIssue[] {
  ...
  private getValidationFlags(
    ...
    if (
      issues.some((i) => i.type === QualityIssueType.CULTURAL_INSENSITIVITY)
    ) {
      flags.push('CULTURAL_REVIEW_NEEDED');
    }

    if (
      issues.some((i) => i.type === QualityIssueType.WEDDING_PROTOCOL_VIOLATION)
    ) {
