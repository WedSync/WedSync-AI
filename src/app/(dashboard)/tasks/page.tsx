...
const mockTemplates: TaskTemplate[] = [
  {
    ...
    category: 'venue_management',
    description:
      'Complete ceremony venue setup including seating, decorations, and AV',
    tasks: [
      {
...
export default function TasksPage() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (pageState.filters.assigned_to?.length) {
      filtered = filtered.filter(
        (task) =>
          task.assigned_to &&
          pageState.filters.assigned_to!.includes(task.assigned_to),
      );
    }
    ...
    if (pageState.filters.deadline_from) {
      filtered = filtered.filter(
        (task) => task.deadline >= pageState.filters.deadline_from!,
      );
    }

    if (pageState.filters.deadline_to) {
      filtered = filtered.filter(
        (task) => task.deadline <= pageState.filters.deadline_to!,
      );
    }
  ...
  const handleFilterChange = useCallback(
    (filterKey: keyof TaskFilter, value: any) => {
      setPageState((prev) => ({
        ...prev,
        filters: {
          ...prev.filters,
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Plan, organize, and track wedding tasks with intelligent
              scheduling
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() =>
                setPageState((prev) => ({ ...prev, isCreateModalOpen: true }))
              }
              className="
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          {/* Clear Filters */}
          {(pageState.searchQuery ||
            Object.keys(pageState.filters).length > 0) && (
            <button
              onClick={() =>
                setPageState((prev) => ({
                  ...prev,
      ...
      {pageState.isCreateModalOpen && (
        <div className="fixed inset-0 z-50 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div
            ...
            data-testid="task-creator-modal"
          >
            <TaskCreator
              ...
              onCancel={() =>
                setPageState((prev) => ({ ...prev, isCreateModalOpen: false }))
              }
      ...
      <TaskTemplateLibrary
        ...
        onClose={() =>
          setPageState((prev) => ({ ...prev, isTemplateLibraryOpen: false }))
        }
