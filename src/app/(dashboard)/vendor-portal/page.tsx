      ...
      const formattedWeddings =
        data?.map((connection) => ({
          ...
          couple_names: `${connection.clients?.first_name || ''} ${connection.clients?.last_name || ''}${
            connection.clients?.partner_first_name
              ? ` & ${connection.clients?.partner_first_name} ${connection.clients?.partner_last_name || ''}`
              : ''
          }`,
          wedding_date: connection.clients?.wedding_date || '',
          venue_name: connection.clients?.venue_name || 'TBD',
          status:
            connection.clients?.status === 'completed'
              ? 'completed'
              : new Date(connection.clients?.wedding_date || '') > new Date()
                ? 'upcoming'
                : 'active',
  ...
  if (!vendorProfile) {
    return (
      <div className="max-w-2xl mx-auto mt-16 text-center">
        ...
        <Button href="/vendors/setup">Create Vendor Profile</Button>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-right text-sm text-gray-500">
              <div>
                Profile: {vendorProfile.profile_completion_score}% complete
              </div>
