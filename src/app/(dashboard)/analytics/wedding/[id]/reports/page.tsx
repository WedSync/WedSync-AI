...
async function WeddingReportBuilder({ weddingId }: { weddingId: string }) {
  ...
  return <ReportBuilder weddingId={weddingId} onSave={handleSaveReport} />;
}
...
export async function generateMetadata({ params }: WeddingReportsPageProps) {
  ...
  return {
    ...
    description:
      'Create custom analytics reports with drag-and-drop widgets for comprehensive wedding planning insights',
  };
}

export default async function WeddingReportsPage({
  params,
}: WeddingReportsPageProps) {
  const { id: weddingId } = await params;
