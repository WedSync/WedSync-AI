...
function generateMockData(): {
  ...
  const vendorNames = [
    ...
    'Luxury Venues Ltd',
    'Petal Perfect Florists',
    'Fine Food Catering',
    'Music Makers DJ',
    'Cinema Wedding Films',
    'Wedding Coordination Experts',
    'Capture the Moment',
    'Vintage Venue Spaces',
    'Botanical Blooms',
    'Culinary Creations',
    'Sound & Light Entertainment',
  ];

  const vendorMetrics: VendorPerformanceMetrics[] = vendorNames.map(
    (name, index) => ({
  ...
  const totalBookings = vendorMetrics.reduce(
  ...
  const dashboardData: AnalyticsDashboardData = {
    ...
    topPerformers: vendorMetrics
      .sort(
        (a, b) =>
          b.totalRevenue +
          b.totalBookings * 1000 -
          (a.totalRevenue + a.totalBookings * 1000),
      )
      .slice(0, 5),
    recentActivity: [],
    performanceTrends: [],
      ...
      try {
        ...
        if (format === 'csv') {
          const csvData = data.vendorMetrics.map((vendor) => ({
            ...
            'Booking Success Rate (%)': Math.round(
              vendor.bookingSuccessRate * 100,
            ),
