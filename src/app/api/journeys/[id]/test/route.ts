    ...
    for (const node of nodes) {
      if (node.type === 'timeline' && node.timeline_config) {
        try {
          ...
          if (actionError) {
            errors.push(
              `Failed to schedule action for node ${node.name}: ${actionError.message}`,
            );
          } else {
          ...
          errors.push(
            `Error calculating schedule for node ${node.name}: ${error.message}`,
          );
        }
      }
    }
...
function calculateTimelineDate(
  ...
  const offsetValue =
    timelineConfig.offset.direction === 'before'
  ...
  if (timelineConfig.business_hours) {
    const [startHour, startMinute] = timelineConfig.business_hours.start
      .split(':')
      .map(Number);
