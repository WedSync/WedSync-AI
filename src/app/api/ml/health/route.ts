...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const meetsRequirements = {
      ...
      performance: healthCheck.vendor_analyzer.performance >= 0.8,
    ...
    const response = {
      ...
      components: {
        conflict_detector: {
          ...
          meets_accuracy_requirement:
            healthCheck.conflict_detector.accuracy >= 0.85,
        ...
        vendor_analyzer: {
          ...
          meets_performance_requirement:
            healthCheck.vendor_analyzer.performance >= 0.8,
          target_performance: 0.8,
        },
        timeline_optimizer: {
          status: healthCheck.timeline_optimizer.status,
          optimization_score: healthCheck.timeline_optimizer.optimization_score,
          meets_optimization_requirement:
            healthCheck.timeline_optimizer.optimization_score >= 0.75,
      ...
      features: {
        ...
        timeline_optimization:
          healthCheck.timeline_optimizer.status === 'healthy',
...
function generateRecommendations(healthCheck: any): string[] {
  ...
  if (healthCheck.conflict_detector.accuracy < 0.85) {
    recommendations.push(
      'Retrain conflict detection model to improve accuracy',
    );
  }

  if (healthCheck.vendor_analyzer.performance < 0.8) {
