...
export async function GET(request: NextRequest) {
  try {
    ...
    const [emailStatus, smsStatus, serviceStatus] = await Promise.all([
      checkServiceSafely('email', () =>
        emailServiceConnector.getServiceStatus(),
      ),
      checkServiceSafely('sms', () => smsServiceConnector.getServiceStatus()),
      journeyIds.length > 0
        ? checkServiceSafely('journey_bridge', () =>
            journeyServiceBridge.getJourneyServiceStatus(journeyIds),
          )
    ...
    if (includeMetrics) {
      const metricsResult = await checkServiceSafely(
        'delivery_metrics',
        async () => {
    ...
    const response = {
      ...
      health_summary: {
        ...
        degraded_services: services.filter((s) => s.status === 'degraded')
          .length,
        unhealthy_services: services.filter((s) => s.status === 'unhealthy')
          .length,
      },
    };
    ...
    switch (action) {
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid action. Supported: test_email, test_sms, restart_connections',
          },
          { status: 400 },
        );
    }
