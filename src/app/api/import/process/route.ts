...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < rawData.length; i += BATCH_SIZE) {
      ...
      const clientsToInsert = parsedBatch.map((client: any) => ({
        ...
        wedding_date: client.wedding_date
          ? new Date(client.wedding_date).toISOString()
          : null,
      ...
      if (options.skipDuplicates) {
        const emails = clientsToInsert.map((c) => c.email).filter(Boolean);
    ...
    try {
      const supabase = await createClient();
      const body: ProcessRequest = await request
        .json()
        .catch(() => ({ importId: '' }));
