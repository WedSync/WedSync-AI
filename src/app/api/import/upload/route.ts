...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: metadataError } = await supabase.from('import_jobs').insert({
      id: importId,
      user_id: user.id,
      file_name: file.name,
      file_size: file.size,
    ...
    if (fileExtension === '.csv') {
      ...
      if (jsonData.length < 2) {
        await supabase
          ...
          .update({
            status: 'failed',
            error: 'File must contain headers and data',
          })
          .eq('id', importId);
    ...
    const mappings = columnMapper.autoDetectMappings(
      columns,
    ...
    const firstBatch = parsedData.slice(
      0,
      Math.min(BATCH_SIZE, parsedData.length),
    );
    const parsedBatch = weddingDataParser.parseRows(
      firstBatch,
      columnMappingsObj,
    );

    // Validate parsed data
    const validationResult = await importValidator.validateBatch(
      parsedBatch.data,
      true,
    );
    ...
    const { error: storageError } = await supabase.storage.from('temp').upload(
      uploadPath,
      JSON.stringify({
        columns,
        data: parsedData,
