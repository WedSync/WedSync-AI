...
async function secureOptimizeHandler(
  ...
  try {
    ...
    const { data: supplier, error: supplierError } = await supabase
      .from('suppliers')
      .select(
        'id, organization_id, status, subscription_tier, daily_budget_pounds',
      )
      .eq('id', supplierId)
      .single();
    ...
    const criticalAlerts = budgetStatus.filter(
    ...
    const monitoringResult = await security.secureRealtimeMonitoring(
      supplierId,
      budgetStatus.find((b) => b.featureType === validatedData.featureType)
        ?.currentSpend || 0,
      ...
      return NextResponse.json(
        {
          ...
          message:
            ...
            'Real-time monitoring security check failed',
      ...
      return NextResponse.json(
        {
          ...
          details: {
            alerts: criticalAlerts,
            action:
              'Increase budget or contact support to re-enable AI features',
          },
        },
    ...
    const aiRequest = {
      ...
      securityContext: {
        ...
        sessionId: createHash('md5')
          .update(`${supplierId}-${clientIp}-${Date.now()}`)
          .digest('hex')
          .substring(0, 16),
        isWeddingSeason,
    ...
    const optimizedRequest =
      await optimizationEngine.optimizeAIRequest(aiRequest);
        ...
        return NextResponse.json(
          {
            ...
            message:
              'Cost calculation validation failed - possible tampering detected',
    ...
    await security.logOptimizationEvent(
      ...
      undefined,
      {
        ...
        savingsPercent: optimizedRequest.potentialSavings
          ? Math.round(
              (optimizedRequest.potentialSavings /
                estimateBaseCost(aiRequest)) *
                100,
            )
          : 0,
        cacheStrategy: optimizedRequest.cacheStrategy,
    ...
    const response = {
      ...
      budget: {
        dailySpend:
          budgetStatus.find((b) => b.featureType === validatedData.featureType)
            ?.currentSpend || 0,
        dailyLimit: supplier.daily_budget_pounds || 5.0,
        remainingToday: Math.max(
          0,
          (supplier.daily_budget_pounds || 5.0) -
            (budgetStatus.find(
              (b) => b.featureType === validatedData.featureType,
            )?.currentSpend || 0),
        ),
        utilizationPercent:
          budgetStatus.find((b) => b.featureType === validatedData.featureType)
        ...
        return NextResponse.json(
          {
            ...
            message:
              'AI optimization temporarily disabled due to budget limits',
...
async function logOptimizationMetrics(
  ...
  try {
    ...
    await supabase.from('ai_optimization_audit_log').insert({
...
function estimateBaseCost(aiRequest: any): number {
  ...
  const outputTokens =
    aiRequest.maxTokens || Math.min(1000, estimatedTokens * 2);
...
function generateWeddingIndustryTip(
  featureType: string,
  optimizedRequest: any,
): string {
  const tips = {
    photo_ai:
      'Security Pro tip: Your photo processing costs are encrypted and validated to prevent billing errors during peak wedding season!',
    venue_descriptions:
      'Smart Security: Similar venue descriptions are cached securely with digital signatures to prevent tampering.',
    content_generation:
      'Wedding Security Wisdom: Client-facing content uses high-security GPT-4, while internal tasks use secure GPT-3.5 optimization.',
    chatbot:
      'Security boost: Common wedding questions are cached with integrity checks, reducing costs while maintaining security.',
    menu_optimization:
      'Cost & Security saver: Seasonal menu patterns are encrypted and cached across similar venues and dietary requirements.',
    timeline_assistance:
      'Peak season security tip: Wedding timeline templates are heavily cached with digital signatures during March-October rush!',
  };

  return (
    tips[featureType as keyof typeof tips] ||
    'Secure optimization applied successfully with comprehensive audit logging!'
  );
}
...
function generateNextSteps(
  ...
  const steps = [
    'Your AI request has been optimized with enterprise-grade security',
  ];
  ...
  if (optimizedRequest.processingMode === 'batch') {
    steps.push(
      'Request scheduled for secure batch processing to maximize savings',
    );
  }

  if (optimizedRequest.potentialSavings > 0.01) {
    steps.push(
      `You're saving £${optimizedRequest.potentialSavings.toFixed(4)} with encrypted cost tracking`,
    );
  }

  steps.push('All optimization decisions logged securely for audit compliance');
  steps.push(
    'Continue using AI features - security and optimization are automatic!',
  );
...
export const POST = withAICostSecurity(DEFAULT_AI_COST_SECURITY)(
  secureOptimizeHandler,
);
