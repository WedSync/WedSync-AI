...
function validateWebhookRequest(
  body: VerificationWebhookRequest,
): WebhookError[] {
  ...
  if (!body.status) {
    ...
    !['completed', 'failed', 'requires_action'].includes(body.status)
  ) {
    errors.push({
      code: 'INVALID_VALUE',
      message: 'status must be completed, failed, or requires_action',
      field: 'status',
    });
  }
  ...
  if (!body.timestamp) {
    ...
    if (timeDiff > 5 * 60 * 1000) {
      errors.push({
        ...
        message: 'timestamp is too old or too far in future',
        field: 'timestamp',
      });
    }
  }
...
async function verifyWebhookSignature(
  ...
  try {
    ...
    if (error || !config?.secret_key) {
      logger.warn('Webhook secret not found for service', {
        serviceId: body.service_id,
      });
      return false;
    }
...
): 'success' | 'failure' | 'requires_action' {
