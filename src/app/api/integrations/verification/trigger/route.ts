...
import {
  ...
  VerificationType,
} from '@/lib/integrations/verification/verification-orchestrator';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingWorkflow = await checkExistingVerification(
      body.supplier_id,
    ...
    const nextSteps = determineInitialNextSteps(
      verificationType,
      body.documents,
    );
  ...
  try {
    ...
    const verificationTypes = [
      ...
      },
      {
        ...
        required_documents: ['insurance_certificate', 'policy_schedule'],
      ...
      },
      {
        ...
        required_documents: ['dbs_certificate', 'identity_documents'],
...
function validateTriggerRequest(
  body: VerificationTriggerRequest,
  ...
  if (body.documents) {
    body.documents.forEach((doc, index) => {
      if (!doc.type) {
        errors.push({
          code: 'MISSING_FIELD',
          message: `Document ${index + 1} type is required`,
      ...
      if (!doc.url) {
        errors.push({
          code: 'MISSING_FIELD',
          message: `Document ${index + 1} URL is required`,
...
async function validateSupplier(
  supplierId: string,
): Promise<{ valid: boolean; supplier?: any }> {
  try {
...
async function logVerificationTrigger(
  request: VerificationTriggerRequest,
  workflowId: string,
): Promise<void> {
  try {
    const supabase = createClient();
    await supabase.from('verification_trigger_logs').insert({
      id: crypto.randomUUID(),
