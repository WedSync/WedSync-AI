...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: deliveryStats } = await supabase.rpc(
    ...
    const { data: channelActivity } = await supabase.rpc(
    ...
    const healthData = {
      ...
      systemStatuses: (systemStatuses || []).map((system) => ({
        ...
        uptime: calculateUptime(
          system.created_at,
          system.last_successful_delivery,
        ),
      })),
...
async function performSystemHealthTest(
  ...
  try {
    ...
    if (system.config?.webhookUrl) {
      ...
      if (lastDelivery) {
        ...
        const hoursSinceLastDelivery =
          (Date.now() - lastDeliveryTime.getTime()) / (1000 * 60 * 60);
    ...
    await supabase.from('integration_health_tests').insert({
      system_id: systemId,
