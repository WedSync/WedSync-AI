...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const includeCelebrityData =
      searchParams.get('include_celebrity_data') === 'true';
    ...
    const [
      ...
      getIncidentStatus(
        ...
        startTime,
        endTime,
        includeCelebrityData,
      ),
      getVendorSecurityStatus(
        validatedRequest.organization_id,
        supabase,
        startTime,
        endTime,
    ...
    const response = NextResponse.json({
      ...
      metadata: {
        ...
        api_version: '1.0',
        security_level: validatedRequest.include_celebrity_data
          ? 'celebrity'
          : 'standard',
      },
    });
...
async function getSystemHealthStatus(
  ...
  metricsService: SecurityMetrics,
) {
  const healthMetrics = await metricsService.getSystemHealthMetrics();

  return {
    overall_status: healthMetrics.systemHealth as
      | 'healthy'
      | 'warning'
      | 'critical'
      | 'degraded',
...
async function getThreatIntelligenceStatus(
  ...
  const celebrityThreats = activeThreats.filter(
    (t) => t.celebrity_client,
  ).length;
...
async function getIncidentStatus(
  ...
  const celebrityIncidents = includeCelebrity
    ? incidents?.filter((i) => i.celebrity_client).length || 0
    : 0;
...
async function getVendorSecurityStatus(
  organizationId: string,
  supabase: any,
  startTime: string,
  endTime: string,
  ...
  const activeVendorSessions = new Set(
    vendorActivity
      ?.filter((a) => a.event_type === 'vendor_session_start')
      .map((a) => a.vendor_id) || [],
  ).size;

  const vendorViolations =
    vendorActivity?.filter((a) =>
  ...
  const celebrityApprovedVendors =
    vendors?.filter((v) => v.celebrity_access_approved).length || 0;

  const recentVendorActivity =
    vendorActivity?.slice(0, 5).map((activity) => ({
      vendor_id: activity.vendor_id,
      activity_type: activity.event_type,
      timestamp: activity.created_at,
      risk_level:
        activity.severity === 'critical'
          ? ('critical' as ThreatLevel)
...
async function getCelebrityProtectionStatus(
  organizationId: string,
  supabase: any,
  timeWindow: string,
) {
  ...
  const celebrityThreats =
    celebrityEvents?.filter((e) =>
      ['threat_detected', 'unauthorized_access', 'privacy_violation'].includes(
        e.event_type,
      ),
    ).length || 0;

  const privacyViolations =
    celebrityEvents?.filter((e) => e.event_type === 'privacy_violation')
      .length || 0;

  return {
    celebrity_clients_monitored: celebrityClients?.length || 0,
    enhanced_monitoring_active:
      celebrityClients?.filter((c) => c.celebrity_tier === 'celebrity')
        .length || 0,
...
async function getComplianceOverview(organizationId: string, supabase: any) {
  ...
  const gdprViolations =
    violations?.filter((v) => v.regulation === 'gdpr').length || 0;
  const soc2Violations =
    violations?.filter((v) => v.regulation === 'soc2').length || 0;
  const ccpaViolations =
    violations?.filter((v) => v.regulation === 'ccpa').length || 0;
  ...
  return {
    ...
    next_compliance_review: new Date(
  ...
  return {
    ...
    database_performance: {
      ...
      connection_pool_usage:
        performanceData.databasePerformance?.connectionPoolUsage || 65,
    ...
    security_processing: {
      ...
      encryption_overhead:
        performanceData.securityProcessing?.encryptionOverhead || 5,
...
async function generateSecurityRecommendations(
  ...
  if (systemHealth.overall_status === 'critical') {
    recommendations.push({
      ...
      description:
        'System health is in critical state requiring immediate attention',
      action_required:
        'Investigate and resolve system health issues immediately',
  ...
  if (threatIntelligence.celebrity_specific_threats > 0) {
    recommendations.push({
      ...
      description:
        'Enhanced protection measures may be required for celebrity clients',
