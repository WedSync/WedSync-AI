...
export const POST = withSecureValidation(
  ...
  async (request: NextRequest, validatedData) => {
    ...
    try {
      ...
      const userOwnedChanges = await validateUserOwnership(
        userId,
        validatedData.changes,
      );
      if (!userOwnedChanges) {
        return NextResponse.json(
          { error: 'Forbidden - invalid data ownership' },
          { status: 403 },
        );
      }
      ...
      const result = await processSyncBatchTransaction(
        userId,
        validatedData,
        sessionId,
      );
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: queueStatus } = await supabaseMcp.callFunction(
    ...
    const serverChanges = lastSync
      ? await getServerChangesSince(userId, lastSync)
      : [];
 ...
 */
async function processSyncBatchTransaction(
  userId: string,
  syncData: any,
  sessionId: string,
) {
  ...
  // Start transaction
  const { data: transactionResult } = await supabaseMcp.callFunction(
  ...
  try {
    ...
    if (syncData.lastSyncTime) {
      result.serverChanges = await getServerChangesSince(
        userId,
...
async function detectConflict(userId: string, change: any) {
  try {
    const { data: conflictResult } = await supabaseMcp.callFunction(
  ...
  try {
    switch (strategy) {
      ...
      case 'merge':
        ...
        const mergedData = mergeConflictData(
          change.data,
