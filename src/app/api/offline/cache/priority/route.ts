...
const CachePrioritySchema = z.object({
  resources: z.array(
    z.object({
      ...
      expirationPolicy: z
        .enum(['wedding_day', 'fixed_duration', 'never_expire', 'access_based'])
        .optional(),
  ...
  async (request: NextRequest, validatedData) => {
    ...
    try {
      ...
      const weddingDayBoost = validatedData.weddingDate
        ? calculateWeddingDayBoost(
            validatedData.weddingDate,
            validatedData.globalSettings?.weddingDayBoostFactor,
          )
        : 0;
      ...
      for (const resource of validatedData.resources) {
        try {
          ...
          const basePriority =
            PRIORITY_LEVELS[
              resource.priority.toUpperCase() as keyof typeof PRIORITY_LEVELS
            ];
      ...
      if (validatedData.globalSettings) {
        await updateGlobalCacheSettings(
          userId,
          validatedData.globalSettings,
          validatedData.deviceInfo?.deviceId,
        );
      }

      // Calculate cache storage summary
      const storageSummary = await calculateCacheStorageSummary(
        userId,
        validatedData.deviceInfo?.deviceId,
      );
 ...
 * Calculate wedding day proximity boost
 */
function calculateWeddingDayBoost(
  weddingDate: string,
...
async function setCacheResourcePriority(userId: string, options: any) {
  ...
  const { data: result } = await supabaseMcp.callFunction(
    'set_cache_priority',
    {
      p_user_id: userId,
 ...
 */
async function updateGlobalCacheSettings(
  userId: string,
  settings: any,
  deviceId?: string,
) {
 ...
 */
async function evaluateCacheCleanup(
  userId: string,
  storageSummary: any,
