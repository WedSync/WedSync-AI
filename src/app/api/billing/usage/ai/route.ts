...
export async function POST(request: NextRequest) {
  try {
    ...
    const subscriptionDetails = await supabase.rpc(
    ...
    const accessCheck = await supabase.rpc('check_feature_access', {
      p_user_id: targetUserId,
      p_feature_key: featureKey,
      p_current_usage: getCurrentUsageForService(
        subscriptionDetails.data?.usage,
        service,
      ),
    });
    ...
    const currentServiceUsage = getCurrentUsageForService(
      updatedUsage,
      service,
    );
    ...
    if (accessCheck.data?.usageLimit && accessCheck.data.usageLimit > 0) {
      const remaining = Math.max(
        0,
        accessCheck.data.usageLimit - currentServiceUsage,
      );
      response.headers.set(
        'X-RateLimit-Limit',
        accessCheck.data.usageLimit.toString(),
      );
  ...
  try {
    ...
    const aiUsage = {
      ...
      music_recommendations: {
        ...
        feature_key: getFeatureKeyForService(
          'music_recommendations',
          plan.tier,
        ),
      },
    ...
    return NextResponse.json({
      ...
      plan: plan.tier,
      ai_services: aiUsage,
      total_ai_usage: Object.values(aiUsage).reduce(
...
function getFeatureKeyForService(service: string, userPlan?: string): string {
  const serviceMap = {
    ...
    music_recommendations:
      userPlan === 'premium' || userPlan === 'enterprise'
        ? 'ai:music_unlimited'
        : 'ai:music_recommendations',
    floral_suggestions:
      userPlan === 'premium' || userPlan === 'enterprise'
        ? 'ai:floral_unlimited'
        : 'ai:floral_suggestions',
    faq_extraction: 'ai:faq_extraction',
    chatbot_interactions:
      userPlan === 'enterprise' ? 'ai:chatbot_unlimited' : 'ai:chatbot',
  };
...
function calculateUsageWarnings(
  current: number,
  limit: number,
  plan: string,
): string[] {
