...
function initializeWebSocketServer() {
  if (!wss) {
    ...
    console.log(
      `WebSocket server started on port ${process.env.WEBSOCKET_PORT || '3001'}`,
    );
  }
      ...
      case 'disconnect_user':
        ...
        if (!data.userId) {
          return NextResponse.json(
            {
              success: false,
              error: 'User ID is required for disconnect action',
            },
            { status: 400 },
          );
        }
...
export async function OPTIONS(request: NextRequest) {
  const { wss: wsServer, collaborationHandler: handler } =
