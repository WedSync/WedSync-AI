...
async function getRealTimeStats(
  ...
  recentLogs.forEach((log) => {
    eventTypeDistribution[log.event_type] =
      (eventTypeDistribution[log.event_type] || 0) + 1;
  });
  ...
  const failedAuthEvents = recentLogs.filter(
    (log) =>
  ...
  return {
    summary: {
      ...
      unique_users: new Set(
        recentLogs.map((log) => log.user_id).filter(Boolean),
      ).size,
      unique_ip_addresses: new Set(
    ...
    security_indicators: {
      ...
      critical_events: recentLogs.filter((log) => log.severity === 'critical')
        .length,
      error_rate:
        recentLogs.length > 0
          ? Math.round(
              (recentLogs.filter((log) => log.severity === 'error').length /
                recentLogs.length) *
                100,
            )
          : 0,
...
async function getSystemHealthMetrics(
  ...
  const errorEvents = systemEvents.filter(
    (event) => event.event_type === AuditEventType.SYSTEM_ERROR_OCCURRED,
  );

  const maintenanceEvents = systemEvents.filter(
    (event) =>
      event.event_type === AuditEventType.SYSTEM_MAINTENANCE_START ||
      event.event_type === AuditEventType.SYSTEM_MAINTENANCE_END,
  );

  const backupEvents = systemEvents.filter(
    (event) => event.event_type === AuditEventType.SYSTEM_BACKUP_CREATED,
  );
  ...
  const uptimePercentage =
    ((totalMinutes - maintenanceMinutes) / totalMinutes) * 100;
  ...
  return {
    ...
    errors: {
      ...
      error_rate_per_hour: Math.round(
        (errorEvents.length / timeWindowMinutes) * 60,
      ),
      critical_errors: errorEvents.filter((e) => e.severity === 'critical')
        .length,
    },
    operations: {
      maintenance_windows: maintenanceEvents.length / 2, // Start/End pairs
      successful_backups: backupEvents.filter((e) => e.severity !== 'error')
        .length,
...
async function getPerformanceMetrics(
  ...
  const eventsWithPerformance = allEvents.filter(
    (event) => event.performance_metrics?.duration_ms != null,
  );

  const durations = eventsWithPerformance.map(
    (event) => event.performance_metrics!.duration_ms!,
  );
  ...
  const p95ResponseTime =
    durations.length > 0 ? calculatePercentile(durations, 95) : 0;

  const p99ResponseTime =
    durations.length > 0 ? calculatePercentile(durations, 99) : 0;
...
async function getActiveAlerts(organizationId: string) {
  ...
  const criticalAlerts =
    alerts?.filter(
      (alert) =>
        alert.details?.severity === 'critical' || alert.type === 'error',
    ) || [];
...
function calculateMaintenanceTime(maintenanceEvents: any[]): number {
  ...
  const startEvents = maintenanceEvents.filter(
    (e) => e.event_type === AuditEventType.SYSTEM_MAINTENANCE_START,
  );

  const endEvents = maintenanceEvents.filter(
    (e) => e.event_type === AuditEventType.SYSTEM_MAINTENANCE_END,
  );
  ...
  startEvents.forEach((startEvent) => {
    ...
    if (correspondingEnd) {
      const durationMs =
        new Date(correspondingEnd.timestamp).getTime() -
