...
const reportQuerySchema = z.object({
  ...
  compliance_standards: z
    .array(z.enum(['GDPR', 'CCPA', 'PCI_DSS', 'SOX', 'HIPAA']))
    .optional(),
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (queryParams.compliance_standards) {
      queryParams.compliance_standards =
        queryParams.compliance_standards.split(',');
    }
...
async function generateAuditReport(
  ...
  switch (query.report_type) {
    ...
    case 'wedding_activity':
      return await generateWeddingActivityReport(
        query,
        startDate,
        endDate,
        supabase,
    ...
    case 'vendor_activity':
      return await generateVendorActivityReport(
        query,
        startDate,
        endDate,
        supabase,
    ...
    case 'user_behavior':
      return await generateUserBehaviorReport(
        query,
        startDate,
        endDate,
        supabase,
...
async function generateComplianceReport(
  ...
  const deletionEvents = dataModifications.filter(
    (e) => e.action === 'delete' && e.resource_type === 'user',
  );
    ...
    for (const event of deletionEvents) {
      ...
      if (associatedDataCheck.data && associatedDataCheck.data.length > 0) {
        violations.push({
          ...
          description:
            'Associated user data still accessible after user deletion',
  ...
  report.summary = {
    ...
    high_severity_violations: violations.filter((v) => v.severity === 'high')
      .length,
...
async function generateSecurityReport(
  ...
  const { data: accessPatterns } = await supabase.rpc(
...
async function generateWeddingActivityReport(
  query: any,
  startDate: Date,
  endDate: Date,
  supabase: any,
  ...
  report.recommendations = generateWeddingRecommendations(
    report,
    query.wedding_id,
    supabase,
  );
...
async function generateRiskAssessmentReport(
  ...
  // User risk profiles
  const { data: userRiskProfiles } = await supabase.rpc(
    'calculate_user_risk_profiles',
    {
      start_date: startDate.toISOString(),
      end_date: endDate.toISOString(),
    },
  ...
  report.summary = {
    ...
    avg_risk_score:
      riskData?.reduce((sum, e) => sum + (e.risk_score || 0), 0) /
      (riskData?.length || 1),
...
function generateWeddingRecommendations(
  report: any,
  weddingId: string,
  supabase: any,
): string[] {
  ...
  if (report.critical_changes.length > 5) {
    recommendations.push(
      'Review change approval workflows for critical wedding data',
    );
...
function generateRiskMitigationStrategies(report: any): string[] {
  ...
  if (report.high_risk_events.length > 10) {
    strategies.push(
      'Enhance monitoring for frequently occurring high-risk event types',
    );
...
function generateRiskRecommendations(report: any): string[] {
  ...
  const totalEvents = Object.values(report.risk_distribution).reduce(
    (a: number, b: any) => a + b,
    0,
  );
...
function calculateComplianceScore(report: any): number {
  ...
  const highSeverityPenalty =
    report.compliance_violations.filter((v: any) => v.severity === 'high')
      .length * 15;
...
function calculateSecurityScore(report: any): number {
  ...
  const criticalEventPenalty =
    report.security_events.filter((e: any) => e.severity === 'critical')
      .length * 15;

  return Math.max(
    0,
    baseScore - failedLoginPenalty - suspiciousPenalty - criticalEventPenalty,
  );
}
...
async function generateVendorActivityReport(
  query: any,
  startDate: Date,
  endDate: Date,
  supabase: any,
...
async function generateUserBehaviorReport(
  query: any,
  startDate: Date,
  endDate: Date,
  supabase: any,
