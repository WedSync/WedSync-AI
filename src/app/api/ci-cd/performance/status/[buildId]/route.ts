...
export async function GET(
  ...
  try {
    ...
    console.log(`✅ Status retrieved for ${buildId}: ${response.status}`);

    return NextResponse.json(response, { status: 200 });
  } catch (error) {
    console.error(
      `❌ Failed to get status for build ${params.buildId}:`,
      error,
    );
...
export async function POST(
  ...
  try {
    ...
    console.log(`📝 Updating status for build: ${buildId}`);
    ...
    console.log(`✅ Status updated for ${buildId}`);
    ...
    console.error(
      `❌ Failed to update status for build ${params.buildId}:`,
      error,
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.log(`🛑 Cancelling validation for build: ${buildId}`);
    ...
    console.log(`✅ Validation cancelled for ${buildId}`);
    ...
    console.error(
      `❌ Failed to cancel validation for build ${params.buildId}:`,
      error,
    );
...
async function findValidationByBuildId(buildId: string): Promise<any> {
  ...
  const mockValidations: Record<string, any> = {
    ...
    build_failed: {
      ...
      recommendation:
        'Optimize database queries and implement caching; Optimize images and implement lazy loading for faster content rendering',
