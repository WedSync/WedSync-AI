...
/**
 * GET /api/cloud/providers/[id]/services
 * List all available services for a cloud provider
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Get provider and list services
    const provider = await orchestrationService.getCloudProvider(providerId);
    let services = await provider.listServices(region);
    ...
    return NextResponse.json({
      ...
      availableCount: services.filter((s) => s.isAvailable).length,
