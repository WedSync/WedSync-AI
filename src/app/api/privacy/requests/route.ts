...
} from '@/lib/compliance/gdpr/data-subject-rights';
...
const privacyRequestSchema = z.object({
  requestType: z.enum(
    Object.values(DATA_SUBJECT_REQUEST_TYPES) as [string, ...string[]],
  ),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResult = await rateLimit.check('privacy_request', request, {
      max: 5,
      window: 3600,
    });
    ...
    return NextResponse.json(
      {
        ...
        message:
          'Privacy request submitted successfully. Please check your email for verification instructions.',
      },
      { status: 201 },
    );
  } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const sanitizedRequests = requests?.map((req) => ({
      ...
      can_download:
        req.status === 'completed' && req.request_type === 'portability',
    }));
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Verify the request
    const isVerified = await dataSubjectRights.verifyRequest(
      requestId,
      verificationToken,
    );
    ...
    if (isVerified) {
      ...
      return NextResponse.json({
        success: true,
        message:
          'Request verified successfully. Processing will begin shortly.',
        status: 'processing',
      });
    } else {
      return NextResponse.json(
