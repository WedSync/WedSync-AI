...
// Helper functions
function validateTemplateSchema(schema: any): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (includeSystem) {
      query = query.or(
        `organization_id.eq.${organizationId},is_system_template.eq.true`,
      );
    } else {
...
export async function replicate(request: NextRequest) {
  try {
    ...
    if (
      sourceTemplate.organization_id !== organization_id &&
      !sourceTemplate.is_system_template
    ) {
      return NextResponse.json(
    ...
    const replicatedTemplate = {
      ...
      template_schema:
        customizations?.template_schema || sourceTemplate.template_schema,
      default_content: {
...
export async function generate(request: NextRequest) {
  try {
    ...
    const requiredFields =
      template.template_schema.fields?.filter((f: any) => f.required) || [];
    ...
    for (const [key, template_content] of Object.entries(
      template.default_content,
    )) {
      if (typeof template_content === 'string') {
        generatedContent[key] = processTemplateVariables(
          template_content,
          variables,
        );
      } else {
    ...
    if (create_content && content_title) {
      const contentData = {
        ...
        rich_content:
          generatedContent.content || generatedContent.rich_content || '',
  ...
  try {
    ...
    if (template.usage_count > 0) {
      ...
      return NextResponse.json({
        success: true,
        message:
          'Template archived successfully (soft delete due to usage history)',
      });
    } else {
