...
export async function POST(request: NextRequest) {
  try {
    ...
    const { content_id, version_note, is_major_version, organization_id } =
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { content_id, action, note, scheduled_at, organization_id } =
      validationResult.data;
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { content_id, target_version, rollback_note, organization_id } =
      validationResult.data;
    ...
    const { data: rollbackVersion, error: rollbackVersionError } =
      await supabase
        .from('content_versions')
