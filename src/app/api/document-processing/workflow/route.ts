  ...
  try {
    ...
    switch (action) {
      case 'start': {
        ...
        const metrics = {
          ...
          accuracyMetrics: {
            ...
            overall:
              workflow.mappingResults?.accuracy ||
              workflow.extractionResults?.accuracy ||
              0,
          },
          performanceMetrics: {
        ...
        console.log(
          `PDF mapping workflow ${action} for document ${validatedData.documentId}:`,
          {
      ...
      case 'resume': {
        ...
        console.log(
          `PDF mapping workflow resumed from ${validatedData.fromStep} for document ${validatedData.documentId}:`,
          {
            status: workflow.status,
            userId: session.user.id,
          },
      ...
      case 'validate': {
        ...
        const validation =
          await integrationService.validateWorkflowCompletion(documentId);
      ...
      case 'export': {
        const { documentId } = WorkflowRequestSchema.parse(body);
        const format =
          (searchParams.get('format') as 'json' | 'csv' | 'pdf') || 'json';

        // Export workflow results
        const exportResult = await integrationService.exportWorkflowResults(
          documentId,
          format,
        );
    ...
    console.log(
      `PDF mapping workflow API call completed in ${processingTime}ms`,
    );
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    switch (action) {
      case 'status': {
        // Get workflow status
        const workflow = await integrationService.getWorkflowStatus(
        ...
        if (validatedQuery.includeDetails) {
          const validation =
            await integrationService.validateWorkflowCompletion(
              validatedQuery.documentId,
            );
          ...
          detailedMetrics = {
            ...
            fieldsBreakdown: {
              ...
              highConfidenceCount:
                workflow.mappingResults?.mappings?.filter(
                  (m) => m.confidence >= 0.85,
                ).length || 0,
            },
          };
