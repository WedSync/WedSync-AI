  ...
  try {
    ...
    for (const correction of validatedData.corrections) {
      try {
        ...
        console.error(
          `Failed to process correction ${correction.mappingId}:`,
          correctionError,
        );
    ...
    const successfulCorrections = processedCorrections.filter(
      (c) => c.status === 'success',
    );
    const failedCorrections = processedCorrections.filter(
      (c) => c.status === 'error',
    );
    ...
    const learningMetrics = {
      ...
      successRate:
        processedCorrections.length > 0
          ? successfulCorrections.length / processedCorrections.length
          : 0,
      averageLearningImpact: calculateAverageLearningImpact(
    ...
    console.log(
      `Correction session completed for user ${session.user.id}:`,
      learningMetrics,
    );
...
async function calculateLearningImpact(
  ...
  try {
    ...
    const { data: currentPattern } = await supabase
      ...
      .eq(
        'pattern_key',
        `${correction.sourceFieldId}_${correction.correctTargetFieldId}`,
      )
      .single();
    ...
    const newConfidence =
      (oldConfidence * currentPattern.sample_size + feedbackValue) /
...
async function saveCorrectionDetails(
  ...
  try {
    await supabase.from('mapping_correction_details').insert({
...
function calculateAverageLearningImpact(corrections: any[]): {
  ...
  for (const correction of corrections) {
    if (correction.learningImpact) {
      totalConfidenceAdjustment += Math.abs(
        correction.learningImpact.confidenceAdjustment,
      );
...
async function updateUserContributionScore(
  ...
  try {
    ...
    const newTotalCorrections =
      (currentScore?.total_corrections || 0) + correctionsCount;
    const contributionPoints = correctionsCount * 10; // 10 points per correction
    const newContributionScore =
      (currentScore?.contribution_score || 0) + contributionPoints;

    await supabase.from('user_mapping_contributions').upsert(
      {
        user_id: userId,
