    ...
    const testsWithStats =
      tests?.map((test) => {
        if (test.status === 'running') {
          ...
          const zScore =
            standardError > 0 ? (testRate - controlRate) / standardError : 0;
          ...
          return {
            ...
            isSignificant,
            hasEnoughData:
              test.control_visitors >= test.minimum_sample_size &&
  ...
  try {
    ...
    return NextResponse.json({
      ...
      estimatedCompletionDate: new Date(
        Date.now() + estimatedCompletionDays * 24 * 60 * 60 * 1000,
      ).toISOString(),
