  ...
  try {
    ...
    const nextMilestone = tierInfo.next_tier
      ? {
          ...
          progress_percentage:
            tierInfo.next_tier.requirements.progress_percentage,
        }
      : undefined;
      ...
      creator_info: {
        ...
        payout_status: {
          recent_payouts: payoutStatus.recent_payouts.map((payout) => ({
            ...
            gross_earnings:
              Math.round((payout.gross_earnings_cents / 100) * 100) / 100,
            net_payout: Math.round((payout.net_payout_cents / 100) * 100) / 100,
          })),
          pending_amount:
            Math.round((payoutStatus.pending_amount_cents / 100) * 100) / 100,
          next_payout_date: payoutStatus.next_payout_date,
          total_lifetime_earnings:
            Math.round(
              (payoutStatus.total_lifetime_earnings_cents / 100) * 100,
            ) / 100,
        },
        earnings_summary: {
          total_lifetime_earnings_cents:
            payoutStatus.total_lifetime_earnings_cents,
          current_month_earnings_cents:
            earningsSummary.current_month_earnings_cents,
        ...
        recent_transactions: recentTransactions.map((transaction) => ({
          ...
          gross_amount:
            Math.round((transaction.gross_sale_amount_cents / 100) * 100) / 100,
          creator_earnings:
            Math.round((transaction.creator_earnings_cents / 100) * 100) / 100,
          commission_amount:
            Math.round((transaction.commission_amount_cents / 100) * 100) / 100,
        })),
  ...
  try {
    ...
    switch (action) {
      case 'manual_tier_upgrade':
        const result = await performManualTierUpgrade(
          creatorId,
          data.target_tier,
          authCheck.userId,
        );
...
async function getEarningsSummary(creatorId: string): Promise<{
  ...
  try {
    ...
    const currentMonthEarnings = (monthlyData || []).reduce(
    ...
    const totalRevenue = (allTimeData || []).reduce(
      (sum, record) => sum + (record.gross_sale_amount_cents || 0),
      0,
    );
...
async function getCommissionHistory(
  ...
  try {
    ...
    const monthlyData: Record<
      string,
      { earnings: number; commission: number; sales: number }
    > = {};
...
async function performManualTierUpgrade(
  ...
  try {
    ...
    await supabase.from('marketplace_tier_upgrade_log').insert({
      creator_id: creatorId,
...
async function logCreatorApiAccess(
  creatorId: string,
  userId: string,
): Promise<void> {
  try {
    const { error } = await supabase.from('marketplace_api_access_log').insert({
      creator_id: creatorId,
      accessed_by: userId,
