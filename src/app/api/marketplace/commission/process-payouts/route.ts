...
function validatePayoutRequest(body: any): { valid: boolean; error?: string } {
  ...
  if (body.period_start && body.period_end) {
    ...
    if (end > new Date()) {
      return {
        valid: false,
        error: 'Cannot process payouts for future periods',
      };
    }
  }
  ...
  if (
    body.creator_ids &&
    (!Array.isArray(body.creator_ids) || body.creator_ids.length === 0)
  ) {
    return {
      valid: false,
      error: 'creator_ids must be a non-empty array when provided',
    };
  }
  ...
  if (body.creator_ids && body.creator_ids.length > 100) {
    return {
      valid: false,
      error: 'Cannot process more than 100 creators at once',
    };
  }
...
async function performDryRun(
  ...
  try {
    ...
    for (const record of commissionData || []) {
      ...
      creatorTotals.set(
        creatorId,
        (creatorTotals.get(creatorId) || 0) + earnings,
      );
    }

    // Filter by minimum threshold
    const MINIMUM_THRESHOLD = 2000; // £20 in pence
    const eligibleCreators = Array.from(creatorTotals.entries()).filter(
      ([_, amount]) => amount >= MINIMUM_THRESHOLD,
    );

    const totalAmount = eligibleCreators.reduce(
...
async function logPayoutProcessingEvent(
  eventType: string,
  eventData: any,
): Promise<void> {
  try {
