...
async function generateSuggestions(
  ...
  try {
    ...
    const templateSuggestions = await getTemplateTitleSuggestions(
      query,
    ...
    const historySuggestions = await getSearchHistorySuggestions(
      query,
      Math.ceil(maxSuggestions * 0.2),
      userId,
    );
...
async function getTemplateTitleSuggestions(
  query: string,
  limit: number,
): Promise<SuggestionItem[]> {
  try {
...
async function getTagSuggestions(
  ...
  try {
    ...
    if (error) {
      ...
      const tagCounts: {
        [key: string]: { count: number; popularity: number; category?: string };
      } = {};
  ...
  try {
    ...
    if (userId) {
      ...
      if (!userError && userHistory && userHistory.length > 0) {
        const userSuggestions = userHistory.map(
        ...
        if (remainingSlots > 0) {
          ...
          if (!popularError && popularHistory) {
            const popularSuggestions = popularHistory.map(
              (search): SuggestionItem => ({
                text: search.search_query,
                type: 'search_history',
                result_count: search.result_count,
...
function deduplicateAndSort(
  ...
  return unique.sort((a, b) => {
    ...
    const aScore =
      aExact + aStartsWith + aTypePriority + Math.min(aPopularity, 100);
    const bScore =
      bExact + bStartsWith + bTypePriority + Math.min(bPopularity, 100);
