...
async function executeMarketplaceSearch(params: SearchParams): Promise<{
  ...
  try {
    ...
    if (
      params.filters?.weddingTypes &&
      params.filters.weddingTypes.length > 0
    ) {
    ...
    const enhancedTemplates =
      params.query && params.query.length > 0
        ? data.map((template) =>
            enhanceWithHighlighting(template, params.query!),
          )
        : data;
...
function enhanceWithHighlighting(
  template: any,
  query: string,
): MarketplaceTemplate {
...
async function getFacetCounts(params: SearchParams): Promise<object> {
  try {
    ...
    let baseQuery = supabase
      .from('marketplace_templates')
      .select(
        'category, minimum_tier, price_cents, average_rating, tags, target_wedding_types',
      )
      .eq('status', 'active');
    ...
    baseResults.forEach((template: any) => {
      ...
      // Tier facets
      facets.tiers[template.minimum_tier] =
        (facets.tiers[template.minimum_tier] || 0) + 1;
...
async function recordSearchAnalytics(data: {
  ...
  try {
    await supabase.from('marketplace_search_analytics').insert({
      user_id: data.userId,
