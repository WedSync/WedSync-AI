...
export async function POST(request: NextRequest) {
  try {
    ...
    const currentResultCount = await getCurrentResultCount(
      body.query,
      body.filters,
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to retrieve saved searches. Please try again.',
      },
      { status: 500 },
    );
  }
...
function validateSaveSearchRequest(body: SaveSearchRequest): {
  ...
  if (
    body.filters.priceMin !== undefined &&
  ...
  if (
    body.filters.priceMax !== undefined &&
  ...
  if (
    body.filters.ratingMin !== undefined &&
    (typeof body.filters.ratingMin !== 'number' ||
      body.filters.ratingMin < 0 ||
      body.filters.ratingMin > 5)
  ) {
...
async function getCurrentResultCount(
  query: string,
  filters: SavedSearchFilters,
): Promise<number> {
  try {
