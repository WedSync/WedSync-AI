...
export async function GET(request: NextRequest) {
  try {
    ...
    const analytics =
      await financialDataProcessingService.generateFinancialAnalytics(
        periodStart,
        periodEnd,
      );
    ...
    const enhancedCreators = await enhanceCreatorData(
      analytics.top_performing_creators,
    );
    ...
    if (includeIntegrity) {
      dataIntegrity =
        await financialDataProcessingService.validateFinancialDataIntegrity(
          periodStart,
          periodEnd,
        );
    }
