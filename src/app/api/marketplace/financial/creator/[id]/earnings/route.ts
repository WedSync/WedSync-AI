...
export async function GET(
  ...
  try {
    ...
    const dailyHistory =
      await financialDataProcessingService.getCreatorEarningsHistory(
        creatorId,
        startDate,
        endDate,
      );
    ...
    const response: CreatorEarningsResponse = {
      ...
      earnings_data: {
        ...
        summary: {
          ...
          paid_earnings_cents:
            (summary.total_earnings_cents || 0) -
            (summary.pending_earnings_cents || 0),
          ...
          current_month_earnings_cents:
            summary.current_month_earnings_cents || 0,
...
async function verifyCreatorAccess(
  ...
  try {
    ...
    const isAdmin = ['admin', 'super_admin', 'finance_admin'].includes(
      profile.role,
    );
...
async function getMonthlyBreakdown(
  ...
  try {
    ...
    if (endDate) {
      ...
      query = query.or(
        `year.lt.${endYear},and(year.eq.${endYear},month.lte.${endMonth})`,
      );
    }
...
async function getPayoutHistory(
  ...
  try {
    let query = supabase
      ...
      .select(
        'payout_date, net_payout_cents, status, payout_method, stripe_transfer_id',
      )
      .eq('creator_id', creatorId)
