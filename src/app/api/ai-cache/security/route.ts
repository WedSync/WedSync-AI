...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `Security ${action} requested by user ${user.id}, org ${profile.organization_id}`,
    );
    ...
    switch (action) {
      ...
      case 'metrics':
        responseData = await getSecurityMetrics(
          supabase,
      ...
      case 'violations':
        ...
        responseData = await getSecurityViolations(
          supabase,
          profile.organization_id,
          hoursBack,
        );
      ...
      case 'audit-logs':
        ...
        responseData = await getAuditLogs(
          supabase,
          profile.organization_id,
          auditLimit,
        );
      ...
      case 'gdpr-report':
        responseData = await securityManager.generateGDPRReport(
          profile.organization_id,
        );
        break;

      case 'retention-status':
        responseData = await getRetentionStatus(
          supabase,
          profile.organization_id,
        );
        break;
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid action. Supported: health, metrics, violations, audit-logs, gdpr-report, retention-status',
          },
          { status: 400 },
        );
    }
  ...
  try {
    ...
    if (!success) {
      return NextResponse.json(
        {
          error:
            'Too many security modification requests. Please try again later.',
        },
        {
          status: 429,
          headers: {
    ...
    console.log(
      `Security ${action} initiated by user ${user.id}, org ${profile?.organization_id}`,
    );
    ...
    switch (action) {
          ...
          return NextResponse.json(
            {
              ...
              details: auditValidation.error.errors,
            },
            { status: 400 },
          );
        }
      ...
      case 'delete-user-data':
          ...
          return NextResponse.json(
            {
              ...
              details: deletionValidation.error.errors,
            },
            { status: 400 },
          );
        }
        ...
        if (
          deletionValidation.data.confirmationToken !==
          `DELETE_${deletionValidation.data.userId}_${Date.now().toString().slice(-6)}`
        ) {
      ...
      case 'resolve-violation':
        ...
        if (updateError) {
          throw new Error(
            `Failed to resolve violation: ${updateError.message}`,
          );
        }
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid action. Supported: log-audit-event, delete-user-data, enforce-retention, resolve-violation',
          },
          { status: 400 },
        );
    }
...
async function getSecurityViolations(
  supabase: any,
  organizationId: string,
  hoursBack: number,
): Promise<any> {
