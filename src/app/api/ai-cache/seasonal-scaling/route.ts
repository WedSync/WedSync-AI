...
import {
  SeasonalScalingAutomator,
  ScalingAction,
  ScalingMetrics,
} from '@/lib/ai-cache/SeasonalScalingAutomator';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `Seasonal scaling ${action} requested by user ${user.id}, org ${profile.organization_id}`,
    );
    ...
    switch (action) {
      ...
      case 'predictions':
        ...
        responseData =
          await scalingAutomator.predictSeasonalTransition(lookaheadDays);
        break;
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid action. Supported: status, predictions, health, metrics',
          },
          { status: 400 },
        );
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const identifier = request.headers.get('x-forwarded-for') || 'anonymous';
    const { success, limit, remaining, reset } = await ratelimit.limit(
      `seasonal_scaling_post:${identifier}`,
    );
    ...
    console.log(
      `Manual scaling ${action} initiated by user ${user.id}, org ${profile.organization_id}, reason: ${reason || 'not specified'}`,
    );
    ...
    switch (action) {
      case 'execute':
        ...
        if (!force) {
          const currentSeason = scalingAutomator.getCurrentSeason();
          if (currentSeason.name === 'peak' && new Date().getDay() === 6) {
            // Saturday during peak season
            return NextResponse.json(
              {
                error:
                  'Scaling during Saturday peak season requires force=true parameter',
...
async function getPerformanceImpact(supabase: any): Promise<{
  ...
  const avgHitRate =
    recentStats.reduce((sum, stat) => sum + (stat.cache_hit_rate || 0), 0) /
    recentStats.length;
  const avgResponseTime =
    recentStats.reduce((sum, stat) => sum + (stat.avg_response_time || 0), 0) /
    recentStats.length;
  const totalCostSavings = recentStats.reduce(
    (sum, stat) => sum + (stat.cost_savings || 0),
    0,
  );
