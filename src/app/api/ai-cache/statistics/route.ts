...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `Cache statistics request: timeRange=${timeRange}h, groupBy=${groupBy}, user=${user.id}`,
    );
    ...
    const [
      ...
      getOverallCacheStatistics(
        supabase,
        profile.organization_id,
        timeRange,
        cacheType,
      ),
      getCacheHitRateStatistics(
        supabase,
        profile.organization_id,
        timeRange,
        groupBy,
      ),
      getLocationBasedStatistics(
        supabase,
        profile.organization_id,
        timeRange,
        location,
    ...
    if (includeDetails) {
      const detailedData = await getDetailedStatistics(
        supabase,
        profile.organization_id,
        timeRange,
      );
  ...
  try {
    ...
    switch (reportType) {
      case 'performance_analysis':
        report = await generatePerformanceAnalysisReport(
          supabase,
          profile.organization_id,
          dateRange,
          filters,
      ...
      case 'cost_optimization':
        report = await generateCostOptimizationReport(
          supabase,
          profile.organization_id,
          dateRange,
        );
      ...
      case 'capacity_planning':
        report = await generateCapacityPlanningReport(
          supabase,
          profile.organization_id,
          dateRange,
        );
      ...
      case 'regional_analysis':
        report = await generateRegionalAnalysisReport(
          supabase,
          profile.organization_id,
          dateRange,
        );
      ...
      default:
        return NextResponse.json(
          {
            error:
              'Invalid report type. Available types: performance_analysis, cost_optimization, capacity_planning, regional_analysis',
          },
          { status: 400 },
        );
    }
    ...
    if (includeRecommendations) {
      report.recommendations = await generateOptimizationRecommendations(
        supabase,
        profile.organization_id,
      );
    }
    ...
    if (includeProjections) {
      report.projections = await generateUsageProjections(
        supabase,
        profile.organization_id,
      );
    }
...
async function getLocationBasedStatistics(
  ...
  try {
    ...
    if (specificLocation) {
      locationMetrics =
        await locationPartitioner.getMarketPerformanceMetrics(season);
      locationMetrics = locationMetrics.filter((m) =>
        m.marketKey.includes(specificLocation),
      );
    } else {
      locationMetrics =
        await locationPartitioner.getMarketPerformanceMetrics(season);
...
async function getVendorStatistics(
  ...
  try {
    const vendorPerformance = await vendorOptimizer.getVendorCachePerformance(
      undefined,
      timeRangeHours,
    );
...
async function getCostAnalysis(
  ...
  const totalSaved = costData.reduce(
    (sum: number, record: any) => sum + (record.cost_saved_cents || 0),
    0,
  );
...
async function generatePerformanceAnalysisReport(
  ...
  return {
    summary:
      'Performance analysis shows good cache efficiency with room for optimization',
    keyMetrics: {
      avgHitRate: 0.78,
      avgResponseTime: 180,
...
async function generateUsageProjections(supabase: any, organizationId: string) {
  return {
