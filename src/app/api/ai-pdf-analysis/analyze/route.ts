...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Processing AI PDF analysis for user ${body.user_id} (${body.user_tier})`,
    );
    console.log(
      `Pages: ${body.pages.length}, Urgency: ${analysisRequest.urgency_level}`,
    );

    // Process with AI service
    const result: ProcessingResult =
      await aiPDFAnalysisService.analyzePDF(analysisRequest);
    ...
    console.log(`- Cost incurred: £${result.cost_incurred}`);
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        confidence_summary: getConfidenceSummary(
