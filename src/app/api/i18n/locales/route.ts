...
const deleteLocaleSchema = z
  ...
  .refine((data) => data.id || (data.language_code && data.region_code), {
...
async function getTranslationStats(supabase: any, localeCode: string) {
  try {
    ...
    const completionPercentage = totalKeys
      ? Math.round(((translatedKeys || 0) / totalKeys) * 100)
      : 0;
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Created locale configuration ${localeCode} by user ${user.id}`,
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { id, language_code, region_code, ...updateData } =
    ...
    console.log(
      `Updated locale configuration ${enrichedLocale.locale_code} by user ${user.id}`,
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { id, language_code, region_code, force_delete } =
      validationResult.data;
    ...
    if (force_delete) {
      ...
      console.log(
        `Deactivated locale configuration ${enrichedLocale.locale_code} by user ${user.id}`,
      );
