...
class AnalyticsPerformanceMonitor {
  ...
  getQueryPerformanceMetrics(timeframe: string): QueryPerformanceMetric[] {
    ...
    return Object.entries(groupedMetrics).map(
      ([queryType, metrics]: [string, any[]]) => {
        const durations = metrics
          .map((m) => m.duration_ms)
          .sort((a, b) => a - b);
        const errors = metrics.filter((m) => !m.success);
        const dataProcessed = metrics.reduce(
          (sum, m) => sum + m.data_size_mb,
          0,
        );
  ...
  getCacheEfficiencyMetrics(timeframe: string): CacheEfficiencyMetric[] {
    ...
    const recentMetrics = cacheMetrics.filter(
      (m) => new Date(m.timestamp).getTime() > cutoffTime,
    );
    ...
    return Object.entries(groupedMetrics).map(
      ([cacheType, metrics]: [string, any[]]) => {
        const hits = metrics.filter((m) => m.hit);
        const misses = metrics.filter((m) => !m.hit);

        const avgCachedTime =
          hits.length > 0
            ? hits.reduce((sum, m) => sum + m.response_time_ms, 0) / hits.length
            : 0;

        const avgUncachedTime =
          misses.length > 0
            ? misses.reduce((sum, m) => sum + m.response_time_ms, 0) /
              misses.length
            : 0;
  ...
  getSystemHealth(): {
    ...
    const last5Minutes = recentQueries.filter(
      (m) => new Date(m.timestamp).getTime() > Date.now() - 5 * 60 * 1000,
    );

    const avgDuration =
      last5Minutes.length > 0
        ? last5Minutes.reduce((sum, m) => sum + m.duration_ms, 0) /
          last5Minutes.length
        : 0;
    ...
    checks.push({
      ...
      status:
        avgDuration > 2000 ? 'fail' : avgDuration > 1000 ? 'warn' : 'pass',
    ...
    const errorRate =
      last5Minutes.length > 0 ? (errors.length / last5Minutes.length) * 100 : 0;
    ...
    const recentCache = recentCacheMetrics.filter(
      (m) => new Date(m.timestamp).getTime() > Date.now() - 5 * 60 * 1000,
    );
    ...
    const cacheHitRate =
      recentCache.length > 0
        ? (cacheHits.length / recentCache.length) * 100
        : 0;
  ...
  try {
    ...
    const cacheHitRatio =
      stats.blks_hit && stats.blks_read
        ? (stats.blks_hit / (stats.blks_hit + stats.blks_read)) * 100
        : 0;

    const transactionSuccessRate =
      stats.xact_commit && stats.xact_rollback
        ? (stats.xact_commit / (stats.xact_commit + stats.xact_rollback)) * 100
        : 100;
...
async function getAnalyticsDataQualityMetrics(
  ...
  try {
    ...
    const qualityChecks = await Promise.all([
      ...
      supabase
        ...
        .not(
          'event_type',
          'in',
          '(portal_login,form_submit,document_download,message_sent,email_open,email_click,call_scheduled,meeting_attended,payment_made,form_view,portal_view)',
        )
        .gte('created_at', cutoffTime.toISOString())
        .single(),

      // Check for duplicate events (same client, type, timestamp)
      supabase.rpc('check_duplicate_engagement_events', {
    ...
    const [missingClients, invalidEvents, duplicates, invalidProgress] =
      qualityChecks;
  ...
  try {
    ...
    if (validatedParams.metrics.includes('query_performance')) {
      metricsPromises.push(
        getQueryPerformanceData(supabase, validatedParams.timeframe).then(
          (data) => ({ query_performance: data }),
        ),
      );
    }

    if (validatedParams.metrics.includes('cache_efficiency')) {
      const cacheMetrics = performanceMonitor.getCacheEfficiencyMetrics(
    ...
    if (validatedParams.metrics.includes('resource_usage')) {
      const resourceMetrics = performanceMonitor.getResourceUsageMetrics(
        validatedParams.timeframe,
      );
      metricsPromises.push(
        Promise.resolve({ resource_usage: resourceMetrics }),
      );
    }

    if (validatedParams.metrics.includes('data_quality')) {
      metricsPromises.push(
        getAnalyticsDataQualityMetrics(
          supabase,
          validatedParams.timeframe,
    ...
    if (
      validatedParams.metrics.includes('database_performance') ||
      validatedParams.include_details
    ) {
      metricsPromises.push(
        getDatabasePerformanceMetrics(supabase, validatedParams.timeframe).then(
          (data) => ({ database_performance: data }),
        ),
      );
    }
...
function generatePerformanceRecommendations(performanceData: any): Array<{
  ...
  if (performanceData.query_performance) {
    const slowQueries = performanceData.query_performance.filter(
      (q: QueryPerformanceMetric) => q.average_duration_ms > 1000,
    );
  ...
  if (performanceData.cache_efficiency) {
    const lowHitRateCaches = performanceData.cache_efficiency.filter(
      (c: CacheEfficiencyMetric) => c.hit_rate < 50,
    );
  ...
  if (performanceData.system_health) {
    const failedChecks = performanceData.system_health.checks.filter(
      (c: any) => c.status === 'fail',
    );
    const warningChecks = performanceData.system_health.checks.filter(
      (c: any) => c.status === 'warn',
    );
  ...
  if (
    ...
    performanceData.data_quality.data_quality_score < 95
  ) {
    recommendations.push({
      type: 'warning' as const,
      ...
      action:
        'Review data validation rules and implement data cleaning procedures',
    });
  }
