...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { success, limit, remaining, reset } = await configRateLimit.check(
      100,
      identifier,
    );
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (settings || layout) {
      ...
      if (settingsError) {
        throw new Error(
          `Failed to update dashboard settings: ${settingsError.message}`,
        );
      }
    }
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'import') {
      ...
      // Import widgets
      await widgetService.importDashboardConfig({
        widgets: config.widgets as DashboardWidget[],
      });
