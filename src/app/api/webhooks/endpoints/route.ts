...
} from '@/lib/webhooks/webhook-security';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const urlValidation = WebhookSecurity.validateWebhookUrl(
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (
      validatedData.endpoint_url &&
      validatedData.endpoint_url !== existingEndpoint.endpoint_url
    ) {
  ...
  try {
    ...
    if (pendingDeliveries && pendingDeliveries.length > 0) {
      return NextResponse.json(
        {
          error:
            'Cannot delete endpoint with pending deliveries. Please wait for all deliveries to complete.',
        },
        { status: 409 },
      );
    }
