...
export async function GET(
  ...
  try {
    ...
    const includeSetupBreakdown =
      searchParams.get('include_setup_breakdown') !== 'false';
    const bufferTime = parseInt(
      searchParams.get('buffer_time_minutes') || '15',
    );
    ...
    const validSchedules = supplierSchedules.filter(
...
export const POST = withSecureValidation(
  ...
  async (
    request: NextRequest,
    validatedData: z.infer<typeof generateSchedulesSchema>,
  ) => {
    try {
      const supabase = await createClient();
