...
const listTimelinesQuerySchema = z.object({
  ...
  offset: z
    ...
    .optional(),
  sort: z
    .enum(['created_at', 'updated_at', 'wedding_date', 'name'])
    .default('updated_at'),
...
export const GET = withQueryValidation(
  ...
  async (
    request: NextRequest,
    query: z.infer<typeof listTimelinesQuerySchema>,
  ) => {
    try {
...
export const POST = withSecureValidation(
  ...
  async (
    request: NextRequest,
    validatedData: z.infer<typeof createTimelineSchema>,
  ) => {
    try {
      const supabase = await createClient();
      ...
      await supabase.from('timeline_collaborators').insert({
