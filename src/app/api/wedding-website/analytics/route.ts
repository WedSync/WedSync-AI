...
async function exportAnalyticsData(
  ...
  if (date_range) {
    ...
    startDate.setDate(
      startDate.getDate() - parseInt(date_range.replace('d', '')),
    );
...
function validateAnalyticsConfig(config: Partial<AnalyticsConfig>): string[] {
  ...
  if (
    ...
    !config.google_analytics.ga4_measurement_id
  ) {
  ...
  if (
    config.custom_analytics?.enabled &&
...
function generateAnalyticsRecommendations(
  ...
  if (
    metrics &&
    metrics.device_breakdown.mobile > metrics.device_breakdown.desktop
  ) {
    recommendations.push({
      type: 'info',
      title: 'Mobile Optimization',
      description:
        'Most visitors use mobile devices - ensure mobile experience is optimized',
      priority: 'high',
    });
  }
