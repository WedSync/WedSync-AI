...
async function purgeCDNCache(
  ...
  if (purgeResult.success) {
    // Log purge activity
    await supabase.from('cdn_activity_log').insert({
...
function generateCDNRecommendations(website: any) {
  ...
  if (!website.cdn_enabled) {
    recommendations.push({
      ...
      title: 'Enable CDN',
      description:
        'Improve your website performance with global content delivery',
      impact: 'High',
      effort: 'Low',
    });
  }
...
function generatePerformanceRecommendations(testResults: any[]) {
  const recommendations = [];
  const avgLoadTime =
    testResults.reduce((sum, result) => sum + result.load_time, 0) /
    testResults.length;
  ...
  if (avgLoadTime > 500) {
    recommendations.push(
      'Enable Brotli compression for better file compression',
    );
  }

  const cacheHitRate =
    testResults.filter((r) => r.cache_status === 'HIT').length /
    testResults.length;
  if (cacheHitRate < 0.8) {
