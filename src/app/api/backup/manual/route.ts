...
export async function POST(request: NextRequest) {
  try {
    ...
    if (weddingId) {
      priority =
        await prioritizationService.calculateWeddingPriority(weddingId);
    } else {
      priority =
        await prioritizationService.calculateOrganizationPriority(
          organizationId,
        );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const recoveryEngine = new DisasterRecoveryEngine(supabase);
    const backups = await recoveryEngine.getManualBackups(
      organizationId,
      limit,
      offset,
    );
