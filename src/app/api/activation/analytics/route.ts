...
export async function GET(request: NextRequest) {
  try {
    ...
    const completedUsers =
      statusDistribution?.filter((s) => s.completed_at).length || 0;
    const atRiskUsers =
      statusDistribution?.filter((s) => s.activation_score < 30).length || 0;
    const activeUsers =
      statusDistribution?.filter(
        (s) => s.activation_score >= 30 && !s.completed_at,
      ).length || 0;

    // Group analytics by step for funnel visualization
    const funnelSteps = analyticsData?.reduce(
      (acc, row) => {
    ...
    Object.values(funnelSteps || {}).forEach((step: any) => {
      if (step.total_entered > 0) {
        step.avg_conversion_rate =
          (step.total_completed / step.total_entered) * 100;
      }
    });
    ...
    const dailyMetrics = analyticsData?.reduce(
      (acc, row) => {
        if (!acc[row.date]) {
          acc[row.date] = {
            date: row.date,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: generateError } = await supabase.rpc(
      'generate_daily_funnel_analytics',
      {
