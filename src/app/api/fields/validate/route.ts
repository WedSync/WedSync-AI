...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.field && body.value !== undefined) {
      ...
      const validationResult = fieldEngine.validateFields(
        fields as FormField[],
        values,
      );

      // Group results by field
      const fieldResults = fields.reduce(
        (acc, field) => {
          const fieldErrors = validationResult.errors.filter(
            (e) => e.field === field.id,
          );
          const fieldWarnings = (validationResult.warnings || []).filter(
            (w) => w.field === field.id,
          );
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          fields: fieldResults,
          crossFieldIssues: {
            errors: validationResult.errors.filter(
              (e) => e.type === 'crossField',
            ),
            warnings: (validationResult.warnings || []).filter(
              (w) => w.field === 'crossField',
            ),
          },
        },
      });
      ...
      return NextResponse.json(
        {
          error:
            'Invalid request format. Expected either {field, value} or {fields, values}',
        },
        { status: 400 },
      );
    }
