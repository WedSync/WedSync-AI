  ...
  try {
    ...
    if (response === 'accepted') {
      ...
      try {
        ...
        const { error: statsError1 } = await supabase.rpc(
          'increment_vendor_connections',
          {
            vendor_id: connectionRequest.from_vendor_id,
          },
        );

        const { error: statsError2 } = await supabase.rpc(
          'increment_vendor_connections',
          {
            vendor_id: connectionRequest.to_vendor_id,
          },
        ...
        if (statsError1 || statsError2) {
          console.warn(
            'Failed to update vendor network statistics:',
            statsError1 || statsError2,
          );
