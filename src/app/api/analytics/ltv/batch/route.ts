...
async function getSuppliersFromFilters(
  ...
  for (const filter of filters) {
    ...
    // Activity filters
    if (
      filter.monthsActiveMin !== undefined ||
      filter.monthsActiveMax !== undefined
    ) {
      const cutoffMin = filter.monthsActiveMin
        ? new Date(
            Date.now() - filter.monthsActiveMin * 30 * 24 * 60 * 60 * 1000,
          )
        : new Date(0);
      const cutoffMax = filter.monthsActiveMax
        ? new Date(
            Date.now() - filter.monthsActiveMax * 30 * 24 * 60 * 60 * 1000,
          )
        : new Date();
...
async function createBatchJob(
  ...
  await supabase.from('batch_processing_jobs').insert({
    batch_id: batchId,
  ...
  try {
    ...
    if (!auth) {
      return NextResponse.json(
        {
          error:
            'Unauthorized. Batch operations require admin/executive permissions.',
        },
        { status: 401 },
      );
    }
    ...
    const backgroundProcessing =
      body.backgroundProcessing || supplierIds.length > 100;
    ...
    if (error instanceof Error) {
      if (error.message.includes('timeout')) {
        return NextResponse.json(
          {
            error:
              'Batch processing timeout. Please use background processing for large batches.',
