...
export async function GET(request: NextRequest) {
  try {
    ...
    const alerts = await engagementScoringService.getAtRiskAlerts(
      params.supplier_id,
      params.resolved,
    );
    ...
    if (params.severity) {
      filteredAlerts = alerts.filter(
        (alert) => alert.severity === params.severity,
      );
    }
  ...
  try {
    ...
    if (action === 'resolve') {
      ...
      if (params.resolution_note) {
        await supabase.from('alert_resolutions').insert({
