...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get connection type
    const connectionTypes = allMetrics
      .map((m) => m.connectionType)
      .filter(Boolean);
    const primaryConnectionType =
      connectionTypes.length > 0 ? connectionTypes[0] : null;
    ...
    const sessionStart =
      timestamps.length > 0 ? Math.min(...timestamps) : sessionData.sessionEnd;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const avgSessionDuration =
      totalSessions > 0
        ? sessions!.reduce((sum, s) => sum + (s.session_duration_ms || 0), 0) /
          totalSessions
        : 0;
