      ...
      cohorts?.map((cohort) => ({
        ...
        performance: {
          ...
          isStale: cohort.last_calculated_at
            ? new Date(cohort.last_calculated_at).getTime() <
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Missing required fields: name, cohortType, timeWindow.start, timeWindow.end',
    ...
    const config = {
      ...
      timeWindow: {
        ...
        granularity: (timeWindow.granularity || 'monthly') as
          | 'daily'
          | 'weekly'
          | 'monthly',
      },
    ...
    return NextResponse.json(
      {
        ...
        meta: {
          ...
          version: '1.0.0',
          calculationTimeMs:
            Date.now() - new Date(result.calculatedAt).getTime(),
        },
      },
      { status: 201 },
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (segmentationCriteria)
      updateData.segmentation_criteria = segmentationCriteria;
