...
async function getNodePerformanceBreakdown(supabase: any, journeyId: string) {
  ...
  const nodePerformance =
    nodes?.map((node) => {
      const nodeAnalytics =
        analytics?.filter((a) => a.node_id === node.id) || [];
      const totalExecutions = nodeAnalytics.reduce(
        (sum, a) => sum + (a.executions || 0),
        0,
      );
      const totalSuccesses = nodeAnalytics.reduce(
        (sum, a) => sum + (a.successes || 0),
        0,
      );
      const totalFailures = nodeAnalytics.reduce(
        (sum, a) => sum + (a.failures || 0),
        0,
      );
      const avgExecutionTime =
        nodeAnalytics.length > 0
          ? nodeAnalytics.reduce(
              (sum, a) => sum + (a.avg_execution_time_ms || 0),
              0,
            ) / nodeAnalytics.length
          : 0;
...
async function getJourneyConversionFunnel(supabase: any, journeyId: string) {
  ...
  const funnel =
    nodes?.map((node, index) => {
      const instancesReachedNode =
        instances?.filter((instance) =>
          instance.node_executions?.some((exec) => exec.node_id === node.id),
        ).length || 0;

      const instancesCompletedNode =
        instances?.filter((instance) =>
          instance.node_executions?.some(
            (exec) => exec.node_id === node.id && exec.status === 'completed',
          ),
        ).length || 0;
...
async function generateOptimizationRecommendations(
  ...
  const nodePerformance = await getNodePerformanceBreakdown(
    supabase,
    journeyId,
  );
  ...
  const bottleneckNodes = nodePerformance.filter(
    (n) => n.success_rate < 50 && n.executions > 10,
  );
  ...
  for (let i = 1; i < funnel.length; i++) {
    ...
    const dropOffRate =
      funnel[i - 1].reached > 0 ? dropOff / funnel[i - 1].reached : 0;
  ...
  const trendDirection =
    recentPerformance.length >= 2
      ? recentPerformance[recentPerformance.length - 1].engagement_score -
        recentPerformance[0].engagement_score
      : 0;
  ...
  if (trendDirection < -0.1) {
    recommendations.push({
      ...
      description:
        'Engagement scores have been declining over the past week. Consider refreshing your content or timing.',
