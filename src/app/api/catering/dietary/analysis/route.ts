...
const dietaryAnalysisSchema = z.object({
  ...
  dietaryRestrictions: z.array(
    z.string().min(1, 'Restriction names cannot be empty'),
  ),
  ...
  return withSecureValidation(
    ...
    async ({ body, user }) => {
      try {
        ...
        console.log(
          `[DIETARY ANALYSIS] User ${user.id} analyzed ${validatedData.ingredients.length} ingredients for ${validatedData.dietaryRestrictions.length} restrictions`,
          {
            userId: user.id,
