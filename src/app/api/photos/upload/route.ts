...
function validateImageMagicNumber(buffer: Buffer, mimeType: string): boolean {
  const magicNumbers =
    IMAGE_MAGIC_NUMBERS[mimeType as keyof typeof IMAGE_MAGIC_NUMBERS];
  ...
  if (mimeType === 'image/heic') {
    ...
    return (
      buffer.length >= 12 &&
      magicNumbers.every((byte, index) => buffer[4 + index] === byte)
    );
  }

  return (
    buffer.length >= magicNumbers.length &&
  ...
  try {
    ...
    if (!validateImageMagicNumber(buffer, file.type)) {
      return NextResponse.json(
        {
          error:
            'Invalid file format. File content does not match declared type.',
        },
        { status: 400 },
      );
    }
