...
function generateReferralCode(
  length: number = REFERRAL_DEFAULTS.CODE_LENGTH,
): string {
...
export async function POST(
  request: NextRequest,
): Promise<NextResponse<CreateReferralResponse>> {
  try {
    ...
    const sanitizedData = {
      description: validation.data.description
        ? sanitizeReferralInput(validation.data.description)
        : undefined,
      campaign_name: validation.data.campaign_name
        ? sanitizeReferralInput(validation.data.campaign_name)
        : undefined,
    ...
    if (
      ...
      !validateExpiryDate(sanitizedData.expires_at)
    ) {
      return NextResponse.json(
