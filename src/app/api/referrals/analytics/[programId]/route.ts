...
export async function GET(
  ...
  try {
    ...
    const clickThroughRate =
      totalMetrics.totalInvitations > 0
    ...
    const roi =
      totalMetrics.totalRewardsPaid > 0
        ? ((totalMetrics.totalRevenue - totalMetrics.totalRewardsPaid) /
            totalMetrics.totalRewardsPaid) *
          100
        : 0;
    ...
    const processedTopReferrers =
      topReferrers?.map((referrer) => {
        const totalEarnings =
          referrer.referral_conversions?.reduce(
