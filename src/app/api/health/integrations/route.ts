...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const tableChecks = await Promise.allSettled([
        ...
        supabase
          ...
          .select('count', { count: 'exact', head: true }),
        supabase
          .from('wedding_dietary_summaries')
          .select('count', { count: 'exact', head: true }),
        supabase
    ...
    console.log(
      `[HEALTH CHECK] Overall status: ${healthCheck.status} (${healthCheck.overall.responseTime}ms)`,
      {
          ...
          case 'openai':
            try {
              ...
              detailedHealth.results.openai =
                await openAIService.getServiceHealth();
            } catch (error: any) {
              detailedHealth.results.openai = {
                status: 'error',
                message: error.message,
          ...
          case 'notifications':
            try {
              ...
              detailedHealth.results.notifications =
                await notificationService.getServiceHealth();
            } catch (error: any) {
              detailedHealth.results.notifications = {
                status: 'error',
                message: error.message,
