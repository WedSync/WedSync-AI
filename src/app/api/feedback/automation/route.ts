...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Manual automation trigger requested for session ${validatedData.sessionId}`,
    );
    ...
    try {
      ...
      const trigger = {
        ...
        sentimentCategory:
          validatedData.sentimentCategory ||
        ...
        weddingContext: {
          ...
          isWeddingSeason:
            validatedData.weddingContext?.isWeddingSeason ??
            isCurrentlyWeddingSeason(),
          vendorType:
            validatedData.weddingContext?.vendorType ||
            userProfile?.vendor_type,
          userTier:
            validatedData.weddingContext?.userTier ||
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { userId, sessionId, status, actionType, page, limit } =
      automationQuerySchema.parse(queryParams);
