...
} from '@/lib/services/super-connector-service';

// Query parameter validation schema
const superConnectorQuerySchema = z.object({
  action: z
    .enum(['identify', 'analyze', 'priority', 'update'])
    .default('identify'),
  ...
  invitation_type: z
    .enum(['invitation', 'referral', 'collaboration'])
    .optional(),
});
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    switch (action) {
      case 'identify':
        const superConnectors =
          await SuperConnectorService.identifySuperConnectors(force_refresh);
        ...
        const publicConnectors = paginatedConnectors.map((sc) => ({
          ...
          avg_connection_strength:
            Math.round(sc.avg_connection_strength * 10) / 10,
      ...
      case 'analyze':
        ...
        if (
          !session.user.role ||
          !['admin', 'marketing_manager', 'data_analyst'].includes(
            session.user.role,
          )
        ) {
          return NextResponse.json(
            {
              error: 'FORBIDDEN',
              message: 'Insufficient permissions for network analysis',
            },
            { status: 403 },
          );
        }
        ...
        const networkAnalysis =
          await SuperConnectorService.getNetworkAnalysis();
      ...
      case 'priority':
          ...
          return NextResponse.json(
            {
              ...
              message:
                'Priority calculation requires user_id, recipient_id, and invitation_type',
            },
            { status: 400 },
          );
        }
        ...
        const calculatedPriority =
          await SuperConnectorService.calculateInboxPriority(
            recipient_id,
            user_id,
            invitation_type,
          );
        ...
        if (invitation_type === 'collaboration') {
          priorityFactors.push(
            'Collaboration invitation receives priority boost',
          );
        }
      ...
      case 'update':
        ...
        if (
          ...
          return NextResponse.json(
            {
              error: 'FORBIDDEN',
              message: 'Insufficient permissions for super-connector updates',
            },
            { status: 403 },
          );
        }
        ...
        SuperConnectorService.identifySuperConnectors(true)
          ...
          .catch((error) =>
            console.error('Super-connector refresh failed:', error),
          );
    ...
    const performanceThreshold =
      action === 'identify' ? 1000 : action === 'priority' ? 50 : 500;

    if (processingTime > performanceThreshold * 0.9) {
      console.warn(
        `Super-connector ${action} took ${processingTime}ms - approaching ${performanceThreshold}ms limit`,
      );
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Process updates
    await SuperConnectorService.updateSuperConnectorStatus(
      updates.map((update) => ({
