...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { action, user_id, timeframe, status_filter, limit, offset } =
    ...
    if (user_id && user_id !== session.user.id) {
      if (
        ...
        return NextResponse.json(
          {
            error: 'FORBIDDEN',
            message: 'Cannot access other users reward data',
          },
          { status: 403 },
        );
      }
    ...
    switch (action) {
      ...
      case 'tiers':
        const currentTier =
          await ReferralRewardService.getReferrerTier(targetUserId);
        ...
        return NextResponse.json(
          {
            ...
            message: 'Use POST for reward calculations',
          },
          { status: 405 },
        );
    ...
    if (totalTime > 500) {
      console.warn(
        `Rewards ${action} took ${totalTime}ms - may need optimization`,
      );
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: 'Unable to determine action type from request body',
        },
        { status: 400 },
      );
    }
    ...
    if (actionType === 'calculate') {
      const { referee_id, referral_type, conversion_value, metadata } =
      ...
      const { reward_id, fulfillment_method, notes } =
        validationResult.data as any;

      // Check permissions for fulfillment
      if (
        !session.user.role ||
        !['admin', 'finance_manager', 'marketing_manager'].includes(
          session.user.role,
        )
      ) {
        return NextResponse.json(
          {
            error: 'FORBIDDEN',
            message: 'Insufficient permissions for reward fulfillment',
          },
          { status: 403 },
        );
      }
    ...
    if (actionTime > performanceThreshold * 0.9) {
      console.warn(
        `Reward ${actionType} took ${actionTime}ms - approaching ${performanceThreshold}ms limit`,
      );
    }

    responseData.performance_metrics.processing_time_ms =
      Date.now() - startTime;
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (
      !session.user.role ||
      !['admin', 'finance_manager'].includes(session.user.role)
    ) {
      return NextResponse.json(
        {
          error: 'FORBIDDEN',
          message: 'Admin permissions required for reward updates',
        },
        { status: 403 },
      );
    }
    ...
    const { reward_id, action, reason, new_expiry_date } =
      validationResult.data;
