...
export async function GET(
  ...
  try {
    ...
    const recipientLimit = Math.min(
      parseInt(searchParams.get('recipient_limit') || '50'),
      100,
    );
    ...
    if (campaign.channels.includes('email')) {
      const emailStats = await supabase
        ...
        .eq('organization_id', userProfile.organization_id)
        .or(
          `metadata->>campaign_id.eq.${campaignId},template_type.eq.bulk_${campaignId}`,
        );

      const emailStatusCounts = (emailStats.data || []).reduce(
    ...
    if (campaign.channels.includes('sms')) {
      ...
      const smsStatusCounts = (smsStats || []).reduce(
        (acc: any, notification) => {
          acc[notification.status] = (acc[notification.status] || 0) + 1;
          acc.total_cost = (acc.total_cost || 0) + (notification.cost || 0);
          acc.total_segments =
            (acc.total_segments || 0) + (notification.segments || 1);
          return acc;
        },
  ...
  try {
    ...
    switch (action) {
      case 'retry_failed':
        ...
        const { count: retriedEmails } = await failedEmailQuery.select('*', {
          count: 'exact',
          head: true,
        });
        ...
        const { count: retriedSMS } = await failedSMSQuery.select('*', {
          count: 'exact',
          head: true,
        });
      ...
      case 'refresh_stats':
        ...
        const [
          ...
          supabase
            .from('bulk_message_recipients')
            .select(
              'email_status, sms_status, email_opened_at, email_clicked_at',
            )
            .eq('campaign_id', campaignId),
        ]);

        // Calculate stats
        const emailStatusCounts = (emailStats || []).reduce(
          (acc: any, email) => {
        ...
        await supabase
          ...
          .update({
            sent_count:
              (emailStatusCounts.sent || 0) + (smsStatusCounts.sent || 0),
            delivered_count:
              (emailStatusCounts.delivered || 0) +
              (smsStatusCounts.delivered || 0),
            failed_count:
              (emailStatusCounts.failed || 0) + (smsStatusCounts.failed || 0),
