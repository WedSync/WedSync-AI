...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch SMS analytics
    const smsAnalytics = await getSMSAnalytics(
      supabase,
      user.id,
      validatedParams,
    );
    ...
    const combinedMetrics = aggregateMetrics(
      emailAnalytics,
      smsAnalytics,
      validatedParams,
    );
...
function aggregateMetrics(emailData: any, smsData: any, params: any) {
  ...
  const byChannel = {
    email: {
      ...
      deliveryRate:
        emailData.sent > 0 ? (emailData.delivered / emailData.sent) * 100 : 0,
      openRate:
        emailData.delivered > 0
          ? (emailData.opened / emailData.delivered) * 100
          : 0,
    },
    sms: {
      sent: smsData.sent,
      ...
      deliveryRate:
        smsData.sent > 0 ? (smsData.delivered / smsData.sent) * 100 : 0,
      openRate:
        smsData.delivered > 0 ? (smsData.opened / smsData.delivered) * 100 : 0,
    },
  };
...
function calculatePerformanceScores(metrics: any) {
  ...
  const scores = {
    ...
    delivery: Math.min(
      100,
      (metrics.summary.deliveryRate / benchmarks.deliveryRate) * 100,
    ),
    engagement: Math.min(
      100,
      (metrics.summary.openRate / benchmarks.openRate) * 100,
    ),
    quality: Math.max(
      0,
      100 - (metrics.summary.bounceRate / benchmarks.bounceRate) * 50,
    ),
    retention: Math.max(
      0,
      100 - (metrics.summary.unsubscribeRate / benchmarks.unsubscribeRate) * 50,
    ),
  };

  scores.overall =
    (scores.delivery + scores.engagement + scores.quality + scores.retention) /
    4;
...
async function trackConversion(supabase: any, data: any) {
  await supabase.from('communication_conversions').insert({
...
function generateRecommendations(metrics: any, engagement: any, issues: any) {
  ...
  if (metrics.openRate < 20) {
    recommendations.push({
      ...
      priority: 'high',
      message:
        'Low open rate detected. Consider improving subject lines and send timing.',
    });
  }
  ...
  if (metrics.clickRate < 2) {
    recommendations.push({
      ...
      priority: 'medium',
      message:
        'Low click rate. Improve call-to-action buttons and content relevance.',
    });
  }
