...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const recipient of validatedData.recipients) {
      ...
      if (validatedData.channels.includes('email')) {
        ...
        if (!recipient.email && validatedData.test_type === 'send_test') {
          testResult.warnings?.push(
            `Recipient ${recipient.name} has no email address for testing`,
          );
        }
        ...
        const subjectPreview = sanitizedSubject
          ? personalizationEngine.generatePreview(
              sanitizedSubject,
              personalizationVars,
            )
          : undefined;
        ...
        if (emailPreview.personalizedContent.length === 0) {
          recipientResult.email.validation_errors?.push(
            'Email content is empty after personalization',
          );
        ...
        if (emailPreview.missingTokens.length > 0) {
          recipientResult.email.validation_errors?.push(
            `Missing tokens: ${emailPreview.missingTokens.join(', ')}`,
          );
        }
        ...
        if (validatedData.test_type === 'send_test' && recipient.email) {
          try {
            ...
            await EmailService.sendEmail({
              ...
              template: {
                type: 'div',
                props: { dangerouslySetInnerHTML: { __html: testContent } },
              } as any,
            ...
            recipientResult.email.test_error =
              error instanceof Error ? error.message : 'Unknown error';
            testResult.errors?.push(
              `Failed to send test email to ${recipient.name}: ${recipientResult.email.test_error}`,
            );
          }
        }

        testResult.summary.total_tokens_found +=
          emailPreview.tokensFound.length;
        testResult.summary.total_missing_tokens +=
          emailPreview.missingTokens.length;
      }
      ...
      if (validatedData.channels.includes('sms')) {
        ...
        if (!recipient.phone && validatedData.test_type === 'send_test') {
          testResult.warnings?.push(
            `Recipient ${recipient.name} has no phone number for testing`,
          );
        }
        ...
        const estimatedCost =
          segments * (recipient.phone?.startsWith('+1') ? 0.0075 : 0.05); // Simplified pricing
        ...
        if (smsPreview.personalizedContent.length === 0) {
          recipientResult.sms.validation_errors?.push(
            'SMS content is empty after personalization',
          );
          testResult.summary.validation_passed = false;
        }
        ...
        if (smsPreview.personalizedContent.length > 320) {
          recipientResult.sms.validation_errors?.push(
            `SMS content too long: ${smsPreview.personalizedContent.length} characters (max 320)`,
          );
        }

        if (smsPreview.missingTokens.length > 0) {
          recipientResult.sms.validation_errors?.push(
            `Missing tokens: ${smsPreview.missingTokens.join(', ')}`,
          );
        }

        // Test send if requested
        if (
          validatedData.test_type === 'send_test' &&
          recipient.phone &&
          organization
        ) {
          try {
            ...
            recipientResult.sms.test_error =
              error instanceof Error ? error.message : 'Unknown error';
            testResult.errors?.push(
              `Failed to send test SMS to ${recipient.name}: ${recipientResult.sms.test_error}`,
            );
          }
        }
        ...
        testResult.summary.total_missing_tokens +=
          smsPreview.missingTokens.length;
        testResult.summary.estimated_total_cost =
          (testResult.summary.estimated_total_cost || 0) + estimatedCost;
      }
      ...
      if (validatedData.personalization) {
        ...
        const allVariablesUsed = Object.keys(personalizationVars).filter(
          (key) => allTokensFound.has(key) && !allMissingTokens.has(key),
        );

        recipientResult.personalization = {
          variables_used: allVariablesUsed,
          variables_missing: Array.from(allMissingTokens),
          fallbacks_applied: Object.keys(
            validatedData.fallback_variables || {},
          ),
        };
      }
    ...
    if (testResult.summary.total_missing_tokens > 0) {
      testResult.warnings?.push(
        `Found ${testResult.summary.total_missing_tokens} missing personalization tokens across all recipients`,
      );
    }

    if (validatedData.test_type === 'send_test') {
      testResult.warnings?.push(
        'Test messages have been sent with [TEST] prefixes. These are real messages but clearly marked as tests.',
      );
    }
    ...
    if (
      ...
      testResult.summary.validation_passed
    ) {
...
{{vendor_name}}`,
      ...
      test_types: [
        ...
        },
        {
          ...
          description:
            'Actually send test messages (marked with [TEST] prefix)',
        },
      ],
    };
    ...
    if (exampleType === 'recipients') {
      return NextResponse.json({
        sample_recipients: examples.sample_recipients,
      });
    }
