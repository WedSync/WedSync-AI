...
export async function GET(request: NextRequest) {
  try {
    ...
    if (searchQuery) {
      query = query.or(
        `name.ilike.%${searchQuery}%,description.ilike.%${searchQuery}%,content.ilike.%${searchQuery}%`,
      );
    }
    ...
    const enhancedTemplates: TemplateWithTokens[] = (templates || []).map(
      (template) => {
        const detectedTokens = [
          ...personalizationEngine.extractTokens(template.content || ''),
          ...(template.subject
            ? personalizationEngine.extractTokens(template.subject)
            : []),
        ];
...
export async function POST(request: NextRequest) {
  try {
    ...
    const sanitizedSubject = validatedData.subject
      ? sanitizeString(validatedData.subject, 255)
      : null;
    ...
    const detectedTokens = [
      ...
      ...(sanitizedSubject
        ? personalizationEngine.extractTokens(sanitizedSubject)
        : []),
    ];

    // Merge provided variables with detected tokens
    const allVariables = [
      ...new Set([...validatedData.variables, ...detectedTokens]),
    ];
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateData.content || updateData.subject !== undefined) {
      const personalizationEngine = new PersonalizationEngine();
      const detectedTokens = [
        ...personalizationEngine.extractTokens(
          updateData.content || existingTemplate.content,
        ),
        ...(updateData.subject !== undefined
          ? updateData.subject
            ? personalizationEngine.extractTokens(updateData.subject)
            : []
          : existingTemplate.subject
            ? personalizationEngine.extractTokens(existingTemplate.subject)
            : []),
      ];

      // Merge with provided variables or existing variables
      const existingVariables =
        validatedData.variables || existingTemplate.variables || [];
      updateData.variables = [
        ...new Set([...existingVariables, ...detectedTokens]),
      ];
    ...
    const detectedTokens = [
      ...
      ...(updatedTemplate.subject
        ? personalizationEngine.extractTokens(updatedTemplate.subject)
        : []),
    ];
