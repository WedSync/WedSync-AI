...
async function getOptimizedHealthMetrics(
  ...
  let query = supabase.from('customer_success_dashboard') // Use the optimized view
    .select(`
      organization_id,
      organization_name,
      subscription_tier,
  ...
  if (options.status && options.status !== 'all') {
    ...
    query = query.eq(
      'health_status',
      statusMap[options.status as keyof typeof statusMap],
    );
  }
  ...
  const result = {
    ...
    at_risk_users:
      healthData?.filter(
        (d) => d.health_status === 'at_risk' || d.health_status === 'critical',
      ) || [],
    engagement_trends: await getEngagementTrends(
      supabase,
      organizationId,
      30,
      tracker,
  ...
  try {
    await redis.setex(
      cacheKey,
      CACHE_CONFIG.dashboardMetrics.ttl,
...
async function getSummaryMetrics(
  ...
  const { data, error } = await supabase.rpc(
    'get_organization_summary_metrics',
    {
...
async function getHealthDistribution(
  ...
  const distribution = ranges.map((range) => {
    const count = data.filter(
      (d) => d.overall_score >= range.min && d.overall_score <= range.max,
    ).length;
...
export const POST = withSecureValidation(
  ...
  async (request: NextRequest, validatedData) => {
    ...
    try {
      ...
      const results = await processBulkHealthScores(
        supabase,
        validatedData,
        tracker,
      );
...
async function processBulkHealthScores(
  supabase: any,
  validatedData: any,
  tracker: ApiPerformanceTracker,
) {
  ...
  if (validatedData.filters) {
    ...
    if (
      validatedData.filters.riskLevels &&
      validatedData.filters.riskLevels.length > 0
    ) {
...
async function handleCsvExport(
  ...
  const { data, error } = await processBulkHealthScores(
    supabase,
    validatedData,
    tracker,
  );
  ...
  return new NextResponse(csvContent, {
    headers: {
      ...
      'Content-Disposition':
        'attachment; filename="customer-health-export.csv"',
      'Cache-Control': 'no-cache',
    },
  });
}
