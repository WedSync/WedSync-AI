    ...
    switch (action) {
      ...
      case 'health-summary': {
        ...
        const summary = {
          ...
          criticalRisk: healthScores.filter((s) => s.riskLevel === 'critical')
            .length,
          highRisk: healthScores.filter((s) => s.riskLevel === 'high').length,
          mediumRisk: healthScores.filter((s) => s.riskLevel === 'medium')
            .length,
          lowRisk: healthScores.filter((s) => s.riskLevel === 'low').length,
          averageHealthScore:
            healthScores.reduce((acc, s) => acc + s.healthScore, 0) /
              suppliers.length || 0,
        };
  ...
  try {
    ...
    switch (action) {
      ...
      case 'batch-process': {
        ...
        const result =
          await healthInterventionService.batchProcessInterventions(
            validation.data.organizationId,
