...
export async function POST(request: NextRequest) {
  try {
    ...
    const parseResult = guestService.parseNaturalLanguageNames(
      validatedData.name_input,
    );
      ...
      return NextResponse.json(
        {
          ...
          suggestion:
            'Try formats like "John Smith", "John and Jane Smith", or "The Smith Family"',
        },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          suggestion:
            'Please verify the parsed names are correct before proceeding',
        },
        { status: 422 },
      );
    }
    ...
    const potentialDuplicates = duplicateChecks.filter(
      (check) => check.duplicates.length > 0,
    );
      ...
      return NextResponse.json(
        {
          ...
          suggestion:
            'Review potential duplicates or use force_create=true to create anyway',
        },
        { status: 409 },
      );
    }
    ...
    const shouldCreateHousehold =
      parseResult.household_detected && guestsToCreate.length > 1;

    let householdId: string | null = null;
    if (shouldCreateHousehold) {
      // Create household first
      const householdName = guestService['generateHouseholdName'](
        guestsToCreate.map(
          (g) =>
            ({
    ...
    const analytics = await guestService.calculateGuestCounts(
      validatedData.couple_id,
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!nameInput) {
      return NextResponse.json(
        { error: 'name_input parameter required' },
        { status: 400 },
      );
    }
...
function generateParsingSuggestions(parseResult: any): string[] {
  ...
  if (parseResult.confidence_score < 70) {
    suggestions.push(
      'Consider using a more standard format like "First Last" or "First and Second Last"',
    );
  }

  if (parseResult.guests.length === 0) {
    suggestions.push(
      'No valid names detected. Try formats like "John Smith" or "John and Jane Smith"',
    );
  }

  if (
    parseResult.guests.some(
      (g: any) => g.first_name === 'Mr.' || g.first_name === 'Mrs.',
    )
  ) {
    suggestions.push(
      'Titles detected. You may want to replace "Mr." and "Mrs." with actual first names',
    );
  }

  if (parseResult.household_detected && parseResult.guests.length === 1) {
    suggestions.push(
      'Family/household detected but only one name parsed. Consider adding more family members',
    );
  }

  if (parseResult.guests.some((g: any) => g.plus_one)) {
    suggestions.push(
      'Plus-one detected. You can specify the plus-one name later or use "John and Jane Smith" format',
    );
  }
