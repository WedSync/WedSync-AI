  ...
  try {
    ...
    try {
      ...
      if (validatedData.import_type === 'csv') {
        ...
        throw new Error(
          `Import type ${validatedData.import_type} not yet implemented`,
        );
      }
      ...
      if (
        validatedData.household_grouping &&
        importResults.successful_guests.length > 0
      ) {
        await createHouseholdsFromImport(
          supabase,
          importResults.successful_guests,
        );
      }
      ...
      const performanceMetrics = {
        processing_time_ms:
          Date.now() - new Date(importSession.created_at).getTime(),
...
async function createHouseholdsFromImport(
  supabase: any,
  guests: any[],
): Promise<number> {
  ...
  for (const [key, householdGuests] of householdGroups.entries()) {
    if (householdGuests.length > 1) {
      try {
        ...
        const { data: householdId } = await supabase.rpc(
