...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (!file || !metaDataStr) {
        return NextResponse.json(
          { error: 'File and metadata required' },
          { status: 400 },
        );
      }
    ...
    const processedGuests = await processGuestData(
      guestData,
      validatedData,
      guestService,
    );
    ...
    if (
      processedGuests.errors.length > 0 &&
      processedGuests.validGuests.length === 0
    ) {
      return NextResponse.json(
        {
    ...
    if (validatedData.validate_only) {
      return NextResponse.json({
        ...
        validation_result: {
          ...
          household_groups_detected: await guestService
            .createHouseholdGroups(
              validatedData.couple_id,
    ...
    try {
      ...
      const performanceMetrics = {
        ...
        guests_per_second: Math.round(
          (importResult.successful_count / totalTime) * 1000,
        ),
        batch_processing_time:
          totalTime /
          Math.ceil(
            processedGuests.validGuests.length / validatedData.batch_size,
          ),
      ...
      await supabase
        ...
        .update({
          status:
            importResult.failed_count > 0 ? 'partial_success' : 'completed',
      ...
      return NextResponse.json({
        ...
        results: {
          ...
          households_created: validatedData.create_households
            ? Math.floor(importResult.successful_count / 2.5)
            : 0, // Estimate
        },
        performance_metrics: performanceMetrics,
        analytics,
...
async function processGuestData(
  ...
  for (const [index, rawGuest] of rawData.entries()) {
    try {
      ...
      for (const [sourceField, targetField] of Object.entries(mapping)) {
        ...
        if (value !== undefined && value !== null && value !== '') {
          ...
          if (targetField === 'plus_one') {
            ...
            ['category', 'side', 'age_group', 'rsvp_status'].includes(
              targetField,
            )
          ) {
            // Normalize enum values
            processedGuest[targetField] = normalizeEnumValue(
              targetField,
              value,
            );
          } else {
      ...
      if (
        !processedGuest.first_name ||
        processedGuest.first_name.length === 0
      ) {
      ...
      if (validationErrors.length > 0) {
        errors.push({
          ...
          value:
            `${processedGuest.first_name || ''} ${processedGuest.last_name || ''}`.trim(),
        });
        continue;
      }
      ...
      if (processedGuest.address) {
        processedGuest.address = guestService.normalizeAddress(
          processedGuest.address,
        );
      }
...
function normalizeEnumValue(field: string, value: string): string {
  ...
  switch (field) {
    case 'category':
      if (['family', 'relative', 'relatives'].includes(normalized))
        return 'family';
      if (['friend', 'friends'].includes(normalized)) return 'friends';
      if (['work', 'colleague', 'coworker', 'business'].includes(normalized))
        return 'work';
      return 'other';
    ...
    case 'rsvp_status':
      if (['yes', 'attending', 'accept', 'confirmed'].includes(normalized))
        return 'attending';
      if (['no', 'declined', 'decline', 'reject'].includes(normalized))
        return 'declined';
