  ...
  try {
    ...
    switch (analysisType) {
      ...
      case 'booking_prediction':
        ...
        response =
          await bookingPredictor.batchPredictBookingProbability(clientIds);
        break;
      ...
      case 'pricing_optimization':
        const serviceCategory =
          url.searchParams.get('service') || 'wedding_photography';
        response = await pricingOptimizer.optimizePricing(
          org.id,
          serviceCategory,
        );
        break;
  ...
  try {
    ...
    switch (operation) {
      case 'batch_client_analysis':
        ...
        response =
          await bookingPredictor.batchPredictBookingProbability(client_ids);
        break;
        ...
        for (const type of forecast_types) {
          if (type === 'revenue') {
            ...
            for (const period of periods || [
              ...
              forecasts['revenue'][period] =
                await revenueForecaster.forecastRevenue(org.id, period);
            }
          } else if (type === 'resource_planning') {
            const horizon = periods?.[0] || 12;
            forecasts['resource_planning'] =
              await resourcePlanner.planResources(org.id, horizon);
          }
        }
      ...
      case 'optimization_analysis':
        ...
        if (optimization_types?.includes('pricing')) {
          optimizations['pricing'] = await pricingOptimizer.optimizePricing(
            org.id,
          );
        }
