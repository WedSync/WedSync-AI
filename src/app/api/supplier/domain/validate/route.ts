...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!domainRegex.test(normalizedDomain)) {
      result.errors.push(
        'Invalid domain format. Use alphanumeric characters and dots only.',
      );
    ...
    if (
      reservedDomains.some((reserved) => normalizedDomain.includes(reserved))
    ) {
      result.errors.push('This domain is reserved and cannot be used.');
      result.suggestions?.push(
        'Try using a subdomain of your own domain (e.g., portal.yourbusiness.com)',
      );
      return NextResponse.json({ success: true, data: result });
    }
    ...
    if (parts.length > 3) {
      result.errors.push(
        'Domain has too many subdomains. Use a simpler structure (e.g., portal.yourbusiness.com).',
      );
      return NextResponse.json({ success: true, data: result });
    }
    ...
    if (
      parts.length >= 2 &&
      !suggestedSubdomains.some((sub) => firstPart.includes(sub))
    ) {
      result.suggestions?.push(
        'Consider using professional subdomains like: portal.yourbusiness.com, clients.yourbusiness.com',
      );
    }
    ...
    try {
      ...
      if (!isAvailable) {
        result.errors.push(
          'This domain appears to already be configured in our system or is not available.',
        );
        result.suggestions?.push(
          'Try a different subdomain or contact support if this is your domain.',
        );
      }
    } catch (dnsError) {
      ...
      result.suggestions?.push(
        'Unable to verify domain availability. Please ensure you own this domain.',
      );
    }
