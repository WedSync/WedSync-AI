  ...
  try {
    ...
    const mockNotifications = [
      ...
      },
      {
        ...
        message:
          'Wedding ceremony time changed to 4:00 PM for the Smith wedding',
        priority: 'medium',
    ...
    const allNotifications = [...notifications, ...mockNotifications].sort(
      (a, b) =>
...
function getNotificationType(subject?: string, content?: string): string {
  ...
  if (text.includes('booking') || text.includes('inquiry'))
...
function getActionUrl(metadata?: any): string | undefined {
  ...
  if (metadata.booking_id)
    return `/supplier-portal/bookings/${metadata.booking_id}`;
  if (metadata.event_id)
    return `/supplier-portal/schedule/events/${metadata.event_id}`;
  if (metadata.payment_id)
    return `/supplier-portal/bookings/payment-${metadata.payment_id}`;
