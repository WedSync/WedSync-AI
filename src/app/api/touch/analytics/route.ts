  ...
  try {
    ...
    const analyticsRecord = {
      ...
      target_response_time:
        validatedData.performance_metrics?.target_response_time,
      performance_deviation: validatedData.performance_metrics
        ? validatedData.performance_metrics.actual_response_time -
          validatedData.performance_metrics.target_response_time
        : 0,
      success_rate:
        validatedData.performance_metrics?.success_rate ||
        (validatedData.success ? 1 : 0),
    ...
    await updateRealtimeMetrics(
      supabase,
      validatedData.gesture_type,
      validatedData.response_time,
    );

    // Check for performance issues and trigger alerts
    const targetTime =
      validatedData.performance_metrics?.target_response_time || 100;
      ...
      .upsert(
        {
          ...
          gdpr_consent_date:
            validatedSettings.gdpr_consent_date || new Date().toISOString(),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: metrics, error } = await query.order('created_at', {
      ascending: false,
    });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (analyticsError || settingsError || preferencesError) {
      console.error('Data deletion errors:', {
        analyticsError,
        settingsError,
        preferencesError,
      });
...
function aggregateMetrics(metrics: any[]): any {
  ...
  const averageResponseTime =
    ...
    totalInteractions;
  const successRate =
    metrics.reduce((sum, m) => sum + m.success_rate, 0) / totalInteractions;
...
function groupByWorkflow(metrics: any[]): any {
  return metrics.reduce((acc, m) => {
    ...
    acc[workflow].avg_response_time =
      (acc[workflow].avg_response_time + m.avg_response_time) / 2;
    acc[workflow].success_rate =
      (acc[workflow].success_rate + m.success_rate) / 2;
    return acc;
  }, {});
}
