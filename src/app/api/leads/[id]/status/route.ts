...
export async function PATCH(
  ...
  try {
    ...
    const { data: statusUpdateResult, error: statusError } = await supabase.rpc(
      'update_lead_status',
      {
        client_uuid: id,
    ...
    if (Object.keys(additionalUpdates).length > 0) {
      ...
      await supabase.from('clients').update(additionalUpdates).eq('id', id);
    }
    ...
    await supabase.from('client_activities').insert({
      ...
      activity_description:
        ...
        `Lead status updated to ${validatedData.status}`,
    ...
    if (statusHistory && statusHistory.length > 0) {
      ...
      if (lastChange) {
        const hoursSinceChange = Math.floor(
          (new Date().getTime() -
            new Date(lastChange.status_changed_at).getTime()) /
            (1000 * 60 * 60),
        );
