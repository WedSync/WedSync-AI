...
export async function GET(request: NextRequest) {
  try {
    ...
    const wedMeJourneys: WedMeJourneyView[] = filteredJourneys.map(
      (journey) => {
        const canvasData = journey.journey_definitions?.canvas_data as any;
        const nodes = canvasData?.nodes || [];
        const completedNodes =
          journey.journey_nodes?.filter((n: any) => n.status === 'completed') ||
          [];
        ...
        estimatedCompletion.setDate(
          estimatedCompletion.getDate() + estimatedDays,
        );
                  ...
                  : n.type === 'meeting'
                    ...
                    : n.type === 'payment'
                      ...
                      : n.type === 'review'
                        ...
                        : ('document' as any),
              priority:
                idx === 0 ? 'high' : idx < 3 ? 'medium' : ('low' as any),
              completed: false,
            };
          });
        ...
        const milestones = [
          ...
          },
          {
            ...
            date: new Date(
              new Date(journey.started_at).getTime() + 7 * 24 * 60 * 60 * 1000,
            ),
        ...
        return {
          ...
          vendorName:
            journey.journey_definitions?.organizations?.name ||
            'Unknown Vendor',
          vendorType:
            journey.journey_definitions?.organizations?.vendor_type ||
            'general',
