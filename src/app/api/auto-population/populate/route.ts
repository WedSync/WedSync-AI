...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { formId, clientId, formFields, populationPreferences } =
    ...
    const weddingData =
      await autoPopulationService.getOrCreateWeddingData(clientId);

    if (!weddingData) {
      return NextResponse.json(
        {
          success: false,
          error: 'Could not access wedding data - Please try again',
        },
        { status: 500 },
      );
    }
    ...
    if (detectedMappings.length > 0) {
      ...
      autoPopulationService
        .createFormFieldMappings(formId, detectedMappings)
        .catch((error) =>
          console.error('Background mapping creation failed:', error),
        );
    }
    ...
    const filteredMappings = detectedMappings.filter((mapping) => {
      if (
        populationPreferences?.minimumConfidence &&
    ...
    if (processingTime > 3000) {
      ...
      console.warn(
        `Slow auto-population: ${processingTime}ms for ${fieldsDetected} fields`,
      );
    }
