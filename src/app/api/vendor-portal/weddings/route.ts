  ...
  try {
    ...
    if (status && status !== 'all') {
      ...
      switch (status) {
        ...
        case 'active':
          query = query
            .lt('clients.wedding_date', now)
            .neq('clients.status', 'completed');
          break;
        case 'completed':
    ...
    const weddings = await Promise.all(
      connections?.map(async (connection) => {
        ...
        return {
          ...
          notes: client.notes,
          other_vendors:
            otherVendors?.map((v) => v.suppliers).filter(Boolean) || [],
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (
      !connection ||
      connection.organization_id !== userProfile?.organization_id
    ) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 403 });
    }
