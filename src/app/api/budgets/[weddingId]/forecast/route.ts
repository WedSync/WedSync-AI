...
export async function GET(
  ...
  try {
    ...
    if (spendingValues.length > 1) {
      ...
      const sumXY = spendingValues.reduce(
    ...
    for (let i = 0; i < horizon_months; i++) {
      ...
      const monthName = projectedMonth.toLocaleString('default', {
        month: 'short',
        year: '2-digit',
      });
      ...
      switch (method) {
        ...
        case 'exponential':
          projectedMonthlySpend =
            avgMonthlySpending * Math.pow(1 + trend / avgMonthlySpending, i);
          break;
        case 'seasonal':
          // Simple seasonal adjustment (increase spending closer to wedding date)
          const weddingDate = new Date(wedding.wedding_date);
          const monthsToWedding = Math.max(
            1,
            (weddingDate.getTime() - projectedMonth.getTime()) /
              (1000 * 60 * 60 * 24 * 30),
          );
          const seasonalMultiplier =
            monthsToWedding <= 3 ? 1.5 : monthsToWedding <= 6 ? 1.2 : 1.0;
    ...
    if (monthsToWedding > 0 && avgMonthlySpending > 0) {
      const recommendedMonthlySpend =
        (totalBudget - currentSpent) / monthsToWedding;
      ...
      if (Math.abs(spendingPaceDiff) > recommendedMonthlySpend * 0.2) {
        recommendations.push({
          title:
            spendingPaceDiff > 0 ? 'Spending Too Quickly' : 'Underspending',
          description:
            spendingPaceDiff > 0
          ...
          urgency:
            Math.abs(spendingPaceDiff) > recommendedMonthlySpend * 0.4
              ? 'medium'
              : 'low',
        });
      }
