    ...
    switch (type) {
          ...
          summaries?.map((summary) => ({
            ...
            avg_response_time_ms:
              summary.total_requests > 0
                ...
                : 0,
            total_request_size_mb: Math.round(
              summary.total_request_size_bytes / 1024 / 1024,
            ),
            total_response_size_mb: Math.round(
              summary.total_response_size_bytes / 1024 / 1024,
            ),
      ...
      case 'organizations':
        ...
        for (const org of organizations || []) {
          ...
          const errorCount =
            orgLogs?.filter((log) => log.status_code >= 400).length || 0;
          const avgResponseTime =
            totalRequests > 0
              ? Math.round(
                  orgLogs!.reduce((sum, log) => sum + log.response_time_ms, 0) /
                    totalRequests,
                )
              : 0;
      ...
      case 'errors':
        ...
        const { data: errorLogs, error: errorsError } = await supabase
          .from('api_usage_logs')
          .select(
            'timestamp, organization_id, endpoint, method, status_code, response_time_ms, user_agent, ip_address',
          )
          .gte('status_code', 400)
          .gte('timestamp', start.toISOString())
          .lte('timestamp', end.toISOString())
