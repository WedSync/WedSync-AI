...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rangeMs =
      timeRanges[period as keyof typeof timeRanges] || timeRanges['24h'];
    ...
    const { data: events, error: eventsError } = await eventsQuery.order(
      'created_at',
      { ascending: true },
    );
    ...
    if (eventsError) {
      logger.error(
        'Failed to fetch rate limit events for analytics',
        eventsError,
      );
    ...
    metrics.recordHistogram(
      'admin_api.rate_limits.analytics.duration',
      Date.now() - startTime,
    );
  }
}
...
async function processAnalyticsData(
  ...
  const summary = {
    totalRequests: 0,
    blockedRequests: events.filter((e) => e.event_type === 'limit_exceeded')
      .length,
    allowedRequests: 0,
    blockRate: 0,
  };
  ...
  const endpointMap = new Map<
    string,
    { requests: number; blocked: number; avgResponseTime: number }
  >();
  ...
  // Calculate summary
  summary.totalRequests = Object.values(byTier).reduce(
    (sum, tier) => sum + tier.requests,
    0,
  );
  summary.allowedRequests = summary.totalRequests - summary.blockedRequests;
  summary.blockRate =
    summary.totalRequests > 0
      ? (summary.blockedRequests / summary.totalRequests) * 100
      : 0;
...
async function getTopBlockedEntities(
  ...
  try {
    ...
    const entityMap = new Map<
      string,
      { count: number; tier: string; lastSeen: string }
    >();
