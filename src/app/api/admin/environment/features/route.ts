...
import {
  featureFlagManager,
  evaluateFeatureFlag,
} from '@/lib/environment/feature-flag-manager';
    ...
    switch (action) {
      ...
      case 'evaluate':
        ...
        const context = {
          ...
          isWeddingSeason:
            new Date().getMonth() >= 4 && new Date().getMonth() <= 9, // May-Oct
      ...
      case 'bulk_evaluate':
        ...
        const bulkEvaluations = await featureFlagManager.evaluateFlags(
          keys,
          bulkContext,
        );

        metrics.incrementCounter(
          'api.environment.feature_flags_bulk_evaluated',
          1,
          {
        ...
        return NextResponse.json(
          {
            ...
            error: {
              message:
                'Invalid action. Use: list, evaluate, bulk_evaluate, analytics',
            },
          },
          { status: 400 },
        );
    ...
    switch (action) {
      case 'create':
        ...
        metrics.incrementCounter('api.environment.feature_flag_created', 1, {
          type,
          affects_wedding_day:
            weddingContext?.affectsWeddingDay?.toString() || 'false',
        });
      ...
      case 'emergency_disable':
        ...
        metrics.incrementCounter(
          'api.environment.feature_flag_emergency_disabled',
          1,
          {
            key,
          ...
          return NextResponse.json(
            {
              success: false,
              error: {
                message: 'environment and enabled (boolean) are required',
              },
            },
            { status: 400 },
          );
        ...
        return NextResponse.json(
          {
            ...
            error: {
              message:
                'Invalid action. Use: create, update, emergency_disable, toggle_environment',
            },
          },
          { status: 400 },
        );
