...
export async function GET(request: NextRequest) {
  try {
    ...
    const refreshInterval = parseInt(
      url.searchParams.get('refreshInterval') || '300',
    ); // 5 minutes default

    console.log('🤖 Generating comprehensive ML predictions...');
    ...
    const [churnData, revenueData, viralData, ltvData, anomalyData] =
      await Promise.allSettled([
        includeChurn
          ? generateChurnPredictions(churnPredictor, supabase)
          : null,
        includeRevenue ? generateRevenueForecast(revenueForecaster) : null,
        includeViral
          ? generateViralPredictions(viralPredictor, supabase)
          : null,
    ...
    const response: PredictionsResponse = {
      churn:
        churnData.status === 'fulfilled' && churnData.value
          ? churnData.value
          : getDefaultChurnPredictions(),
      revenue:
        revenueData.status === 'fulfilled' && revenueData.value
          ? revenueData.value
          : getDefaultRevenueForecast(),
      viral:
        viralData.status === 'fulfilled' && viralData.value
          ? viralData.value
          : getDefaultViralPredictions(),
      ltv:
        ltvData.status === 'fulfilled' && ltvData.value
          ? ltvData.value
          : getDefaultLTVPredictions(),
      anomalies:
        anomalyData.status === 'fulfilled' && anomalyData.value
          ? anomalyData.value
          : [],
...
async function generateChurnPredictions(
  ...
  const predictions = await Promise.allSettled(
    sampleSuppliers.map((supplier) =>
      churnPredictor.predictChurn(supplier.id).catch((error) => {
        console.warn(
          `Churn prediction failed for supplier ${supplier.id}:`,
          error,
        );
        return null;
      }),
  ...
  // Aggregate results
  const criticalRisk = validPredictions.filter(
    (p) => p.riskLevel === 'critical',
  ).length;
  const highRisk = validPredictions.filter(
    (p) => p.riskLevel === 'high',
  ).length;
  const totalMRRAtRisk = validPredictions.reduce(
    (sum, p) => sum + p.monthlyRevenueLoss,
    0,
  );
  const avgChurnProbability =
    validPredictions.reduce((sum, p) => sum + p.churnProbability, 0) /
    validPredictions.length;
  const avgSeasonalAdjustment =
    validPredictions.reduce((sum, p) => sum + p.seasonalAdjustment, 0) /
    validPredictions.length;
  ...
  const factorCounts = allFactors.reduce(
    (counts, factor) => {
      counts[factor.factor] =
        (counts[factor.factor] || 0) + Math.abs(factor.impact);
      return counts;
    },
  ...
  const interventionsRequired = validPredictions.reduce(
...
async function generateRevenueForecast(
  ...
  return {
    ...
    seasonalFactors: forecast.seasonalFactors.map(
...
async function generateViralPredictions(
  ...
  console.log('🚀 Generating viral predictions...');

  // Get platform viral prediction
  const platformPrediction = await viralPredictor.predictViralGrowth(
    'platform',
    'platform',
  );
  ...
  const topPerformers =
    topSuppliers
      ?.map((s) => s.business_name || 'Unnamed Business')
      .slice(0, 3) || [];

  return {
    platformViralCoefficient:
      Math.round(platformPrediction.viralCoefficient * 100) / 100,
...
async function generateLTVPredictions(
  ...
  console.log('💎 Generating LTV predictions...');
  ...
  const ltvPredictions = await Promise.allSettled(
    suppliers.slice(0, 20).map((supplier) =>
      ltvPredictor.predictLTV(supplier.id).catch((error) => {
        console.warn(
          `LTV prediction failed for supplier ${supplier.id}:`,
          error,
        );
        return null;
      }),
  ...
  const averageLTV =
    validPredictions.reduce((sum, p) => sum + p.predictedLTV, 0) /
    validPredictions.length;

  // LTV distribution by segment
  const distribution = validPredictions.reduce(
    (dist, p) => {
  ...
  const highValueCustomers = validPredictions.filter(
    (p) => p.ltvSegment === 'gold' || p.ltvSegment === 'platinum',
  ).length;
  const optimizationOpportunities = validPredictions.reduce(
