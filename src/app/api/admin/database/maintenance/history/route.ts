...
export async function GET(request: NextRequest) {
  try {
    ...
    const { success, limit, reset, remaining } =
      await maintenanceHistoryRateLimit.limit(request.ip || 'anonymous');
      ...
      data: {
        ...
        metadata: {
          ...
          nextRecommendedMaintenance:
            await getNextRecommendedMaintenance(supabase),
          criticalMaintenancePending: recommendations.filter(
    ...
    if (bloatedTables && bloatedTables.length > 0) {
      bloatedTables.forEach((table: any, index: number) => {
        ...
        scheduledOps.push({
          ...
          estimatedDuration:
            table.n_dead_tup > 50000 ? '15-30 minutes' : '5-15 minutes',
 ...
 */
async function getMaintenanceRecommendations(
  supabase: any,
  ...
  try {
    ...
    const missingIndexRecommendations =
      await generateMissingIndexRecommendations(supabase);
    ...
    if (bloatedTables && bloatedTables.length > 0) {
      bloatedTables.forEach((table: any) => {
        ...
        recommendations.push({
          ...
          estimatedDuration:
            table.n_dead_tup > 50000 ? '10-30 minutes' : '2-10 minutes',
          weddingSeasonSafe: !isWeddingCriticalTable(table.tablename),
          lastRecommendedAt: new Date().toISOString(),
          issueDetectedAt: new Date().toISOString(),
        });
 ...
 */
async function generateMissingIndexRecommendations(
  supabase: any,
): Promise<MaintenanceRecommendation[]> {
  try {
 ...
 */
async function getMaintenanceSummaryStats(
  supabase: any,
 ...
 */
async function getNextRecommendedMaintenance(
  supabase: any,
): Promise<string | null> {
  try {
