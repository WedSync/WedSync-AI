  ...
  try {
    ...
    if (
      ...
      responseData.data.services[
        service as keyof typeof responseData.data.services
      ]
    ) {
      responseData.data = {
        ...responseData.data,
...
function getOverallHealthStatus(
  ...
  const criticalDown = criticalServices.some(
    (service) =>
      health.services[service as keyof typeof health.services]?.status ===
      'down',
  );
  ...
  const anyDegraded = Object.values(health.services).some(
