...
export class AdminWebhookProcessorService {
  ...
  private registerProcessors() {
    ...
    this.processors.set('supabase', {
      ...
      shouldNotifyAdmin: (event) =>
        event.type === 'database_error' || event.type === 'auth_error',
    });
    ...
    this.processors.set('tave', {
      ...
      shouldNotifyAdmin: (event) =>
        event.type === 'sync_error' || event.type === 'api_limit_exceeded',
    });
  ...
  async processWebhook(
    ...
    try {
      ...
      if (!processor) {
        throw new Error(
          `No processor registered for webhook source: ${source}`,
        );
      }
      ...
      if (!processingResult.success) {
        ...
        if (processor.shouldNotifyAdmin?.(eventData)) {
          await this.sendWebhookFailureNotification(
            source,
            eventData,
      ...
      // Send admin notification if this should notify admin
      if (processor.shouldNotifyAdmin?.(eventData)) {
        await this.sendWebhookSuccessNotification(
          source,
          eventData,
    ...
    try {
      ...
      switch (event.type) {
        ...
        default:
          return {
            success: true,
            result: { message: 'Event acknowledged but not processed' },
          };
      }
    } catch (error) {
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Stripe processing error',
      };
    }
  }

  // Twilio event processor
  private async processTwilioEvent(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Twilio processing error',
      };
    }
  }

  // Supabase event processor
  private async processSupabaseEvent(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
      ...
      switch (event.type) {
        ...
        default:
          return {
            success: true,
            result: { message: 'Supabase event acknowledged' },
          };
      }
    } catch (error) {
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Supabase processing error',
      };
    }
  }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Google Calendar processing error',
      };
    }
  }

  // Tave CRM event processor
  private async processTaveEvent(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
  ...
  private async processSystemHealthEvent(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
      logger.info('Processing system health event', {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'System health processing error',
      };
    }
  }
  ...
  private async handleStripeInvoiceFailure(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
  ...
  private async handleStripeSubscriptionCanceled(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
    // Implementation for handling subscription cancellations
    return {
      success: true,
      result: { action: 'subscription_canceled_handled' },
    };
  }
  ...
  private async handleSupabaseDatabaseError(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
  ...
  private async handleSupabaseAuthError(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
  ...
  private async handleGoogleCalendarSyncFailure(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
    // Implementation for handling calendar sync failures
    return {
      success: true,
      result: { action: 'calendar_sync_failure_handled' },
    };
  }

  private async handleTaveSyncError(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
  ...
  private async handleTaveApiLimitExceeded(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
  ...
  private async handleCriticalSystemEvent(
    event: any,
  ): Promise<{ success: boolean; result?: any; error?: string }> {
    // Implementation for handling critical system events
    return {
      success: true,
      result: { action: 'critical_system_event_handled' },
    };
  }
  ...
  private async sendWebhookSuccessNotification(
    source: string,
    eventData: any,
    result: any,
  ): Promise<void> {
