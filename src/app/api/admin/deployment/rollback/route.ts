...
export async function POST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Invalid confirmation code. Required: EMERGENCY_ROLLBACK_CONFIRMED',
          timestamp,
        },
        { status: 400 },
      );
    ...
    const isWeddingHours =
      isWeddingDay && currentHour >= 8 && currentHour <= 22;

    if (isWeddingHours) {
      console.warn(`🚨 SATURDAY WEDDING DAY ROLLBACK INITIATED`, {
        userId: user.id,
        deploymentId,
    ...
    console.log(`🔄 Starting rollback to deployment ${deploymentId}`, {
      userId: user.id,
    ...
    try {
      ...
      console.error(`❌ Rollback failed for deployment ${deploymentId}`, {
        error:
      ...
      const errorResponse: RollbackResponse = {
        success: false,
        error:
          rollbackError instanceof DeploymentError
            ? rollbackError.message
    ...
    const criticalErrorResponse: RollbackResponse = {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Critical rollback system failure',
      timestamp,
      metadata: {
        duration: Date.now() - rollbackStart,
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to get rollback information',
      },
      { status: 500 },
    );
  }
