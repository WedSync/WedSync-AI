  ...
  try {
    ...
    if (
      ...
      recommendations.push(
        'Scanned PDF detected - OCR processing will be used',
      );
    }
    ...
    if (content.includes('/AcroForm') || content.includes('/Field')) {
      hasFormFields = true;
      recommendations.push(
        'PDF form fields detected - these will be processed',
      );
    }
    ...
    if (estimatedPageCount > 20) {
      recommendations.push(
        'Large document detected - consider splitting into smaller files',
      );
    }
