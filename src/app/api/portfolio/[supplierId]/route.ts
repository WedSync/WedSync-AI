...
const portfolioQuerySchema = z.object({
  ...
  category: z
    .enum(['ceremony', 'reception', 'portraits', 'details', 'venue'])
    .optional(),
  featured: z
    .string()
  ...
  search: z
    ...
    .optional(),
  sortBy: z
    .enum(['created_at', 'views', 'likes', 'ai_score', 'title'])
    .default('created_at'),
...
export const GET = withQueryValidation(
  ...
  async (request: NextRequest, validatedQuery, { params }: RouteParams) => {
    try {
      ...
      const resultImages = hasMore
        ? images.slice(0, validatedQuery.limit)
        : images;
      const nextCursor = hasMore
        ? resultImages[resultImages.length - 1].created_at
        : null;
      ...
      const transformedImages = resultImages.map((img) => ({
        ...
        analytics: validatedQuery.include_analytics
          ? img.portfolio_analytics
          : undefined,
      }));
      ...
      const summary = portfolioSummary?.reduce(
        (acc, img) => {
          acc.totalImages++;
          acc.categoryCounts[img.category] =
            (acc.categoryCounts[img.category] || 0) + 1;
...
export const POST = withSecureValidation(
  ...
  async (request: NextRequest, validatedData, { params }: RouteParams) => {
    try {
      ...
      if (
        accessError ||
        imageAccess.length !== validatedData.image_ids.length
      ) {
        return NextResponse.json(
