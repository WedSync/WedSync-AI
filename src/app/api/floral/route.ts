...
async function getFlowers(supabase: any, searchParams: URLSearchParams) {
  ...
  let query = supabase.from('flowers').select('*').eq('is_active', true);
...
async function getTemplates(supabase: any, searchParams: URLSearchParams) {
  ...
  let query = supabase.from('floral_arrangement_templates').select('*');
  ...
  const { data: templates, error } = await query.order('user_rating', {
    ascending: false,
  });
