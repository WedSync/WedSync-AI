...
export async function POST(request: NextRequest) {
  try {
    ...
    if (template_id) {
      ...
      const { data: estimate, error } = await supabase.rpc(
        'calculate_arrangement_cost',
        {
          template_id,
          region: targetRegion,
      ...
      return NextResponse.json(
        {
          error:
            'Either template_id or (custom_flowers, stem_counts, arrangement_type) must be provided',
        },
        { status: 400 },
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .select(
        'id, name, arrangement_type, style_category, estimated_cost_range, labor_difficulty, size_category',
      );
    ...
    const { data: templates, error } = await query.order(
      'estimated_cost_range->min',
    );
  ...
  for (const flowerName of flowerNames) {
    ...
    if (flowers && flowers.length > 0) {
      ...
      const seasonalMultiplier =
        flower.seasonal_flower_availability?.[0]?.cost_multiplier || 1.0;
      ...
      flowerDetails.push({
        ...
        total: totalCost,
        availability:
          flower.seasonal_flower_availability?.[0]?.availability_status ||
          'unknown',
      });
    }
  }
