...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get basic stats
    const basicStats =
      await knowledgeEngine.getKnowledgeBaseStats(organization_id);
    ...
    if (detailed) {
      try {
        ...
        const contentStats = volumeStats?.reduce(
          (acc, doc) => {
            const wordCount = doc.content.split(/\s+/).length;
            acc.total_words += wordCount;
            acc.total_characters += doc.content.length;
            acc.average_words = Math.round(
              acc.total_words / volumeStats.length,
            );
            return acc;
          },
          {
        ...
        const timeline = groupByTimePeriod(
          timelineData || [],
          timelineGrouping,
        );
...
function generateRecommendations(basicStats: any, detailedAnalytics?: any) {
  ...
  if (basicStats.total_documents < 10) {
    recommendations.push({
      ...
      priority: 'high',
      message:
        'Consider adding more documents to improve search quality. Aim for at least 20-30 core documents.',
      action:
        'Add more FAQ entries, service descriptions, and vendor information',
    });
  }
  ...
  if (basicStats.categories?.length < 5) {
    recommendations.push({
      ...
      priority: 'medium',
      message:
        'Diversify content across more categories to better serve different user needs.',
  ...
  if (detailedAnalytics?.content_statistics?.average_words < 50) {
    recommendations.push({
      ...
      priority: 'medium',
      message:
        'Documents seem quite short. Longer, more detailed content improves search relevance.',
  ...
  const hasRecentActivity =
    basicStats.last_updated &&
    new Date(basicStats.last_updated) >
  ...
  if (!hasRecentActivity) {
    recommendations.push({
      ...
      priority: 'low',
      message:
        "Knowledge base hasn't been updated recently. Fresh content improves user experience.",
      action:
        'Review and update existing content, add new seasonal information',
    });
  }
