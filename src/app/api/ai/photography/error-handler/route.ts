...
async function getHealthStatus(): Promise<NextResponse> {
  try {
    ...
    const isHealthy = Object.values(stats.circuitBreakerStates).every(
      (state) => state === 'CLOSED',
    );
    ...
    const healthStatus = {
      ...
      services: {
        colorAnalysis:
          stats.circuitBreakerStates['color-analysis'] === 'CLOSED'
            ? 'operational'
            : 'degraded',
        styleMatching:
          stats.circuitBreakerStates['style-matching'] === 'CLOSED'
            ? 'operational'
            : 'degraded',
        moodBoard:
          stats.circuitBreakerStates['mood-board'] === 'CLOSED'
            ? 'operational'
            : 'degraded',
        openAiVision:
          stats.circuitBreakerStates['openai-vision'] === 'CLOSED'
            ? 'operational'
            : 'degraded',
      },
      metrics: {
  ...
  try {
    ...
    const aiError: AiPhotographyError = {
      ...
      userMessage:
        ...
        'An error occurred in the AI photography features.',
      context: {
...
async function resetErrorHandler(): Promise<NextResponse> {
  try {
    ...
    const authHeader =
      process.env.NODE_ENV === 'development' ? 'valid' : 'invalid';
