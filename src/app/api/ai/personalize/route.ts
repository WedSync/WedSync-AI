...
const AI_PERSONALIZATION_RATE_LIMITS: Record<
  string,
 ...
 */
function validateAIAccess(
  subscriptionTier: string,
  aiCreditsUsed: number,
  aiCreditsLimit: number | null,
) {
  const tierConfig =
    AI_PERSONALIZATION_RATE_LIMITS[subscriptionTier.toUpperCase()];

  if (!tierConfig || tierConfig.requests === 0) {
    throw new Error(
      `AI personalization not available for ${subscriptionTier} tier. Upgrade to Professional or higher.`,
    );
  }
  ...
  if (aiCreditsLimit !== null && aiCreditsUsed >= aiCreditsLimit) {
    throw new Error(
      'AI credits limit reached. Upgrade your plan or wait for renewal.',
    );
  }
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        error.message.includes('credits limit')
      ) {
