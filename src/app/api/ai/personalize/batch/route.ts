...
const BATCH_LIMITS: Record<
  string,
  { maxItems: number; maxConcurrent: number }
> = {
...
function validateBatchAccess(
  ...
  if (!tierConfig || tierConfig.maxItems === 0) {
    throw new Error(
      `Batch personalization not available for ${subscriptionTier} tier. Upgrade to Professional or higher.`,
    );
  }

  if (requestCount > tierConfig.maxItems) {
    throw new Error(
      `Batch size exceeds limit. Maximum ${tierConfig.maxItems} items allowed for ${subscriptionTier} tier.`,
    );
  }
  ...
  if (
    aiCreditsLimit !== null &&
    aiCreditsUsed + requestCount > aiCreditsLimit
  ) {
    throw new Error(
      `Insufficient AI credits. Required: ${requestCount}, Available: ${aiCreditsLimit - aiCreditsUsed}`,
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const batchName =
      validatedData.batchName || `Batch ${new Date().toISOString()}`;

    // Create batch record
    await supabase.from('personalization_batches').insert({
      id: batchId,
    ...
    const insertPromises = results.map((result, index) =>
      supabase.from('personalizations').insert({
        batch_id: batchId,
        supplier_id: supplier.id,
        original_content: result.originalContent,
        personalized_content: result.personalizedContent,
        context_type:
          validatedData.requests[index].personalizationSettings.context,
    ...
    return NextResponse.json({
      ...
      usage: {
        ...
        creditsRemaining: supplier.ai_credits_limit
          ? supplier.ai_credits_limit -
            ((supplier.ai_credits_used || 0) + creditsUsed)
          : null,
