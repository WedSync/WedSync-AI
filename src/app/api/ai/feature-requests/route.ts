...
const QuerySchema = z.object({
  ...
  user_role: z
    ...
    .optional(),
  sort_by: z
    .enum(['created_at', 'rice_score', 'priority_level', 'title'])
    .default('created_at'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build base query
    let baseQuery = supabase.from('feature_requests').select(`
        *,
    ...
    const { count, error: countError } = await supabase
      ...
      .eq(
        'organization_id',
        profile.role === 'admin' ? undefined : profile.organization_id,
      );

    if (countError) {
      logger.error('Failed to get feature requests count', {
        error: countError,
      });
    }
