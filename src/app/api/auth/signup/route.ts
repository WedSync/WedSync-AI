...
import {
  coupleAuthService,
  type CoupleSignupData,
} from '@/lib/services/coupleAuthService';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (signupData.invitationToken) {
      const invitation = await coupleAuthService.validateInvitation(
    ...
    return NextResponse.json({
      ...
      next_step: signupData.invitationToken
        ? 'complete_onboarding'
        : 'setup_wedding_details',
    });
  } catch (error: any) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const validation =
      await coupleAuthService.validateInvitation(invitationToken);
    ...
    const supplierName =
      invitation.vendors?.business_name ||
