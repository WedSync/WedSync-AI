...
const subscribeRequestSchema = z.object({
  channels: z
    ...
    .max(5, 'Maximum 5 channels per request'),
...
async function getOrganizationLimits(organizationId: string) {
  ...
  const tierLimits = {
    ...
    STARTER: {
      maxConnections: 50,
      allowedChannels: ['form_submissions', 'client_messages'],
    },
    PROFESSIONAL: {
      maxConnections: 200,
      allowedChannels: [
...
export async function POST(request: NextRequest) {
  ...
  try {
        ...
        const subscriptionParams: EnhancedRealtimeSubscriptionParams = {
          ...
          filters: {
            ...
            filter: buildFilterString(
              channelType as WeddingChannelType,
    ...
    // 7. Success response
    const successfulSubscriptions = subscriptionResults.filter(
      (r) => r.status === 'active',
    );
    const totalActiveSubscriptions =
      currentMetrics.activeSubscriptions + successfulSubscriptions.length;
