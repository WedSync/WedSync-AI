...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const testResponses: PerformanceTestResponse[] = (tests || []).map(
      (test) => ({
        id: test.id,
        name: test.name,
    ...
    const testStats = {
      ...
      completedTests:
        stats?.filter((t) => t.status === 'completed').length || 0,
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (
      weddingSizeCategory &&
      !validWeddingSizes.includes(weddingSizeCategory)
    ) {
      return NextResponse.json(
        {
