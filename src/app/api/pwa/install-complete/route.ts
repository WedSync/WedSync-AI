...
const installCompleteSchema = z.object({
  ...
  installation_method: z
    .enum(['browser_prompt', 'manual_menu', 'share_menu', 'other'])
    .optional(),
  device_info: z
    .object({
      platform: z
        .string()
        .optional()
        .transform((val) => (val ? sanitizePlatform(val) : null)),
...
async function hashIP(ip: string): Promise<string> {
  ...
  try {
    ...
    const data = encoder.encode(
      ip + process.env.IP_HASH_SALT || 'wedsync-pwa-salt-2025',
    );
...
async function logInstallCompleteEvent(data: any, clientInfo: any) {
  try {
    ...
    if (error) {
      logger.error('Failed to log PWA installation complete event', {
        error,
        data,
      });
      return false;
    ...
    logger.error('Exception logging PWA installation complete event', {
      error,
    });
    return false;
  }
...
async function updateInstallationMetrics(userId?: string, clientInfo?: any) {
  try {
    ...
    const { error } = await supabase.from('pwa_analytics_summary').upsert(
      {
        date: today,
