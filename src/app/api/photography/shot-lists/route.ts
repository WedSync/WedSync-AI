...
export async function POST(request: NextRequest) {
  try {
    ...
    const generatedShotList =
      await shotListGenerator.generateShotList(generationParams);
    ...
    const processedShotLists =
      shotLists?.map((shotList) => ({
        ...
        analytics: {
          ...
          shots_by_priority: analyzeShotsByPriority(
            shotList.generated_shots || [],
          ),
        },
      })) || [];
...
function summarizeEquipment(equipment: any[]) {
  return {
    ...
    essential_count: equipment.filter((item) => item.priority === 'essential')
      .length,
    recommended_count: equipment.filter(
