...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.generate_variations) {
      ...
      const moodBoardVariations =
        await moodBoardBuilder.generateMoodBoardVariations(
          baseRequest,
          variations,
        );
    ...
    const generatedMoodBoard =
      await moodBoardBuilder.generateMoodBoard(moodBoardRequest);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: moodBoards, error: moodBoardsError } = await supabase
      .from('mood_boards')
      .select(
        'id, title, description, style_summary, created_at, mood_board_data',
      )
      .eq('user_id', user.id)
    ...
    return NextResponse.json({
      ...
      mood_boards: moodBoards.map((board) => ({
        ...
        created_at: board.created_at,
        primary_colors:
          board.mood_board_data?.color_palette?.primary_colors || [],
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Update mood board
    const updatedMoodBoard = await moodBoardBuilder.updateMoodBoard(
      mood_board_id,
      updates,
    );
