...
export const POST = withFeatureGate('ai:photo_processing', {
  ...
  try {
    ...
    const coordinatedAnalysis =
      await photographyAICoordinator.analyzeWithFullIntegration({
    ...
    coordinatedAnalysis.integration_metrics.processing_time_ms =
      Date.now() - startTime;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!analysisId) {
      return createErrorResponse(
        'MISSING_PARAMETER',
        'analysis_id parameter required',
        400,
        requestId,
        startTime,
      );
...
async function logAnalysis(
  ...
  try {
    await supabase.from('photo_analysis_records').insert({
      ...
      request_data: {
        mood_board_id:
          logData.analysis_result.photography_analysis.mood_board_id,
      },
      response_data: logData.analysis_result,
      processing_time_ms: logData.processing_time_ms,
      integration_scores: {
        style_consistency:
          logData.analysis_result.style_consistency.style_match_score,
        color_harmony:
          logData.analysis_result.color_harmony.color_harmony_score,
        overall_coherence:
          logData.analysis_result.integration_metrics.overall_coherence_score,
      },
      teams_integrated:
        logData.analysis_result.integration_metrics.teams_integrated,
