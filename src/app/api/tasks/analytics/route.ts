  ...
  try {
    ...
    switch (action) {
      case 'wedding_progress':
        ...
        const analytics =
          await analyticsService.getWeddingProgressMetrics(weddingId);
        return NextResponse.json({
          success: true,
          analytics,
        });
      ...
      case 'trend_data':
        ...
        const trendData = await analyticsService.getTaskTrendData(
          weddingId,
          trendDays,
        );
        return NextResponse.json({
      ...
      case 'predictive_analytics':
        ...
        const predictiveAnalytics =
          await analyticsService.getPredictiveAnalytics(weddingId);
        return NextResponse.json({
          success: true,
  ...
  try {
    ...
    switch (action) {
      case 'refresh_analytics':
        ...
        const [analytics, teamMetrics, trendData, predictiveAnalytics] =
          await Promise.all([
      ...
      summary: {
        ...
        team_efficiency_avg:
          teamMetrics.length > 0
            ? teamMetrics.reduce((sum, m) => sum + m.efficiency_score, 0) /
              teamMetrics.length
            : 0,
    ...
    if (format === 'pdf') {
      ...
      return {
        ...
        files: [
          {
            name: 'summary.csv',
            url: `/api/reports/download/${weddingId}/summary.csv`,
          },
          {
            name: 'team_performance.csv',
            url: `/api/reports/download/${weddingId}/team.csv`,
          },
          {
            name: 'trends.csv',
            url: `/api/reports/download/${weddingId}/trends.csv`,
          },
        ],
      };
    }
...
function generateRecommendations(exportData: any): string[] {
  ...
  if (summary.completion_rate < 50 && analytics.days_until_wedding < 30) {
    recommendations.push(
      '🚨 URGENT: Consider adding additional resources or extending timeline. Less than 50% complete with 30 days remaining.',
    );
  }

  if (summary.overdue_tasks > 0) {
    recommendations.push(
      `📅 Address ${summary.overdue_tasks} overdue tasks immediately to prevent cascading delays.`,
    );
  }
  ...
  if (summary.velocity < 2 && analytics.days_until_wedding < 14) {
    recommendations.push(
      '⚡ Increase task completion velocity by enabling parallel work streams or adding team members.',
    );
  }
  ...
  if (summary.team_efficiency_avg < 70) {
    recommendations.push(
      '👥 Review team workload distribution and provide additional support to struggling team members.',
    );
  }
  ...
  if (analytics.risk_level === 'high' || analytics.risk_level === 'critical') {
    recommendations.push(
      '⚠️ Implement daily standups and consider bringing in experienced wedding coordinators.',
    );
  }

  // Category-specific recommendations
  const underperformingCategories = analytics.category_breakdown.filter(
    (cat) => cat.completion_rate < 30,
  );
  if (underperformingCategories.length > 0) {
    recommendations.push(
      `📊 Focus immediate attention on underperforming categories: ${underperformingCategories.map((c) => c.category).join(', ')}`,
    );
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push(
      '✅ Project is on track! Continue current pace and monitor for any emerging bottlenecks.',
    );
  }
