...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await routeTaskGetAction(
      action,
      searchParams,
 ...
 */
async function routeTaskGetAction(
  action: string,
    ...
    const result = await taskCreationService.createSingleTask(
      data.wedding_id,
      {
        ...
        deadline: validated_data.deadline
          ? new Date(validated_data.deadline)
          : undefined,
      },
      user.id,
    );
  ...
  try {
    ...
    const result = await taskCreationService.validateTaskTiming(
      data.wedding_id,
      {
        ...
        deadline: validated_data.task_data.deadline
          ? new Date(validated_data.task_data.deadline)
          : undefined,
      },
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        risk_level: conflicts.some((c) => c.severity === 'critical')
          ? 'critical'
