...
const appropriatenessAnalysisSchema = z.object({
  ...
  wedding_context: z
    .object({
      ...
      venue_type: z
        .enum(['church', 'outdoor', 'ballroom', 'beach', 'other'])
        .optional(),
      event_type: z
...
class WeddingMusicAI {
  async analyzeAppropriateness(
    ...
    if (hasInappropriateContent) {
      score = 0.3;
      issues.push(
        'Contains themes that may not be appropriate for wedding celebrations',
      );
  ...
  async cacheAnalysis(
    ...
    console.log(
      `Caching analysis for track ${trackId} in org ${organizationId}`,
    );
  }
...
export const POST = withAnalysisRateLimit(
  ...
  >({
    ...
    handler: async ({
      user,
      body,
    }): Promise<ApiResponse<AppropriatenessAnalysis>> => {
      try {
        ...
        let analysis = await weddingMusicAI.getCachedAnalysis(
          trackId,
          user.organizationId,
        );
        ...
        if (!analysis) {
            ...
            wedding_context
              ? {
                  cultural_considerations:
                    wedding_context.cultural_considerations,
          ...
          await weddingMusicAI.cacheAnalysis(
            trackId,
            analysis,
            user.organizationId,
          );
        ...
        throw ApiException.internal('Failed to analyze music appropriateness', {
