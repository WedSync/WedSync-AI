...
function checkWeddingDayProtocols(ticket: any): {
  ...
  if (
    ticket.hours_until_wedding &&
    ticket.hours_until_wedding <= 48 &&
...
function checkTicketEscalation(
  ticket: any,
  now: Date,
): {
  shouldEscalate: boolean;
  ...
  const resolutionTimeMinutes = needsResolution
...
async function triggerWeddingDayEscalation(
  supabase: any,
  ticket: any,
  emergency: any,
) {
  console.log(
    `🚨 WEDDING DAY EMERGENCY: Ticket ${ticket.id} - ${emergency.reason}`,
  );
  ...
  for (const action of emergency.protocol.actions) {
    try {
      ...
      console.error(
        `Failed to execute wedding day action ${action}:`,
        actionError,
      );
    }
  }
...
async function triggerEscalation(supabase: any, ticket: any, escalation: any) {
  console.log(
    `📈 ESCALATION: Ticket ${ticket.id} to level ${escalation.level} - ${escalation.reason}`,
  );
  ...
  for (const action of escalation.actions) {
    try {
      ...
      console.error(
        `Failed to execute escalation action ${action}:`,
        actionError,
      );
    }
  }
...
async function notifyWeddingSpecialists(
  supabase: any,
  ticket: any,
...
async function attemptAutoReassignment(supabase: any, ticket: any) {
  ...
  if (availableAgents?.length > 0) {
    ...
    // Notify new agent
    await notifyAssignedAgent(supabase, {
      ...ticket,
...
async function logSLABreach(supabase: any, ticket: any) {
  await supabase.from('support_sla_events').insert({
    ticket_id: ticket.id,
    event_type: 'breach',
...
async function createEmergencyChannel(supabase: any, ticket: any) {
  ...
  await supabase.from('emergency_channels').insert({
    ticket_id: ticket.id,
  ...
  try {
    ...
    if (action === 'stats') {
      ...
      escalations?.forEach((esc) => {
        stats.byLevel[esc.escalation_level] =
          (stats.byLevel[esc.escalation_level] || 0) + 1;
        stats.byType[esc.escalation_type] =
          (stats.byType[esc.escalation_type] || 0) + 1;
