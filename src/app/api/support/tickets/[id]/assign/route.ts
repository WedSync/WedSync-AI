...
const AssignTicketSchema = z
  ...
  .refine(
    (data) => data.assigned_to || data.assigned_team || data.auto_assign,
    {
  ...
  try {
    ...
    if (targetAgent && !ticket.first_response_at && !ticket.assigned_to) {
      ...
      await supabase.from('ticket_sla_events').insert({
        ...
        minutes_to_target: Math.round(
          (new Date().getTime() -
            new Date(ticket.sla_first_response_deadline).getTime()) /
            (1000 * 60),
        ),
    ...
    if (targetAgent) {
      ...
      try {
        ...
        console.log(
          'Note: Notifications table not available, skipping notification',
        );
      }
    }
