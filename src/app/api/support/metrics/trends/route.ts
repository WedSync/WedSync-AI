...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const { data: trends, error } = await supabase.rpc('get_support_trends', {
      ...
      if (error || !trends) {
        ...
        const fallbackTrends = await calculateTrendsManually(
          supabase,
          startDate,
          endDate,
          intervalDuration,
      ...
      console.error(
        'Error with stored procedure, falling back to manual calculation:',
        error,
      );
      const fallbackTrends = await calculateTrendsManually(
        supabase,
    ...
    const trends = intervals.map((interval) => {
      ...
      const resolvedTickets = tickets.filter((ticket) => {
        const resolvedAt = ticket.resolved_at
          ...
          : null;
        return (
          resolvedAt && resolvedAt >= intervalStart && resolvedAt < intervalEnd
        );
      });
      ...
      return {
        ...
        wedding_emergencies: intervalTickets.filter(
          (t) => t.is_wedding_emergency,
        ).length,
        critical_tickets: intervalTickets.filter(
          (t) => t.priority === 'critical',
        ).length,
        escalated_tickets: intervalTickets.filter((t) => t.escalation_level > 0)
          .length,
      };
    });
