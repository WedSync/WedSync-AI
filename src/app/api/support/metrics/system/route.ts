...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const { data: systemMetrics, error } = await supabase.rpc(
      ...
      if (error || !systemMetrics) {
        ...
        const fallbackMetrics = await calculateSystemMetricsManually(
          supabase,
          startDate,
          endDate,
        );
 ...
 */
async function calculateSystemMetricsManually(
  supabase: any,
  startDate: Date,
  endDate: Date,
) {
  try {
    ...
    // Tier breakdown
    const tierBreakdown = tickets.reduce(
      (acc, ticket) => {
        const tier = ticket.customer_tier || 'free';
