...
function calculateWeddingUrgency(
  description: string,
  weddingDate?: string,
): {
  urgencyScore: number;
  ...
  if (weddingDate) {
    ...
    const hoursUntilWedding =
      (wedding.getTime() - now.getTime()) / (1000 * 60 * 60);
  ...
  try {
    ...
    const scoredAgents = agents.map((agent) => {
      ...
      if (
        ...
        agent.is_wedding_day_specialist
      ) {
        score += 25;
      }
  ...
  try {
    ...
    if (!subject || !description || !organizationId) {
      return NextResponse.json(
        {
          error:
            'Missing required fields: subject, description, organizationId',
        },
        { status: 400 },
      );
    }
    ...
    if (patternResult.confidence < 70) {
      ...
      classification = {
        ...
        requiresWeddingSpecialist:
          urgencyData.isWeddingDayIssue || aiResult.category === 'wedding_day',
    ...
    return NextResponse.json(
      {
        ...
        classification: {
          ...
          reasoning:
            ...
            `Classified via ${classificationMethod}`,
        },
        urgency: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'patterns') {
      ...
      const patterns = Object.entries(WEDDING_PATTERNS).map(
        ([key, pattern]) => ({
          id: key,
          ...
          requiresWeddingSpecialist:
            pattern.requires_wedding_specialist || false,
        }),
      );
    ...
    if (action === 'stats') {
      ...
      stats?.forEach((record) => {
        ...
        // Method distribution
        methodStats[record.classification_method] =
          (methodStats[record.classification_method] || 0) + 1;
