...
async function verifySupplierConfirmationAccess(
  ...
  if (accessToken) {
    try {
      ...
      if (
        decoded.supplier_id !== supplierId ||
        decoded.timeline_id !== timelineId
      ) {
...
export const POST = withSecureValidation(
  ...
  async (
    request: NextRequest,
    validatedData: z.infer<typeof scheduleConfirmationSchema>,
  ) => {
    try {
      ...
      if (
        existingSchedule.confirmation_status === 'confirmed' &&
      ...
      if (
        validatedData.event_confirmations &&
        validatedData.event_confirmations.length > 0
      ) {
        const eventConfirmations = validatedData.event_confirmations.reduce(
          (acc: any, eventConf: any) => {
        ...
        if (updatedScheduleData.schedule_items) {
          updatedScheduleData.schedule_items =
            updatedScheduleData.schedule_items.map((item: any) => ({
              ...item,
              confirmation: eventConfirmations[item.event_id] || {
                status: 'pending',
              },
              supplier_confirmed_at: eventConfirmations[item.event_id]
      ...
      return NextResponse.json(
        {
          ...
          data: {
            ...
            schedule_summary: {
              ...
              confirmed_events:
                validatedData.event_confirmations?.filter(
...
export async function GET(
  ...
  try {
    ...
    const confirmationData = {
      ...
      schedule_items:
        existingSchedule.schedule_data?.schedule_items?.map((item: any) => ({
