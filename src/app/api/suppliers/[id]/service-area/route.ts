    ...
    try {
      ...
      if (includeOptimization) {
        response.optimization =
          await serviceRadiusCalculator.optimizeServiceArea(supplierId);
      }
...
export async function PUT(
  ...
  try {
    ...
    if (
      !serviceType ||
      !['radius', 'polygon', 'regions', 'nationwide'].includes(serviceType)
    ) {
      return NextResponse.json(
        {
          error:
            'Valid service type is required (radius, polygon, regions, nationwide)',
        },
        { status: 400 },
      );
    }
    ...
    if (
      baseLocation.latitude < -90 ||
      baseLocation.latitude > 90 ||
      baseLocation.longitude < -180 ||
      baseLocation.longitude > 180
    ) {
      return NextResponse.json(
    ...
    if (
      serviceType === 'radius' &&
      (!body.radiusKm || body.radiusKm < 1 || body.radiusKm > 1000)
    ) {
      return NextResponse.json(
        {
          error:
            'Valid radius in kilometers (1-1000) is required for radius service type',
        },
        { status: 400 },
      );
    }
    ...
    if (
      serviceType === 'polygon' &&
      (!body.polygonCoordinates || !Array.isArray(body.polygonCoordinates))
    ) {
      return NextResponse.json(
        {
          error:
            'Valid polygon coordinates are required for polygon service type',
        },
        { status: 400 },
      );
    }
