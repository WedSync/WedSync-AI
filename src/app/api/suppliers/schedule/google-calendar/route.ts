  ...
  sync_settings: z
    .object({
      ...
      conflict_resolution: z
        .enum(['manual', 'auto_reschedule', 'notify_only'])
        .default('manual'),
...
async function handleGoogleCalendarSetup(body: any, userId: string) {
  try {
    ...
    const { supplier_id, organization_id, oauth_code, sync_settings } =
      validatedData;
    ...
    const result =
      await SupplierGoogleCalendarService.setupSupplierGoogleCalendarSync(
        supplier_id,
        organization_id,
        oauth_code,
        sync_settings,
...
async function handleGoogleCalendarSync(body: any, userId: string) {
  try {
    ...
    const { supplier_ids, organization_id, force_sync, sync_window_days } =
      validatedData;
    ...
    const syncResult =
      await SupplierGoogleCalendarService.bulkSyncSuppliersToGoogleCalendar(
        supplierSchedules,
        organization_id,
      );
  ...
  try {
    ...
    switch (action) {
      case 'status':
        ...
        const status =
          await SupplierGoogleCalendarService.getSupplierCalendarSyncStatus(
            supplierId,
            organizationId,
          );
