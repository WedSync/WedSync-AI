...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      await supabase.from('supplier_sms_notification_log').insert({
        organization_id,
        schedule_event_id,
        notification_type,
...
async function handleIncomingSMSWebhook(webhookData: any) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'SMS response processed',
    });
  } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (supplierId) {
      supplierValidation =
        await SupplierScheduleSMSService.validateSupplierForSMS(supplierId);
    }
