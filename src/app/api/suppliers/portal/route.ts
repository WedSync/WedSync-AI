...
} from '@/lib/auth/supplier-auth-service';
  ...
  notification_preferences: z
    .object({
      ...
      notification_types: z
        .array(z.string())
        .default(['schedule_updates', 'messages', 'reminders']),
    })
    .optional(),
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'communications':
        return await handleGetCommunications(
          user.supplier_id,
          user.organization_id,
        );
...
async function handleGetCommunications(
  supplierId: string,
  organizationId: string,
) {
  try {
    const communications =
      await SupplierPortalService.getSupplierCommunicationHub(
        supplierId,
        organizationId,
      );
...
async function authenticateSupplierRequest(request: NextRequest): Promise<{
  ...
  try {
    const sessionToken =
      request.cookies.get('supplier_session')?.value ||
