...
async function handleFeedbackSubmission(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            'Subscription upgrade required to provide feedback on this article',
      ...
      return NextResponse.json(
        {
          ...
          message:
            'You have already submitted this type of feedback for this article',
    ...
    updateArticleAnalytics(
      supabase,
      validatedFeedback.article_id,
      validatedFeedback.feedback_type,
    ...
    const response = {
      ...
      data: {
        ...
        status: spamCheckResult.requiresModeration
          ? 'pending_moderation'
          : 'approved',
...
function performSpamDetection(
  ...
  const content = [feedback.comment, feedback.suggestion]
    .filter(Boolean)
    .join(' ');
  ...
  return {
    ...
    requiresModeration:
      spamScore >= 3 || (content.length > 500 && constructiveScore < 2),
    confidence,
  };
}
...
async function updateArticleAnalytics(
  ...
  try {
    ...
    if (feedbackType === 'helpful') {
      await supabase.rpc('increment_article_helpful_votes', {
        article_uuid: articleId,
      });
    } else if (feedbackType === 'not_helpful') {
      await supabase.rpc('increment_article_not_helpful_votes', {
        article_uuid: articleId,
      });
    }
