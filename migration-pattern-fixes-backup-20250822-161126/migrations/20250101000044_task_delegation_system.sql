...
CREATE POLICY "Task access" ON workflow_tasks
  FOR SELECT USING (
    ...
    created_by IN (SELECT id FROM team_members WHERE user_id = auth.uid()) OR
    assigned_by IN (SELECT id FROM team_members WHERE user_id = auth.uid())
  );
...
CREATE POLICY "Notification access" ON task_notifications
  FOR SELECT USING (
    recipient_id IN (SELECT id FROM team_members WHERE user_id = auth.uid())
  );

-- Workload metrics access
CREATE POLICY "Workload access" ON workload_metrics
  FOR SELECT USING (
    team_member_id IN (SELECT id FROM team_members WHERE user_id = auth.uid()) OR
    EXISTS (SELECT 1 FROM team_members WHERE user_id = auth.uid() AND role IN ('admin', 'manager'))
  );
